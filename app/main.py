...
class UserButtonAccessMiddleware(BaseMiddleware):
    """Block callbacks from old messages after an admin disables a user button."""

    CALLBACK_TO_SETTING = {
        'menu:buy': 'buy',
        'menu:my_services': 'my_services',
        'menu:account': 'account',
        'menu:test_account': 'test_account',
    ...
    async def __call__(self, handler, event, data):
        ...
        setting_name = self.CALLBACK_TO_SETTING.get(callback_data)
        if setting_name:
            _, enabled = await get_user_button(setting_name)
            if not enabled:
                try:
                    await event.answer('این بخش در حال حاضر توسط مدیریت غیرفعال شده است.', show_alert=True)
                except Exception:
                    pass
                return None
        return await handler(event, data)
...
class FreshCallbackOnlyMiddleware(BaseMiddleware):
    ...
    ALLOWED_PREFIXES = (
        ...
        'home:main', 'profile:home', 'back:main', 'report:home',
        ...
        # Private-message conversation controls are attached to proactive messages
        # and must work even when another menu message is the chat's active UI.
        'private_chat:', 'private_admin:',
    )
...
async def create_tables():
    ...
    async with engine.begin() as conn:
        ...
        upgrades = [
            ...
            "ALTER TABLE reseller_accounts ADD COLUMN IF NOT EXISTS inbound_mode VARCHAR(16) DEFAULT 'automatic'",
            "ALTER TABLE reseller_accounts ADD COLUMN IF NOT EXISTS inbound_ids JSON DEFAULT '[]'",
            "UPDATE reseller_accounts SET inbound_mode = 'automatic' WHERE inbound_mode IS NULL OR inbound_mode NOT IN ('automatic', 'manual')",
            "UPDATE reseller_accounts SET inbound_ids = '[]' WHERE inbound_ids IS NULL",
            ...
            'ALTER TABLE service_deletion_tasks ADD COLUMN IF NOT EXISTS admin_notified_at TIMESTAMP NULL',
...
async def main():
    ...
    dp.callback_query.middleware(UserButtonAccessMiddleware())
    ...
    for r in [
        ...
        account.router, my_services.router, tickets.router, buy.router, test_account.router, reseller.router, private_messages.router,
    ]:
        dp.include_router(r)
    ...
    scheduler.add_job(
        deliver_scheduled_backup,
        'interval',
        minutes=1,
        id='scheduled_backup_delivery',
        replace_existing=True,
        next_run_time=datetime.now(scheduler.timezone),
    )
    scheduler.add_job(
