...
from app.jobs.server_sync import sync_all_servers
        ...
        try:
            ...
            if message:
                ...
                if active_mid and int(active_mid) != int(message.message_id):
                    try:
                        await event.answer('این پیام قدیمی است. لطفاً از آخرین پیام ربات استفاده کنید.', show_alert=True)
                    except Exception:
                        pass
                    try:
...
async def main():
    ...
    scheduler.add_job(scan_payg_usage, 'interval', minutes=max(10, settings.PAYG_SCAN_MINUTES), id='payg_scanner', replace_existing=True)
    scheduler.add_job(scan_service_alerts, 'interval', minutes=45, args=[bot], id='service_alerts', replace_existing=True)
    scheduler.add_job(scan_anti_sharing, 'interval', minutes=10, args=[bot], id='anti_sharing_scanner', replace_existing=True)
    scheduler.add_job(sync_all_servers, 'interval', seconds=max(600, settings.SERVER_SYNC_SECONDS), id='server_inbound_sync', replace_existing=True)
    scheduler.start()
    try:
