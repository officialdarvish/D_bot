...
from app.services.reseller_service import apply_reseller_usage_delta, drop_inactive_reservation, sync_reseller_service_panel_snapshot
...
async def _process_disabled_services(bot, session, now: datetime) -> None:
    ...
    for svc in disabled:
        ...
        if is_test_service:
            ...
            # The dedicated cleanup worker removes them after six hours and
            # persists panel retries independently from this notification job.
            continue

        if age >= DISABLED_PURGE_AFTER:
            # A dedicated cleanup queue removes the local row at the deadline
            # and retries panel deletion even when the panel is temporarily
            # unreachable. Do not send more reminders after the grace period.
            continue
