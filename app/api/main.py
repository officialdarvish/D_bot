...
async def admin_json_error_middleware(request: Request, call_next):
    try:
        ...
        if wants_json:
            return JSONResponse({'ok': False, 'message': str(exc) or exc.__class__.__name__}, status_code=500)
        raise
...
async def _build_service(session, user: User, server: Server, plan: Plan, username: str):
    ...
    if server.server_type == 'xui':
        ...
        service.xui_uuid = _xui_text(created.get('uuid')) if isinstance(created, dict) else None
...
# STATIC UI
from fastapi.staticfiles import StaticFiles
app.mount('/ui', StaticFiles(directory='app/api/static', html=True), name='ui')
