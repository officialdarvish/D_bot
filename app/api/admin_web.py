...
async def server_test(request: Request, server_type:str=Form('xui'), panel_url:str=Form(...), panel_path:str=Form('/'), username:str=Form(''), password:str=Form(''), router_name:str=Form(''), _: str = Depends(_auth_user)):
    ...
    return JSONResponse({'ok': True, 'message': f'Connection OK. {len(inbounds)} inbound(s) found.', 'inbound_ids': [x['id'] for x in inbounds], 'inbounds': inbounds})
...
async def server_add(request: Request, server_type:str=Form('xui'), scope:str=Form('public'), name:str=Form(...), display_name:str=Form(''), panel_url:str=Form(...), panel_path:str=Form('/'), subscription_url:str=Form(''), username:str=Form(''), password:str=Form(...), router_name:str=Form(''), default_protocol:str=Form('openvpn'), openvpn_profile_id:int=Form(0), l2tp_server:str=Form(''), l2tp_ipsec_secret:str=Form(''), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        if server_type == 'mikrotik':
            ...
            meta={'display_name':display_name or name,'inbound_ids':[],'inbounds':[],'scope':scope if scope in ('public','reseller','all') else 'public','panel_base_url':final_url,'router_name':router,'default_protocol':default_protocol or 'openvpn','openvpn_profile_id': int(openvpn_profile_id or 0), 'l2tp_server': l2tp_server, 'l2tp_ipsec_secret': l2tp_ipsec_secret}
            srv=Server(name=name,server_type='mikrotik',panel_url=final_url,subscription_url=None,username=router,password_encrypted=encrypt_text(password),is_active=True,meta=meta)
        ...
        base_url, web_path, final_url = _normalize_panel_parts(panel_url, panel_path)
        srv=Server(name=name,server_type=server_type,panel_url=final_url,subscription_url=subscription_url or None,username=username,password_encrypted=encrypt_text(password),is_active=True,meta={'display_name':display_name or name,'inbound_ids':[],'inbounds':[],'scope':scope if scope in ('public','reseller','all') else 'public','panel_base_url':base_url,'panel_path':web_path})
        s.add(srv); await s.commit(); await s.refresh(srv)
...
async def server_edit(request: Request, sid:int, name:str=Form(...), scope:str=Form('public'), display_name:str=Form(''), panel_url:str=Form(...), panel_path:str=Form('/'), subscription_url:str=Form(''), username:str=Form(''), password:str=Form(''), router_name:str=Form(''), default_protocol:str=Form('openvpn'), openvpn_profile_id:int=Form(0), l2tp_server:str=Form(''), l2tp_ipsec_secret:str=Form(''), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        base_url, web_path, final_url = _normalize_panel_parts(panel_url, panel_path)
        srv.name=name; srv.panel_url=final_url; srv.subscription_url=subscription_url or None; srv.username=username
        if password: srv.password_encrypted=encrypt_text(password)
        meta=dict(srv.meta or {}); meta['display_name']=display_name or name; meta['scope']=scope if scope in ('public','reseller','all') else 'public'; meta['panel_base_url']=base_url; meta['panel_path']=web_path; srv.meta=meta
        await s.flush()
        ok_sync, old_ids, new_ids, err = await refresh_server_inbounds(s,srv,force_plan_update=True)
        await s.commit()
        if not ok_sync:
