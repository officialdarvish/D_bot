...
def _validate_backup_payload(data: Any) -> None:
    ...
    try:
        backup_format = int(meta.get('format') or 1)
    except (TypeError, ValueError) as exc:
        raise ValueError('Backup format is invalid') from exc
    if backup_format > PORTABLE_BACKUP_FORMAT:
        raise ValueError('This backup was created by a newer D BOT version')
    if backup_format >= PORTABLE_BACKUP_FORMAT:
        verify_checksum(data)
        portable_secret_box_from_meta(meta)
        return

    # Formats 1-3 used an HMAC tied to the source VPS FERNET_KEY/BOT_TOKEN.
    # That signature cannot be verified on a different installation, so it is
    # treated as legacy metadata instead of a portability blocker. The schema is
    # still validated above, and encrypted credentials are preflight-checked
    # before any destination data is deleted.
    sig = str(meta.get('signature') or '')
    if settings.BACKUP_REQUIRE_SIGNATURE and sig and hmac.compare_digest(sig, _sign_backup(data)):
        return
    logger.warning('Accepting schema-valid legacy D BOT backup without a local signature match')
...
def _custom_panel_server_payload(origin: str, auth_username: str, password: str, scope: str, router: dict[str, Any], display_name: str = '', default_protocol: str = 'openvpn', openvpn_profile_id: int = 0, l2tp_server: str = '', l2tp_ipsec_secret: str = '', badge_color: str = '', badge_label: str = '', badge_emoji: str = '') -> dict[str, Any]:
    ...
    meta = {
        ...
        'router_online': MikroTikService().router_is_online(router, True),
...
async def server_test(request: Request, server_type:str=Form('xui'), panel_url:str=Form(...), panel_path:str=Form('/'), username:str=Form(''), password:str=Form(''), api_key:str=Form(''), router_name:str=Form(''), _: str = Depends(_auth_user)):
    if server_type == 'mikrotik':
        ...
        online = sum(1 for r in routers if MikroTikService().router_is_online(r, True))
    ...
    async with SessionLocal() as s:
        for router_row in routers:
            ...
            if existing:
                ...
                # Preserve the administrator enable/disable switch; runtime health lives in meta.router_online.
                meta = dict(existing.meta or {})
                meta.update(payload['meta'])
                meta.setdefault('admin_enabled', bool(existing.is_active))
                existing.meta = meta
            else:
                existing = Server(
                    name=payload['name'],
                    ...
                    is_active=True,
                    meta={**payload['meta'], 'admin_enabled': True},
                )
                s.add(existing)
            saved += 1
        await s.commit()
        ...
        for router_row in routers:
            ...
            if existing:
                ...
                existing = Server(
                    ...
                    password_encrypted=encrypt_text(secret_to_save),
                    is_active=True, meta={**payload['meta'], 'admin_enabled': True}
                )
                s.add(existing)
            saved += 1
        await s.commit()
...
async def server_refresh(request: Request, sid:int, _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        if srv.server_type == 'mikrotik':
            service = MikroTikService()
            try:
                routers = await service.routers(srv)
            except Exception as exc:
                meta = dict(srv.meta or {})
                meta.update({
                    'router_online': False,
                ...
                meta.setdefault('admin_enabled', bool(srv.is_active))
                srv.meta = meta
                await s.commit()
                return fail(request, 'MikroTik / Custom connection failed: ' + str(exc), 400)
            meta = dict(srv.meta or {})
            ...
            if not matched:
                ...
                srv.meta = meta
                await s.commit()
                return fail(request, 'MikroTik / Custom returned no routers.', 400)
            if matched:
                meta.update({
                    'custom_panel': True,
                    'custom_panel_name': 'MikroTik / Custom',
                ...
                # Keep the administrator enable/disable switch independent from
                # transient router reachability.
                srv.meta = meta
            await s.commit()
...
def _reseller_inbound_mode(reseller: ResellerAccount | None) -> str:
    return 'manual' if str(getattr(reseller, 'inbound_mode', '') or '').strip().lower() == 'manual' else 'automatic'


def _resolve_reseller_inbound_config(server: Server | None, inbound_mode: str, inbound_ids) -> tuple[str, list[int], str | None]:
    mode = 'manual' if str(inbound_mode or '').strip().lower() == 'manual' else 'automatic'
    if not server:
        if mode == 'manual':
            return mode, [], 'This reseller has no server assigned. Assign a reseller server before using manual inbound mode.'
        return mode, [], None
    if server.server_type != 'xui':
        return 'automatic', [], None
    available = _server_available_inbound_ids(server)
    if not available:
        return mode, [], 'The reseller server has no active inbound. Refresh the server connection first.'
    if mode == 'automatic':
        # Automatic is intentionally resolved live during create/renew. Do not
        # freeze the current list into the reseller account.
        return mode, [], None
    selected = _clean_plan_inbound_ids(inbound_ids)
    if not selected:
        return mode, [], 'Manual mode requires at least one inbound.'
    invalid = [iid for iid in selected if iid not in available]
    if invalid:
        return mode, [], 'Invalid or inactive reseller inbound IDs: ' + ', '.join(str(x) for x in invalid)
    return mode, selected, None
...
async def reseller_edit(request: Request, rid:int, total_gb:int=Form(0), expires_at:str=Form(''), days:int=Form(0), inbound_mode:str|None=Form(None), inbound_ids:str|None=Form(None), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        obj=await s.get(ResellerAccount,rid)
        if obj:
            old_total = int(obj.total_bytes or 0)
            new_total = max(0, int(total_gb or 0))*1024**3
            obj.total_bytes = new_total
            # Inbound policy belongs to the reseller account, not the reseller
            # server globally. Legacy forms omit these fields and therefore keep
            # the existing policy unchanged.
            if inbound_mode is not None:
                server = await s.get(Server, obj.server_id) if obj.server_id else None
                mode, selected_ids, inbound_error = _resolve_reseller_inbound_config(server, inbound_mode, inbound_ids or '')
                if inbound_error:
                    return fail(request, inbound_error)
                obj.inbound_mode = mode
                obj.inbound_ids = selected_ids if mode == 'manual' else []
            ...
            if new_total != old_total:
                user = await s.get(User, obj.user_id)
                ...
                s.add(ResellerServiceActivity(
                    ...
                    event_key=f'quota_adjustment:{obj.id}:{now.strftime("%Y%m%d%H%M%S%f")}',
                    username=(user.username or user.full_name or str(user.telegram_id)) if user else '-',
                    volume_bytes=abs(delta),
                    previous_volume_bytes=old_total,
                    duration_days=0,
                    expires_at=obj.expires_at,
                    meta={
                        'source': 'admin_web_edit',
                        'status': 'success',
                        'added_bytes': max(delta, 0),
                        'deducted_bytes': max(-delta, 0),
                        'quota_before_bytes': old_total,
                        'quota_after_bytes': new_total,
                        'quota_remaining_after_bytes': new_total,
                        'quota_delta_bytes': delta,
...
def _backup_models():
    return [
        ...
        ('service_deletion_tasks', ServiceDeletionTask),
        ('services', ClientService), ('reseller_service_activities', ResellerServiceActivity),
        ('orders', Order), ('wallet_transactions', WalletTransaction),
        ('tickets', Ticket), ('ticket_messages', TicketMessage), ('test_account_usages', TestAccountUsage), ('openvpn_profiles', OpenVPNProfile),
        ('test_account_counters', TestAccountCounter), ('service_username_counters', ServiceUsernameCounter), ('reseller_username_counters', ResellerUsernameCounter),
    ]
...
def _restore_delete_order():
    return [
        ...
        ResellerServiceActivity, TestAccountUsage, ClientService, ServiceDeletionTask, DiscountUsage, ResellerTopupRequest, ResellerBuildConfig,
        ResellerPackage, ResellerAccessRequest, ResellerAccount, DiscountCode, Plan, PaymentCard,
        TestAccountCounter, ServiceUsernameCounter, ResellerUsernameCounter, ServerCategory, Server, Setting, User,
    ]
...
def _backup_format(data: dict[str, Any]) -> int:
    try:
        return int((data.get('meta') or {}).get('format') or 1)
    except (TypeError, ValueError) as exc:
        raise ValueError('Backup format is invalid') from exc


def _prepare_backup_for_restore(
    data: dict[str, Any],
    *,
    legacy_secret: str = '',
    legacy_secret_kind: str = 'auto',
) -> dict[str, Any]:
    """Preflight and convert source credentials before the database is touched.

    Portable format 4 stores server credentials in a self-contained backup
    envelope. During restore they are decrypted from that envelope and encrypted
    again with the destination installation's FERNET_KEY. Formats 1-3 can also be
    restored across installations when the admin supplies the source FERNET_KEY
    or the source BOT_TOKEN used to derive it.
    """
    prepared: dict[str, Any] = {'meta': dict(data.get('meta') or {})}
    for key, _ in _backup_models():
        prepared[key] = [dict(row) for row in (data.get(key, []) or [])]

    backup_format = _backup_format(data)
    portable_box = portable_secret_box_from_meta(prepared['meta']) if backup_format >= PORTABLE_BACKUP_FORMAT else None
    source_box = legacy_source_box(legacy_secret, legacy_secret_kind) if backup_format < PORTABLE_BACKUP_FORMAT else None

    for row in prepared.get('servers', []):
        raw = str(row.get('password_encrypted') or '')
        server_name = str(row.get('name') or row.get('id') or 'unknown')
        if backup_format >= PORTABLE_BACKUP_FORMAT:
            plaintext = open_portable_secret(raw, portable_box)
        else:
            plaintext = open_legacy_secret(raw, source_box, f'server credential for "{server_name}"')
        row['password_encrypted'] = encrypt_text(plaintext)

    # Keep the recoverable website password portable as well. The password hash
    ...
    for row in prepared.get('settings', []):
        ...
        if backup_format >= PORTABLE_BACKUP_FORMAT:
            ...
            plaintext = open_legacy_secret(raw, source_box, 'the website password credential')
        row['value'] = encrypt_text(plaintext)
    return prepared
...
async def _export_backup_payload() -> dict[str, Any]:
    ...
    async with SessionLocal() as session:
        async def rows(section: str, model):
            ...
            for obj in result:
                ...
                if section == 'servers':
                    try:
                        plaintext = decrypt_text(str(item.get('password_encrypted') or ''))
                    except RuntimeError as exc:
                        raise ValueError(
                            f'Server credential for "{item.get("name") or item.get("id")}" cannot be read on this installation. '
...
def _normalize_telegram_target(destination: str, value: str) -> str:
    return normalize_telegram_target(
        destination,
        value,
        default_bot_chat_id=_default_backup_chat_id(),
    )


def _effective_backup_chat_id(
    destination: str,
    value: str,
    settings_map: dict[str, str] | None = None,
) -> str:
    """Resolve a usable Bot API target.

    Telegram invite links are not chat IDs. When the bot has recently been added
    as an administrator, the my_chat_member handler stores the real numeric chat
    ID and this helper transparently uses it.
    """
    settings_map = settings_map or {}
    return resolve_telegram_target(
        destination,
        value,
        detected_chat_id=str(settings_map.get('backup_last_detected_chat_id') or ''),
        detected_chat_type=str(settings_map.get('backup_last_detected_chat_type') or ''),
        default_bot_chat_id=_default_backup_chat_id(),
    )


def _telegram_error_message(data: dict[str, Any]) -> str:
    description = str(data.get('description') or '').strip()
    error_code = data.get('error_code')
    if description:
        return f'Telegram error{f" {error_code}" if error_code else ""}: {description}'
...
async def _telegram_send_backup(
    ...
    *,
    ...
    if not token or not chat_id:
        return {'ok': False, 'admin_ok': False, 'message': 'Backup destination is not configured'}
    if destination in {'channel', 'group'} and is_private_invite_link(chat_id):
        return {'ok': False, 'admin_ok': False, 'message': private_target_help()}
    base = f'https://api.telegram.org/bot{token}'
    try:
        ...
        return {'ok': False, 'admin_ok': False, 'message': f'Telegram backup upload failed: {exc}'}
    ok = bool(data.get('ok'))
    return {
        'ok': ok,
        'admin_ok': ok,
...
async def _restore_backup_payload(
    ...
    *,
    ...
    prepared = _prepare_backup_for_restore(
        data,
        legacy_secret=legacy_secret,
        legacy_secret_kind=legacy_secret_kind,
    )
    restored: dict[str, int] = {}
    deferred_user_referrals: list[tuple[int, int]] = []
    deferred_server_categories: list[tuple[int, int]] = []
    async with SessionLocal() as session:
        async with session.begin():
            # Break the only cyclic/self references before clearing an existing
            # destination database. This makes restore reliable even when both
            # Server.category_id and ServerCategory.server_id are populated.
                ...
                for item in prepared.get(key, []) or []:
                    ...
                    for k, v in dict(item).items():
                        if k not in colnames:
                            continue
                        if key == 'users' and k == 'referred_by_user_id' and v is not None:
                            deferred_user_referrals.append((int(item['id']), int(v)))
                            v = None
                        if key == 'servers' and k == 'category_id' and v is not None:
                            deferred_server_categories.append((int(item['id']), int(v)))
                            v = None
            ...
            for user_id, referred_by_user_id in deferred_user_referrals:
                await session.execute(
                    update(User).where(User.id == user_id).values(referred_by_user_id=referred_by_user_id)
                )
            for server_id, category_id in deferred_server_categories:
                await session.execute(
                    update(Server).where(Server.id == server_id).values(category_id=category_id)
                )
...
async def backup_run(request: Request, _: str = Depends(_auth_user)):
    ...
    chat_id = _effective_backup_chat_id(destination, (m.get('backup_chat_id') or m.get('backup_channel') or '').strip(), m)
    ...
    try:
        ...
        await _save_settings_map({
            ...
            'backup_last_backup_message': str(exc),
            'backup_last_backup_at': datetime.utcnow().isoformat(),
        })
        return fail(request, f'Backup failed: {exc}')
    try:
        result = await _telegram_send_backup(
            ...
            destination,
            caption=(
                '📦 D BOT portable backup v4\n\n'
                '✅ New backup document\n'
                '✅ Restorable from Backup & Restore'
            ),
            filename=f"dbot_portable_backup_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json",
        )
        await _save_settings_map({
...
async def backup_download(_: str = Depends(_auth_user)):
    try:
        ...
        raise HTTPException(status_code=400, detail=f'Backup failed: {exc}') from exc
    return FileResponse(
        path,
        media_type='application/json',
        filename='dbot_portable_backup_v4.json',
...
async def backup_restore(
    ...
    legacy_secret: str = Form(''),
    legacy_secret_kind: str = Form('auto'),
    _: str = Depends(_auth_user),
):
    if not (file.filename or '').lower().endswith('.json'):
        return fail(request, 'Only JSON backup files are supported')
    raw = await file.read(max(1024, int(settings.BACKUP_MAX_UPLOAD_BYTES or 104_857_600)) + 1)
    if len(raw) > int(settings.BACKUP_MAX_UPLOAD_BYTES or 104_857_600):
    ...
    try:
        ...
        logger.warning('Legacy backup restore needs source secret: %s', exc)
        return JSONResponse(
            {
                'ok': False,
                'code': 'legacy_secret_required',
                'message': f'Restore needs the old source secret: {exc}',
            },
            status_code=409,
        )
    except LegacyBackupSecretInvalid as exc:
        logger.warning('Legacy backup source secret was rejected: %s', exc)
        return JSONResponse(
            {
                'ok': False,
                'code': 'legacy_secret_invalid',
        ...
        return fail(request, f'Restore failed: {exc}', 400)
    except Exception:
        logger.exception('Backup restore failed unexpectedly')
        return fail(request, 'Restore failed because of an unexpected database or server error. Check API logs.', 500)
...
def srv_json(sv):
    ...
    return {
        ...
        'router_online': MikroTikService().router_is_online({'online': m.get('router_online')}, bool(sv.is_active)),
...
async def toggle_server(request: Request, sid: int, _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        if server.server_type == 'mikrotik':
            ...
            meta['admin_enabled'] = bool(server.is_active)
            meta['admin_status_changed_at'] = datetime.utcnow().isoformat(timespec='seconds')
            server.meta = meta
        await s.commit()
...
async def api_v2_reseller_services(rid: int, _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        # Backfill previously approved reseller quota recharges so older package
        # purchases are also visible in the activity list. Exact before/after
        # quota cannot be reconstructed for legacy events, so only known values
        # are populated.
        existing_topup_keys = set((await s.execute(
            select(ResellerServiceActivity.event_key).where(
                ResellerServiceActivity.reseller_id == rid,
                ResellerServiceActivity.action == 'topup',
            )
        )).scalars().all())
        approved_topups = (await s.execute(
            select(ResellerTopupRequest).where(
                ResellerTopupRequest.reseller_id == rid,
                ResellerTopupRequest.status == 'approved',
            )
        )).scalars().all()
        for req in approved_topups:
            key = f'topup:{req.id}'
            if key in existing_topup_keys:
                continue
            s.add(ResellerServiceActivity(
                reseller_id=rid,
                service_id=None,
                ...
                action='topup',
                event_key=key,
                username=reseller_user.username or reseller_user.full_name or str(reseller_user.telegram_id),
                volume_bytes=int(req.volume_bytes or 0),
                previous_volume_bytes=0,
                duration_days=0,
                expires_at=None,
                meta={
                    'source': 'backfill',
                    'request_id': req.id,
                    'added_bytes': int(req.volume_bytes or 0),
                    'legacy_quota_snapshot_unavailable': True,
                },
    ...
    activity_labels = {
        'create': 'Created / Purchase',
        'renew': 'Renewed',
        'topup': 'Quota Recharged',
        'quota_adjustment': 'Quota Adjusted',
        'add_volume': 'Service Volume Added',
        'add_days': 'Service Days Added',
        'create_failed': 'Create Failed',
        'renew_failed': 'Renew Failed',
        'add_volume_failed': 'Add Volume Failed',
        'add_days_failed': 'Add Days Failed',
    }
    activity_items = []
    for activity in activities:
        ...
        activity_items.append({
            ...
            'action_label': activity_labels.get(activity.action, activity.action.replace('_', ' ').title()),
            ...
            'old_unused_bytes': int(details.get('old_unused_bytes') or 0),
            'added_bytes': int(details.get('added_bytes') or 0),
            'deducted_bytes': int(details.get('deducted_bytes') or 0),
            'requested_volume_bytes': int(details.get('requested_volume_bytes') or 0),
            'quota_before_bytes': int(details.get('quota_before_bytes') or 0),
            'quota_after_bytes': int(details.get('quota_after_bytes') or 0),
            'quota_remaining_after_bytes': int(details.get('quota_remaining_after_bytes') if details.get('quota_remaining_after_bytes') is not None else (details.get('quota_after_bytes') or 0)),
            'quota_delta_bytes': int(details.get('quota_delta_bytes') or 0),
            'quota_snapshot_version': int(details.get('quota_snapshot_version') or 0),
            'service_total_before_bytes': int(details.get('service_total_before_bytes') or 0),
            'service_total_after_bytes': int(details.get('service_total_after_bytes') or 0),
            'status': str(details.get('status') or ('failed' if activity.action.endswith('_failed') else 'success')),
            ...
            'package_title': str(details.get('package_title') or ''),
            'legacy_quota_snapshot_unavailable': bool(details.get('legacy_quota_snapshot_unavailable')),
