...
def _custom_panel_server_payload(origin: str, auth_username: str, password: str, scope: str, router: dict[str, Any], display_name: str = '', default_protocol: str = 'openvpn', openvpn_profile_id: int = 0, l2tp_server: str = '', l2tp_ipsec_secret: str = '', badge_color: str = '', badge_label: str = '', badge_emoji: str = '') -> dict[str, Any]:
    ...
    meta = {
        ...
        'l2tp_server': l2tp_server or 'de.rgiftshop.ir',
        'l2tp_ipsec_secret': l2tp_ipsec_secret or '123456789',
        'inbound_ids': [],
        'inbounds': [],
...
async def server_edit(request: Request, sid:int, name:str=Form(...), scope:str=Form('public'), display_name:str=Form(''), panel_url:str=Form(...), panel_path:str=Form('/'), subscription_url:str=Form(''), username:str=Form(''), password:str=Form(''), api_key:str=Form(''), router_name:str=Form(''), default_protocol:str=Form('openvpn'), openvpn_profile_id:int=Form(0), l2tp_server:str=Form(''), l2tp_ipsec_secret:str=Form(''), badge_color:str=Form(''), badge_label:str=Form(''), badge_emoji:str=Form(''), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        if server_type == 'mikrotik':
            ...
            meta.update({'display_name':display_name or name or srv.name,'scope':scope if scope in ('public','reseller','all') else 'public','panel_base_url':srv.panel_url,'router_name':router,'auth_username':auth_username,'default_protocol':default_protocol or meta.get('default_protocol') or 'openvpn','openvpn_profile_id':int(openvpn_profile_id or 0),'l2tp_server':l2tp_server or meta.get('l2tp_server') or 'de.rgiftshop.ir','l2tp_ipsec_secret':l2tp_ipsec_secret or meta.get('l2tp_ipsec_secret') or '123456789'}); meta.update(_server_badge_meta('mikrotik', badge_color, badge_label, badge_emoji, default_protocol)); srv.meta=meta
            await s.commit()
...
async def plan_add(request: Request, plan_kind:str=Form('public'), title:str=Form(...), volume_gb:int=Form(0), duration_days:int=Form(0), reseller_validity_days:int=Form(365), price_irt:int=Form(0), category_id:int=Form(0), server_id:int=Form(0), _: str = Depends(_auth_user)):
    ...
    async with SessionLocal() as s:
        ...
        inbs=(srv.meta or {}).get('inbound_ids') if srv and srv.server_type == 'xui' else []
        s.add(Plan(title=title,volume_gb=volume_gb,duration_days=duration_days,price_irt=price_irt,category_id=category_id,server_id=server_id,inbound_ids=inbs or [],is_active=True, meta={})); await s.commit()
...
async def plan_edit(request: Request, pid:int, title:str=Form(...), volume_gb:int=Form(0), duration_days:int=Form(0), price_irt:int=Form(0), category_id:int=Form(0), server_id:int=Form(0), _: str = Depends(_auth_user)):
    ...
    async with SessionLocal() as s:
        ...
        if not srv:
            return fail(request, 'Selected server was not found.', 404)
        inbs=(srv.meta or {}).get('inbound_ids') if srv and srv.server_type == 'xui' else []
        obj.title=title; obj.volume_gb=volume_gb; obj.duration_days=duration_days; obj.price_irt=price_irt; obj.category_id=category_id; obj.server_id=server_id; obj.inbound_ids=inbs or []; obj.meta={}
        await s.commit()
...
@router.get('/admin/api/v2/service-types')
async def api_v2_service_types(_: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        items = (await s.execute(select(Setting).where(Setting.key.like('service_type:custom:%')))).scalars().all()
        order_row = await s.get(Setting, 'service_type_order')
    settings_map = {x.key: x.value for x in items}
    rows = _ordered_service_types([x for x in items if _is_service_type_row(x)], order_row.value if order_row else '')
    return {'ok': True, 'items': [
        {'key': x.key, 'value': x.value, 'is_active': _service_type_is_active(settings_map, x.key)}
        for x in rows
    ]}
