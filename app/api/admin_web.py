...
</style><script>function togglePassword(){{const i=document.getElementById('loginPassword'),b=document.getElementById('togglePass');if(i){{i.type=i.type==='password'?'text':'password';b.textContent=i.type==='password'?'Show':'Hide';}}}}</script></head><body><main class="login-wrap"><form class="login-card" method="post" action="/login"><input type="hidden" name="next_url" value="{e(next)}"><div class="login-logo">D</div><h1>D BOT Owner Login</h1><p>Secure access to Cyber Admin</p>{note}<label>Username<input name="username" autocomplete="username" required placeholder="Enter owner username"></label><label>Password<div class="password-row"><input id="loginPassword" name="password" type="password" autocomplete="current-password" required placeholder="Enter password"><button id="togglePass" type="button" onclick="togglePassword()">Show</button></div></label><button class="btn" type="submit">Login</button><p class="copyright">All design and development belongs to <b>D Bot</b>.</p></form></main></body></html>"""
