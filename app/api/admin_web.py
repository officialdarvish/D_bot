...
import html, secrets, subprocess, re, json, os, shutil, tempfile, asyncio
...
from app.database.models import (
    ...
    ResellerBuildConfig, AntiSharingViolation, ResellerTopupRequest, WalletTransaction, PaygUsageLog, TestAccountUsage
)
...
NAV=[('/admin','🏠','Dashboard','Dashboard'),('/admin/service-types','🧬','Service Types','Service Types'),('/admin/servers','🖥','Servers','Servers'),('/admin/categories','🗂','Categories','Categories'),('/admin/plans','📦','Plans','Plans'),('/admin/payments','💳','Payments','Payments'),('/admin/orders-report','📄','Orders Report','Orders Report'),('/admin/discounts','🎟','Discount Codes','Discount Codes'),('/admin/resellers','🤝','Resellers','Resellers'),('/admin/backup','🧰','Backup','Backup'),('/admin/settings','⚙️','Settings','Settings')]
...
async function postForm(form){const btn=form.querySelector('button[type=submit],button:not([type])'); if(btn)btn.classList.add('loading'); try{let r=await fetch(form.action,{method:(form.method||'POST').toUpperCase(),body:new FormData(form),headers:{'X-Requested-With':'fetch','Accept':'application/json'}});let ct=(r.headers.get('content-type')||'').toLowerCase();if(r.redirected||ct.includes('text/html')){location.href=r.url||location.href;return false;}let j=await readJsonOrError(r);if(!j.ok){notify(j.message||'Error',false);return false;}notify(j.message||'Saved',true);setTimeout(()=>{location.href=j.redirect||location.href},700);return false}catch(e){notify(String(e),false);return false}}
async function callAction(url){try{let join=url.includes('?')?'&':'?';let r=await fetch(url+join+'ajax=1',{headers:{'X-Requested-With':'fetch','Accept':'application/json'}});let j=await readJsonOrError(r);if(!j.ok){notify(j.message||'Error',false);return}notify(j.message||'Done',true);setTimeout(()=>{location.href=j.redirect||location.href},700)}catch(e){notify(String(e),false)}}
...
(function(){
  ...
  function ajaxHeaders(){return {'X-Requested-With':'fetch','Accept':'application/json'}}
  ...
  window.dbotCallAction = async function(url){
    try{
      ...
      const res=await fetch(url + join + 'ajax=1', {headers:ajaxHeaders()});
      const ct=(res.headers.get('content-type')||'').toLowerCase();
      if(res.redirected || ct.includes('text/html')){
        location.href=res.url || location.href;
        return;
...
@router.post('/login')
async def do_login(response: Response, username: str = Form(...), password: str = Form(...), next_url: str = Form('/admin')):
    ...
    # Existing installs can login with .env credentials until they set website credentials from the bot.
    if not user or not pwd:
        user = settings.WEB_ADMIN_USERNAME or 'admin'
        pwd = settings.WEB_ADMIN_PASSWORD or (settings.FERNET_KEY or '')[:24]
    if not pwd or not (secrets.compare_digest(username, user) and secrets.compare_digest(password, pwd)):
        return RedirectResponse('/login?error=1', status_code=303)
    token = secrets.token_urlsafe(32)
    _sessions[token] = {'username': username, 'created_at': _now(), 'expires_at': _now() + timedelta(minutes=timeout_min)}
    res = RedirectResponse(next_url if next_url.startswith('/') else '/admin', status_code=303)
    res.set_cookie('dbot_admin_token', token, max_age=timeout_min*60, httponly=True, secure=True, samesite='lax')
    return res
...
async def server_test(request: Request, server_type:str=Form('xui'), panel_url:str=Form(...), panel_path:str=Form('/'), username:str=Form(...), password:str=Form(''), _: str = Depends(_auth_user)):
    if server_type != 'xui':
        return JSONResponse({'ok': True, 'message': 'This server type does not require X-UI inbound sync.', 'inbound_ids': []})
    if not password:
        return fail(request, 'Password is required to test a new server connection.')
    ...
    try:
        ...
        return fail(request, 'Connection test failed: ' + str(ex), 400)
    if not ok_conn:
...
async def server_add(request: Request, server_type:str=Form('xui'), scope:str=Form('public'), name:str=Form(...), display_name:str=Form(''), panel_url:str=Form(...), panel_path:str=Form('/'), subscription_url:str=Form(''), username:str=Form(...), password:str=Form(...), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        base_url, web_path, final_url = _normalize_panel_parts(panel_url, panel_path)
        ...
        await s.commit()
        if not ok_sync:
            return fail(request, 'Server saved, but panel test/inbound sync failed: ' + (err or 'Unknown error'))
    return ok(request, '/admin/servers', f'Server added and {len(new_ids)} inbound(s) synchronized')
@router.post('/admin/servers/{sid}/edit')
async def server_edit(request: Request, sid:int, name:str=Form(...), scope:str=Form('public'), display_name:str=Form(''), panel_url:str=Form(...), panel_path:str=Form('/'), subscription_url:str=Form(''), username:str=Form(...), password:str=Form(''), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        await s.commit()
        if not ok_sync:
            return fail(request, 'Server updated, but panel test/inbound sync failed: ' + (err or 'Unknown error'))
...
async def plan_add(request: Request, plan_kind:str=Form('public'), title:str=Form(...), volume_gb:int=Form(0), duration_days:int=Form(0), reseller_validity_days:int=Form(365), price_irt:int=Form(0), category_id:int=Form(0), server_id:int=Form(0), _: str = Depends(_auth_user)):
    if not server_id:
    ...
    async with SessionLocal() as s:
        ...
        if plan_kind == 'reseller':
            s.add(ResellerPackage(title=title, volume_gb=volume_gb, reseller_validity_days=reseller_validity_days or 365, price_irt=price_irt, server_id=server_id, is_active=True))
            await s.commit()
        ...
        s.add(Plan(title=title,volume_gb=volume_gb,duration_days=duration_days,price_irt=price_irt,category_id=category_id,server_id=server_id,inbound_ids=inbs or [],is_active=True)); await s.commit()
    return ok(request, '/admin/plans', 'Public plan added')
@router.post('/admin/plans/{pid}/edit')
async def plan_edit(request: Request, pid:int, title:str=Form(...), volume_gb:int=Form(0), duration_days:int=Form(0), price_irt:int=Form(0), category_id:int=Form(0), server_id:int=Form(0), _: str = Depends(_auth_user)):
    if not category_id:
    ...
    async with SessionLocal() as s:
        ...
        if not srv:
            return fail(request, 'Selected server was not found.', 404)
        inbs=(srv.meta or {}).get('inbound_ids') if srv else []
        obj.title=title; obj.volume_gb=volume_gb; obj.duration_days=duration_days; obj.price_irt=price_irt; obj.category_id=category_id; obj.server_id=server_id; obj.inbound_ids=inbs or []
        await s.commit()
...
async def reseller_plan_add(request: Request, title:str=Form(...), volume_gb:int=Form(0), reseller_validity_days:int=Form(365), price_irt:int=Form(0), server_id:int=Form(0), _: str = Depends(_auth_user)):
    ...
    async with SessionLocal() as s:
        ...
        if not srv:
            return fail(request, 'Selected server was not found.', 404)
        s.add(ResellerPackage(title=title, volume_gb=volume_gb, reseller_validity_days=reseller_validity_days, price_irt=price_irt, server_id=server_id, is_active=True))
        await s.commit()
    return ok(request, '/admin/plans', 'Reseller plan added')
...
async def reseller_plan_edit(request: Request, pid:int, title:str=Form(...), volume_gb:int=Form(0), reseller_validity_days:int=Form(365), price_irt:int=Form(0), server_id:int=Form(0), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        try:
            ...
            await s.rollback()
            return fail(request, 'Reseller plan update failed: ' + str(exc), 500)
...
async def reseller_plan_del(request: Request, pid:int, _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        try:
            ...
            await s.rollback()
            return fail(request, 'Reseller plan delete failed: ' + str(exc), 500)
...
def _backup_models():
    return [
        ...
        ('tickets', Ticket), ('ticket_messages', TicketMessage), ('test_account_usages', TestAccountUsage),
    ]
    ...
    async with SessionLocal() as session:
        async def rows(model):
            ...
            for obj in result:
                ...
                out.append(item)
            return out
        data = {'meta': {'app': 'D BOT', 'created_at': datetime.utcnow().isoformat(), 'format': 2}}
...
async def _apply_ssl_for_domain(domain: str) -> tuple[bool, str]:
    ...
    try:
        ...
        msg = ((proc.stdout or '') + '\n' + (proc.stderr or '')).strip()[-2500:] or ('SSL applied' if proc.returncode == 0 else 'SSL command failed')
...
async def backup_restore(request: Request, file: UploadFile = File(...), _: str = Depends(_auth_user)):
    ...
    try:
        ...
        return fail(request, 'Invalid backup JSON: ' + str(ex))
    try:
        restored = await _restore_backup_payload(data)
    except Exception as ex:
        return fail(request, 'Restore failed: ' + str(ex), 500)
...
async def website_settings_save(request: Request, domain:str=Form(''), username:str=Form(''), password:str=Form(''), token_timeout:int=Form(30), _: str = Depends(_auth_user)):
    ...
    async with SessionLocal() as s:
        ...
        if password: await s.merge(Setting(key='web_admin_password', value=password))
