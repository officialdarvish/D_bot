...
from app.core.security import encrypt_text, hash_password, verify_password, is_password_hash
...
from app.database.models import (
    ...
    ResellerBuildConfig, AntiSharingViolation, ResellerTopupRequest, WalletTransaction, PaygUsageLog, TestAccountUsage, OpenVPNProfile
)
...
def user_wallet_total(u: Any) -> int:
    return int(getattr(u, 'wallet_balance', 0) or 0) + int(getattr(u, 'wallet_v2ray_balance', 0) or 0) + int(getattr(u, 'wallet_openvpn_balance', 0) or 0)
...
</style><script>function togglePassword(){{const i=document.getElementById('loginPassword'),b=document.getElementById('togglePass');if(i){{i.type=i.type==='password'?'text':'password';b.textContent=i.type==='password'?'Show':'Hide';}}}}</script></head><body><main class="login-wrap"><form class="login-card" method="post" action="/login"><input type="hidden" name="next_url" value="{e(next)}"><div class="login-logo">D</div><h1>D BOT Owner Login</h1><p>Secure access to Cyber Admin</p>{note}<label>Username<input name="username" autocomplete="username" required placeholder="Enter owner username"></label><label>Password<div class="password-row"><input id="loginPassword" name="password" type="password" autocomplete="current-password" required placeholder="Enter password"><button id="togglePass" type="button" onclick="togglePassword()">Show</button></div></label><button class="btn" type="submit">Login</button><p class="copyright">All design and development belongs to <b>officialDarvish</b>.</p></form></main></body></html>"""
...
async def _dashboard_payload(days: int = 30) -> dict[str, Any]:
    ...
    async with SessionLocal() as s:
        ...
        wallet_expr = func.coalesce(User.wallet_balance,0) + func.coalesce(User.wallet_v2ray_balance,0) + func.coalesce(User.wallet_openvpn_balance,0)
...
async def dashboard(request: Request, page: int = 1, page_size: int = 50, _: str = Depends(_auth_user)):
    ...
    for i,u in enumerate(users,1):
        ...
        v2ray_wallet = getattr(u, 'wallet_v2ray_balance', 0)
        openvpn_wallet = getattr(u, 'wallet_openvpn_balance', 0)
        user_details_html = (
            f'<div class="kvs">'
            ...
            f'<div class="kv"><span>V2Ray wallet</span><b>{money(v2ray_wallet)} Toman</b></div>'
            f'<div class="kv"><span>OpenVPN wallet</span><b>{money(openvpn_wallet)} Toman</b></div>'
            f'</div>'
        )
...
async def service_types(request: Request, _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        items=(await s.execute(select(Setting).where(Setting.key.like('service_type:custom:%')).order_by(Setting.key))).scalars().all()
...
def _server_host_label(url: str) -> str:
    """Return a safe, human-readable server name from a URL."""
    from urllib.parse import urlsplit
    raw = (url or '').strip()
    try:
        ...
        host = raw
    host = (host or 'server').strip().strip('/')
    return host or 'server'


def _safe_server_slug(value: str, fallback: str = 'server') -> str:
    text_value = (value or '').strip() or fallback
    text_value = re.sub(r'\s+', '-', text_value)
    text_value = re.sub(r'[^0-9A-Za-z_.\-]+', '-', text_value).strip('-_.')
    return (text_value[:72] or fallback).strip()


def _suggest_xui_server_fields(panel_base_url: str, panel_path: str, final_url: str, rows: list[dict] | None = None, *, name: str = '', display_name: str = '', subscription_url: str = '') -> dict[str, str]:
    """Build autofill values for the 3x-ui Add Server form.

    The admin should only need panel URL + login credentials. 3x-ui does not
    reliably expose a separate "server display name" field through the public
    API, so we derive a safe name from the first inbound remark and then fall
    back to the panel host/IP.
    """
    rows = rows or []
    first_remark = ''
    for row in rows:
        if not isinstance(row, dict):
            continue
        first_remark = str(row.get('remark') or row.get('tag') or row.get('name') or '').strip()
        if first_remark:
            break
    host_label = _server_host_label(panel_base_url or final_url)
    display = (display_name or '').strip() or first_remark or host_label
    slug_source = (name or '').strip() or display or host_label
    sub_base = (subscription_url or '').strip() or (final_url.rstrip('/') + '/sub/')
    if not sub_base.endswith('/') and '{sub_id}' not in sub_base and '{token}' not in sub_base:
        sub_base += '/'
    return {
        'name': _safe_server_slug(slug_source, 'xui-server'),
        'display_name': display,
        'panel_url': panel_base_url.rstrip('/'),
        'panel_path': panel_path or '/',
        'subscription_url': sub_base,
    }
...
async def servers(request: Request, _: str = Depends(_auth_user)):
    ...
    add_form='<form method="post" action="/admin/servers/add" class="formgrid"><div><label>Profile</label><select name="server_type"><option value="xui">3x-ui Sanaei</option></select></div>'+scope_select+field('name','Server name')+field('display_name','Display name')+field('panel_url','Panel URL / Origin')+field('panel_path','Panel Web Path','text','/')+field('subscription_url','Subscription URL')+field('username','Username')+field('password','Password','password')+'<div class="full"><button class="btn primary">Save</button></div></form>'
    cards=[]
    for sv in items:
        m=sv.meta or {}; inbs=m.get('inbound_ids') or []
        edit='<form method="post" action="/admin/servers/'+str(sv.id)+'/edit" class="formgrid">'+field('name','Server name','text',sv.name)+select_field('scope','Show for',[('public','Public sales'),('reseller','Reseller'),('all','Public + Reseller')],server_scope(sv))+field('display_name','Display name','text',m.get('display_name') or sv.name)+field('panel_url','Panel URL / Origin','text',m.get('panel_base_url') or sv.panel_url)+field('panel_path','Panel Web Path','text',m.get('panel_path') or '/')+field('subscription_url','Subscription URL','text',sv.subscription_url or '')+field('username','Username','text',sv.username)+field('password','New password','password')+'<div class="full"><button class="btn primary">Save</button></div></form>'
        cards.append(f'''<div class="card server-card"><div class="server-glow"></div><div class="headrow"><h3><span class="server-flag">🇺🇸</span> {e(m.get('display_name') or sv.name)}</h3><span class="badge status" data-fa="Online" data-en="Online">Online</span></div><div class="server-meta"><div><span>Panel</span><b>{e(sv.server_type)}</b></div><div><span>Users</span><b>{counts.get(sv.id,0)}</b></div><div><span>Inbound</span><b>{len(inbs)}</b></div><div><span>Status</span><b>ON</b></div></div><div class="kvs"><div class="kv"><span>Server name</span><b>{e(sv.name)}</b></div><div class="kv"><span>Panel type</span><b>{e(sv.server_type)}</b></div><div class="kv"><span>Usage</span><b>Public sales / Resellers</b></div><div class="kv"><span>Users</span><b>{counts.get(sv.id,0)}</b></div><div class="kv"><span>Inbound</span><b>{len(inbs)}</b></div></div><div class="rowactions"><a data-action="1" class="btn success" href="/admin/servers/{sv.id}/refresh">Refresh</a><button class="btn" onclick="openModal('srvEdit{sv.id}')">Edit</button><a data-action="1" class="btn" href="/admin/servers/{sv.id}/duplicate">Duplicate</a><button class="btn danger" onclick="askDelete('/admin/servers/{sv.id}/delete')">Delete</button></div></div>{modal('srvEdit'+str(sv.id),'Edit Server','Edit Server',edit)}''')
...
async def server_test(request: Request, server_type:str=Form('xui'), panel_url:str=Form(...), panel_path:str=Form('/'), username:str=Form(''), password:str=Form(''), router_name:str=Form(''), _: str = Depends(_auth_user)):
    if not password:
        return fail(request, 'Password/API key is required to test a new server connection.')
    if server_type == 'mikrotik':
        api_url = (panel_url or '').strip().rstrip('/')
        fake = Server(name='test', server_type='mikrotik', panel_url=api_url, subscription_url=None, username=(router_name or username or '').strip(), password_encrypted=encrypt_text(password), is_active=True, meta={'router_name': (router_name or username or '').strip(), 'panel_base_url': api_url})
        try:
            ok_conn, rows = await MikroTikService().test_server(fake)
        except Exception:
            logger.exception('MikroTik connection test failed')
            return fail(request, 'MikroTik connection test failed. Check server logs.', 400)
        if not ok_conn:
            return fail(request, 'MikroTik API/Router connection failed. Check Base URL, API key and router name.', 400)
        return JSONResponse({'ok': True, 'message': f'MikroTik connection OK. {len(rows or [])} router(s) returned.', 'inbound_ids': [], 'inbounds': rows or []})
    if server_type != 'xui':
    ...
    fake = Server(name='test', server_type=server_type, panel_url=final_url, subscription_url=None, username=username, password_encrypted=encrypt_text(password), is_active=True, meta={'panel_base_url': base_url, 'panel_path': web_path})
    try:
    ...
    if not ok_conn:
        return fail(request, 'Panel login failed. Check panel URL, username, password, and panel path.', 400)
    inbounds = []
    for row in rows or []:
        if not isinstance(row, dict):
            continue
    ...
    suggested = _suggest_xui_server_fields(base_url, web_path, final_url, inbounds, name='', display_name='', subscription_url='')
    return JSONResponse({'ok': True, 'message': f'Connection OK. {len(inbounds)} inbound(s) found. Server fields were auto-filled.', 'inbound_ids': [x['id'] for x in inbounds], 'inbounds': inbounds, 'suggested': suggested})
...
async def server_add(request: Request, server_type:str=Form('xui'), scope:str=Form('public'), name:str=Form(''), display_name:str=Form(''), panel_url:str=Form(...), panel_path:str=Form('/'), subscription_url:str=Form(''), username:str=Form(''), password:str=Form(...), router_name:str=Form(''), default_protocol:str=Form('openvpn'), openvpn_profile_id:int=Form(0), l2tp_server:str=Form(''), l2tp_ipsec_secret:str=Form(''), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        if server_type == 'mikrotik':
            final_url=(panel_url or '').strip().rstrip('/')
            router=(router_name or username or '').strip()
            auto_name = (name or router or _server_host_label(final_url) or 'mikrotik-server').strip()
            auto_display = (display_name or auto_name).strip()
            meta={'display_name':auto_display,'inbound_ids':[],'inbounds':[],'scope':scope if scope in ('public','reseller','all') else 'public','panel_base_url':final_url,'router_name':router,'default_protocol':default_protocol or 'openvpn','openvpn_profile_id': int(openvpn_profile_id or 0), 'l2tp_server': l2tp_server, 'l2tp_ipsec_secret': l2tp_ipsec_secret}
            srv=Server(name=_safe_server_slug(auto_name, 'mikrotik-server'),server_type='mikrotik',panel_url=final_url,subscription_url=None,username=router,password_encrypted=encrypt_text(password),is_active=True,meta=meta)
            s.add(srv); await s.commit(); await s.refresh(srv)
            ok_sync, rows = await MikroTikService().test_server(srv)
            if not ok_sync:
                return fail(request, 'Server saved, but MikroTik test failed. Check server logs.')
            return ok(request, '/admin/servers', f'MikroTik server added. {len(rows or [])} router(s) returned')
        base_url, web_path, final_url = _normalize_panel_parts(panel_url, panel_path)
        initial = _suggest_xui_server_fields(base_url, web_path, final_url, [], name=name, display_name=display_name, subscription_url=subscription_url)
        srv=Server(name=initial['name'],server_type=server_type,panel_url=final_url,subscription_url=initial['subscription_url'] or None,username=username,password_encrypted=encrypt_text(password),is_active=True,meta={'display_name':initial['display_name'],'inbound_ids':[],'inbounds':[],'scope':scope if scope in ('public','reseller','all') else 'public','panel_base_url':base_url,'panel_path':web_path})
        s.add(srv); await s.commit(); await s.refresh(srv)
        ok_sync, old_ids, new_ids, err = await refresh_server_inbounds(s,srv,force_plan_update=True)
        if ok_sync:
            meta = dict(srv.meta or {})
            suggested = _suggest_xui_server_fields(base_url, web_path, final_url, meta.get('inbounds') or [], name=name, display_name=display_name, subscription_url=subscription_url)
            if not (name or '').strip():
                srv.name = suggested['name']
            if not (display_name or '').strip():
                meta['display_name'] = suggested['display_name']
            if not (subscription_url or '').strip():
                srv.subscription_url = suggested['subscription_url']
            srv.meta = meta
        await s.commit()
        if not ok_sync:
...
async def server_edit(request: Request, sid:int, name:str=Form(...), scope:str=Form('public'), display_name:str=Form(''), panel_url:str=Form(...), panel_path:str=Form('/'), subscription_url:str=Form(''), username:str=Form(''), password:str=Form(''), router_name:str=Form(''), default_protocol:str=Form('openvpn'), openvpn_profile_id:int=Form(0), l2tp_server:str=Form(''), l2tp_ipsec_secret:str=Form(''), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        if server_type == 'mikrotik':
            router=(router_name or username or srv.username or '').strip()
            srv.name=name; srv.panel_url=(panel_url or '').strip().rstrip('/'); srv.subscription_url=None; srv.username=router
            if password: srv.password_encrypted=encrypt_text(password)
            meta=dict(srv.meta or {}); meta.update({'display_name':display_name or name,'scope':scope if scope in ('public','reseller','all') else 'public','panel_base_url':srv.panel_url,'router_name':router,'default_protocol':default_protocol or meta.get('default_protocol') or 'openvpn','openvpn_profile_id':int(openvpn_profile_id or 0),'l2tp_server':l2tp_server,'l2tp_ipsec_secret':l2tp_ipsec_secret}); srv.meta=meta
            await s.commit()
            return ok(request, '/admin/servers', 'MikroTik server updated')
        base_url, web_path, final_url = _normalize_panel_parts(panel_url, panel_path)
        srv.name=(name or srv.name or _server_host_label(final_url)); srv.panel_url=final_url; srv.subscription_url=subscription_url or srv.subscription_url or (final_url.rstrip('/') + '/sub/'); srv.username=username
        if password: srv.password_encrypted=encrypt_text(password)
        meta=dict(srv.meta or {}); meta['display_name']=display_name or name or meta.get('display_name') or srv.name; meta['scope']=scope if scope in ('public','reseller','all') else 'public'; meta['panel_base_url']=base_url; meta['panel_path']=web_path; srv.meta=meta
        await s.flush()
        ok_sync, old_ids, new_ids, err = await refresh_server_inbounds(s,srv,force_plan_update=True)
        await s.commit()
        if not ok_sync:
...
async def server_delete(request: Request, sid:int, _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        await s.execute(update(AntiSharingViolation).where(AntiSharingViolation.server_id==sid).values(server_id=None))
...
async def categories(request: Request, _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        items=(await s.execute(select(ServerCategory).order_by(ServerCategory.id.desc()))).scalars().all(); srvs=(await s.execute(select(Server))).scalars().all()
    opts=''.join(f'<option value="{x.id}">{e(x.name)}</option>' for x in srvs)
    form='<form method="post" action="/admin/categories/add" class="formgrid">'+field('name','Category name')+f'<div><label>Server</label><select name="server_id"><option value="0">-</option>{opts}</select></div><div class="full"><button class="btn primary">Save</button></div></form>'
    cards=''
    for c in items:
        edit='<form method="post" action="/admin/categories/'+str(c.id)+'/edit" class="formgrid">'+field('name','Category name','text',c.name)+f'<div><label>Server</label><select name="server_id"><option value="0">-</option>{opts}</select></div><div class="full"><button class="btn primary">Save</button></div></form>'
        cards+=f'<div class="card"><h3>🗂 {e(c.name)}</h3><p class="muted">Server ID: {e(c.server_id)}</p><div class="rowactions"><button class="btn" onclick="openModal(\'catEdit{c.id}\')">Edit</button><button class="btn danger" onclick="askDelete(\'/admin/categories/{c.id}/delete\')">Delete</button><button class="btn success">Save</button></div></div>'+modal('catEdit'+str(c.id),'Edit Category','Edit Category',edit)
...
async def cat_add(request: Request, name:str=Form(...), server_id:int=Form(0), _: str = Depends(_auth_user)):
    async with SessionLocal() as s: s.add(ServerCategory(name=name,server_id=server_id or None)); await s.commit()
    return ok(request, '/admin/categories', 'Done')
@router.post('/admin/categories/{cid}/edit')
async def cat_edit(request: Request, cid:int, name:str=Form(...), server_id:int=Form(0), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        obj=await s.get(ServerCategory,cid)
        if obj: obj.name=name; obj.server_id=server_id or None; await s.commit()
    return ok(request, '/admin/categories', 'Done')
...
async def cat_del(request: Request, cid:int, _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        # Keep plans and services, only detach them from this category.
...
async def plans(request: Request, _: str = Depends(_auth_user)):
    ...
    for x in rp:
        ...
        reseller+=f'<div class="card"><h3>🤝 {e(x.title)}</h3><div class="value">{money(x.price_irt)}</div><p>{x.volume_gb}GB / {x.reseller_validity_days} days</p><p class="muted">Reseller Plan | Server: {e(x.server_id or "-")}</p><div class="rowactions"><button class="btn" onclick="openModal(\'resellerPlanEdit{x.id}\')">Edit</button><button class="btn danger" onclick="askDelete(\'/admin/plans/reseller/{x.id}/delete\')">Delete</button></div></div>'+modal('resellerPlanEdit'+str(x.id),'Edit Reseller Plan','Edit Reseller Plan',rp_edit)
    notice = ''
...
async def plan_add(request: Request, plan_kind:str=Form('public'), title:str=Form(...), volume_gb:int=Form(0), duration_days:int=Form(0), reseller_validity_days:int=Form(365), price_irt:int=Form(0), category_id:int=Form(0), server_id:int=Form(0), mikrotik_protocol:str=Form('openvpn'), openvpn_profile_id:int=Form(0), l2tp_server:str=Form(''), l2tp_ipsec_secret:str=Form(''), _: str = Depends(_auth_user)):
    ...
    async with SessionLocal() as s:
        ...
        if plan_kind == 'reseller':
            s.add(ResellerPackage(title=title, volume_gb=volume_gb, reseller_validity_days=reseller_validity_days or 365, price_irt=price_irt, server_id=server_id, is_active=True, meta={'mikrotik_protocol': mikrotik_protocol, 'openvpn_profile_id': int(openvpn_profile_id or 0), 'l2tp_server': l2tp_server, 'l2tp_ipsec_secret': l2tp_ipsec_secret}))
            await s.commit()
        ...
        inbs=(srv.meta or {}).get('inbound_ids') if srv else []
        s.add(Plan(title=title,volume_gb=volume_gb,duration_days=duration_days,price_irt=price_irt,category_id=category_id,server_id=server_id,inbound_ids=inbs or [],is_active=True, meta={'mikrotik_protocol': mikrotik_protocol, 'openvpn_profile_id': int(openvpn_profile_id or 0), 'l2tp_server': l2tp_server, 'l2tp_ipsec_secret': l2tp_ipsec_secret})); await s.commit()
    return ok(request, '/admin/plans', 'Public plan added')
@router.post('/admin/plans/{pid}/edit')
async def plan_edit(request: Request, pid:int, title:str=Form(...), volume_gb:int=Form(0), duration_days:int=Form(0), price_irt:int=Form(0), category_id:int=Form(0), server_id:int=Form(0), mikrotik_protocol:str=Form('openvpn'), openvpn_profile_id:int=Form(0), l2tp_server:str=Form(''), l2tp_ipsec_secret:str=Form(''), _: str = Depends(_auth_user)):
    if not category_id:
    ...
    async with SessionLocal() as s:
        ...
        if not srv:
            return fail(request, 'Selected server was not found.', 404)
        inbs=(srv.meta or {}).get('inbound_ids') if srv else []
        obj.title=title; obj.volume_gb=volume_gb; obj.duration_days=duration_days; obj.price_irt=price_irt; obj.category_id=category_id; obj.server_id=server_id; obj.inbound_ids=inbs or []; obj.meta={'mikrotik_protocol': mikrotik_protocol, 'openvpn_profile_id': int(openvpn_profile_id or 0), 'l2tp_server': l2tp_server, 'l2tp_ipsec_secret': l2tp_ipsec_secret}
        await s.commit()
...
async def reseller_plan_add(request: Request, title:str=Form(...), volume_gb:int=Form(0), reseller_validity_days:int=Form(365), price_irt:int=Form(0), server_id:int=Form(0), mikrotik_protocol:str=Form('openvpn'), openvpn_profile_id:int=Form(0), l2tp_server:str=Form(''), l2tp_ipsec_secret:str=Form(''), _: str = Depends(_auth_user)):
    if not server_id:
    ...
    async with SessionLocal() as s:
        ...
        if not srv:
            return fail(request, 'Selected server was not found.', 404)
        s.add(ResellerPackage(title=title, volume_gb=volume_gb, reseller_validity_days=reseller_validity_days, price_irt=price_irt, server_id=server_id, is_active=True, meta={'mikrotik_protocol': mikrotik_protocol, 'openvpn_profile_id': int(openvpn_profile_id or 0), 'l2tp_server': l2tp_server, 'l2tp_ipsec_secret': l2tp_ipsec_secret}))
        await s.commit()
    return ok(request, '/admin/plans', 'Reseller plan added')

@router.post('/admin/plans/reseller/{pid}/edit')
async def reseller_plan_edit(request: Request, pid:int, title:str=Form(...), volume_gb:int=Form(0), reseller_validity_days:int=Form(365), price_irt:int=Form(0), server_id:int=Form(0), mikrotik_protocol:str=Form('openvpn'), openvpn_profile_id:int=Form(0), l2tp_server:str=Form(''), l2tp_ipsec_secret:str=Form(''), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        ...
        try:
            ...
            obj.meta={'mikrotik_protocol': mikrotik_protocol, 'openvpn_profile_id': int(openvpn_profile_id or 0), 'l2tp_server': l2tp_server, 'l2tp_ipsec_secret': l2tp_ipsec_secret}
            await s.commit()
        except Exception:
            await s.rollback()
...
async def disc_add(request: Request, code:str=Form(...), discount_type:str=Form('percent'), value:int=Form(...), max_uses:int=Form(1), per_user_limit:int=Form(1), _: str = Depends(_auth_user)):
    code=(code or '').strip().upper().replace(' ','')
    async with SessionLocal() as s: s.add(DiscountCode(code=code,discount_type=discount_type,value=value,max_uses=max_uses,per_user_limit=per_user_limit,is_active=True)); await s.commit()
    return ok(request, '/admin/discounts', 'Done')
@router.post('/admin/discounts/{did}/edit')
async def disc_edit(request: Request, did:int, code:str=Form(...), discount_type:str=Form('percent'), value:int=Form(...), max_uses:int=Form(1), per_user_limit:int=Form(1), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        obj=await s.get(DiscountCode,did)
        if obj: obj.code=(code or '').strip().upper().replace(' ',''); obj.discount_type=discount_type; obj.value=value; obj.max_uses=max_uses; obj.per_user_limit=per_user_limit; await s.commit()
    return ok(request, '/admin/discounts', 'Done')
...
async def resellers(request: Request, _: str = Depends(_auth_user)):
    ...
    cards=''
    for r,u in rows:
        remain=max(0,(r.total_bytes or 0)-(r.used_bytes or 0)-(r.reserved_bytes or 0)); days='-' if not r.expires_at else max(0,(r.expires_at-datetime.utcnow()).days)
        edit='<form method="post" action="/admin/resellers/'+str(r.id)+'/edit" class="formgrid">'+field('total_gb','Total GB','number',int((r.total_bytes or 0)/1024**3))+field('used_gb','Used GB','number',int((r.used_bytes or 0)/1024**3))+field('reserved_gb','Reserved GB','number',int((r.reserved_bytes or 0)/1024**3))+field('days','Remaining days','number',days if days!='-' else 0)+'<div class="full"><button class="btn primary">Save</button></div></form>'
        cards+=f"""<div class="card"><h3>🤝 {e(u.full_name or u.username or u.telegram_id)}</h3><p class="muted">ID: {u.telegram_id}</p><div class="kvs"><div class="kv"><span>Total volume</span><b>{gb(r.total_bytes)}</b></div><div class="kv"><span>Used</span><b>{gb(r.used_bytes)}</b></div><div class="kv"><span>Reserved / remaining</span><b>{gb(r.reserved_bytes)} / {gb(remain)}</b></div><div class="kv"><span>Remaining days</span><b>{days}</b></div></div><button class="btn" onclick="openModal('resEdit{r.id}')">Edit</button><button class="btn danger" onclick="askDelete('/admin/resellers/{r.id}/delete')">Delete</button></div>"""+modal('resEdit'+str(r.id),'Edit resellers','Edit Reseller',edit)
...
async def reseller_edit(request: Request, rid:int, total_gb:int=Form(0), used_gb:int=Form(0), reserved_gb:int=Form(0), days:int=Form(0), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        obj=await s.get(ResellerAccount,rid)
        if obj:
            obj.total_bytes=total_gb*1024**3; obj.used_bytes=used_gb*1024**3; obj.reserved_bytes=reserved_gb*1024**3; obj.expires_at=datetime.utcnow()+timedelta(days=days) if days else None; await s.commit()
    return ok(request, '/admin/resellers', 'Done')
...
def _backup_models():
    return [
        ...
        ('payg_usage_logs', PaygUsageLog), ('anti_sharing_violations', AntiSharingViolation),
...
def _restore_delete_order():
    return [
        TicketMessage, Ticket, PaygUsageLog, AntiSharingViolation, WalletTransaction, Order,
...
async def settings_save(request: Request, bot_name:str=Form(...), support:str=Form(...), description:str=Form(''), _: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        s.merge(Setting(key='bot_name',value=bot_name)); s.merge(Setting(key='support_username',value=support)); s.merge(Setting(key='admin_description',value=description)); await s.commit()
    return ok(request, '/admin/settings', 'Done')
...
def srv_json(sv):
    ...
    return {
        ...
        'server_type': sv.server_type, 'panel_url': sv.panel_url,
        'panel_base_url': m.get('panel_base_url') or '', 'panel_path': m.get('panel_path') or '/',
        'subscription_url': sv.subscription_url, 'username': sv.username,
...
def plan_json(p):
    return {
        ...
        'inbound_ids': p.inbound_ids or [], 'is_payg': p.is_payg,
        'is_unlimited': p.is_unlimited, 'anti_sharing_enabled': p.anti_sharing_enabled,
...
async def api_v2_dashboard(start_date: str | None = None, end_date: str | None = None, _: str = Depends(_auth_user)):
    ...
    async with SessionLocal() as s:
        ...
        wallet_total = await s.scalar(select(func.coalesce(func.sum(User.wallet_balance + User.wallet_v2ray_balance + User.wallet_openvpn_balance), 0))) or 0
...
async def api_v2_categories(_: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        items=(await s.execute(select(ServerCategory).order_by(ServerCategory.id.desc()))).scalars().all()
    return {'ok':True,'items':[{'id':c.id,'name':c.name,'server_id':c.server_id} for c in items]}
...
async def api_v2_discounts(_: str = Depends(_auth_user)):
    ...
    return {'ok':True,'items':[{'id':d.id,'code':d.code,'discount_type':d.discount_type,'value':int(d.value or 0),'max_uses':d.max_uses,'per_user_limit':d.per_user_limit,'used_count':d.used_count,'expires_at':dt_iso(d.expires_at),'is_active':d.is_active} for d in items]}

@router.get('/admin/api/v2/resellers')
async def api_v2_resellers(_: str = Depends(_auth_user)):
    async with SessionLocal() as s:
        rows=(await s.execute(select(ResellerAccount, User).join(User, User.id==ResellerAccount.user_id).order_by(ResellerAccount.id.desc()))).all()
    return {'ok':True,'items':[{'id':r.id,'user_id':r.user_id,'user':user_json(u),'server_id':r.server_id,'total_bytes':int(r.total_bytes or 0),'used_bytes':int(r.used_bytes or 0),'reserved_bytes':int(r.reserved_bytes or 0),'expires_at':dt_iso(r.expires_at),'is_active':r.is_active,'created_at':dt_iso(r.created_at)} for r,u in rows]}

@router.get('/admin/api/v2/orders')
async def api_v2_orders(start_date:str|None=None, end_date:str|None=None, page:int=1, page_size:int=100, _: str = Depends(_auth_user)):
    now=datetime.utcnow(); page=max(1,page); page_size=max(1,min(page_size,200)); offset=(page-1)*page_size
    start_dt=datetime.strptime(start_date,'%Y-%m-%d') if start_date else now-timedelta(days=30)
    end_dt=(datetime.strptime(end_date,'%Y-%m-%d')+timedelta(days=1)) if end_date else now+timedelta(days=1)
    async with SessionLocal() as s:
        stmt=select(Order).where(Order.created_at>=start_dt, Order.created_at<end_dt)
        total=await s.scalar(select(func.count()).select_from(stmt.subquery())) or 0
        orders=(await s.execute(stmt.order_by(Order.id.desc()).offset(offset).limit(page_size))).scalars().all()
        uids=[o.user_id for o in orders]; pids=[o.plan_id for o in orders if o.plan_id]
        users={u.id:u for u in (await s.execute(select(User).where(User.id.in_(uids)))).scalars().all()} if uids else {}
        plans={p.id:p for p in (await s.execute(select(Plan).where(Plan.id.in_(pids)))).scalars().all()} if pids else {}
    return {'ok':True,'total':int(total),'page':page,'page_size':page_size,'items':[order_json(o, users.get(o.user_id), plans.get(o.plan_id)) for o in orders]}
