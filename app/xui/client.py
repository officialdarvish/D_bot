...
class XUIClient:
    ...
    async def reset_client_plan(self, email: str, total_gb: int | float, expire_days: int, inbound_ids: Iterable[int] | None = None, current_inbound_ids_hint: Iterable[int] | None = None) -> dict[str, Any]:
        ...
        desired_inbound_ids = self._clean_int_ids(inbound_ids or []) if inbound_ids is not None else []
        if inbound_ids is not None and not desired_inbound_ids:
            raise RuntimeError("X-UI renew failed: no valid inbound IDs were selected for this plan")

        # Keep the client attached only to the inbounds selected by the plan.
        # This runs before resetTraffic so the renewal operation applies to the
        # exact plan scope, including when an administrator changed a plan from
        # Automatic to Manual or selected a different inbound set.
        if desired_inbound_ids:
            current_inbound_ids = self._clean_int_ids(found.get("inbound_ids") or current_inbound_ids_hint or [])
            missing_ids = [iid for iid in desired_inbound_ids if iid not in current_inbound_ids]
            extra_ids = [iid for iid in current_inbound_ids if iid not in desired_inbound_ids]
            if missing_ids:
                await self.attach_client(email, missing_ids)
            if extra_ids:
        ...
        # Apply the quota and expiry only to the inbounds selected by this
        # plan. Automatic plans pass the current live server list; Manual plans
        # pass only the administrator-selected IDs.
        data = await self._update_client(client, inbound_ids=(desired_inbound_ids or None))
        ...
        verified_found = await self.find_client(email)
        actual_inbound_ids = self._clean_int_ids((verified_found or {}).get("inbound_ids") or desired_inbound_ids)
        if desired_inbound_ids and set(actual_inbound_ids) != set(desired_inbound_ids):
            raise RuntimeError(
                "X-UI renew failed: client inbound assignment did not match plan "
                f"(expected={desired_inbound_ids}, actual={actual_inbound_ids})"
            )
