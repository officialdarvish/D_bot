...
class XUIClient:
    ...
    def __init__(self, base_url: str, username: str, password: str, *, timeout: float | httpx.Timeout | None = None) -> None:
        ...
        request_timeout = timeout or httpx.Timeout(
            connect=max(float(settings.XUI_CONNECT_TIMEOUT_SECONDS or 8.0), 1.0),
            read=max(float(settings.XUI_READ_TIMEOUT_SECONDS or 35.0), 5.0),
            write=max(float(settings.XUI_WRITE_TIMEOUT_SECONDS or 25.0), 5.0),
    ...
    async def _request(self, method: str, path: str, **kwargs: Any) -> Any:
        # GET/HEAD calls are safe to retry. Some Sanaei panels pause briefly
        # while expiring/enabling a client, which previously caused a hard
        # ReadTimeout after only 12 seconds. Mutating POST requests are NOT
        # retried here because the panel may have completed them before the
        # connection timed out; renewal code reconciles those operations first.
        retry_safe = bool(kwargs.pop("_retry_safe", False)) or str(method).upper() in {"GET", "HEAD", "OPTIONS"}
        attempts = self.read_retry_attempts if retry_safe else 1
        ...
        for attempt in range(attempts):
            try:
                ...
                if not retry_safe or not self._is_transient_transport_error(exc) or attempt >= attempts - 1:
                    raise
                last_exc = exc
                self.last_error = f"Transient {type(exc).__name__} on {method} {path}; retry {attempt + 2}/{attempts}"
                await asyncio.sleep((0.35, 0.8, 1.4, 2.0)[min(attempt, 3)])
        if last_exc is not None:
            raise last_exc
        raise RuntimeError(f"X-UI request failed: {method} {path}")
    ...
    async def reset_client_plan(self, email: str, total_gb: int | float, expire_days: int, inbound_ids: Iterable[int] | None = None, current_inbound_ids_hint: Iterable[int] | None = None, *, exact_inbound_scope: bool = False) -> dict[str, Any]:
        ...
        client. Existing inbound attachments are always preserved. When
        ``inbound_ids`` is provided, those IDs are reattached after enable so a
        Sanaei auto-detach on expiry cannot leave the renewed service unusable.
        By default no inbound is detached. When ``exact_inbound_scope`` is true
        (used by a reseller in Manual mode), memberships outside the configured
        scope are removed after the required IDs are reattached.
        ``current_inbound_ids_hint`` can restore memberships recorded locally
        only when no explicit current plan scope is supplied. When the caller
        supplies ``inbound_ids``, historical hints are non-authoritative so IDs
        from a replaced server cannot break renewal verification.
        """
        found = await self.find_client(email)
        if not found or not found.get("client"):
            raise RuntimeError("X-UI client not found")
        ...
        client = dict(found["client"])
        panel_inbound_ids_before = self._clean_int_ids(found.get("inbound_ids") or [])
        preserved_inbound_ids = self._clean_int_ids(
            [*panel_inbound_ids_before, *(current_inbound_ids_hint or [])]
        )
        additive_inbound_ids = self._clean_int_ids(inbound_ids or [])

        # When renewal has an explicit current plan scope, that scope is the only
        # authoritative set of memberships that must exist after renewal.  The
        # locally stored/current hint may contain IDs from a server/inbound set
        # that was deleted or replaced after the service was created.  Treating
        # those stale IDs as required made an otherwise successful renewal fail
        # forever after a server migration (for example old IDs 149/150 no longer
        # existing on the new panel).
        #
        # Existing extra memberships are still never detached: updateClient omits
        # inboundIds entirely.  We simply do not try to resurrect stale historical
        # IDs when the caller has supplied the current plan's inbound scope.
        explicit_plan_scope = bool(additive_inbound_ids)
        required_inbound_ids = self._clean_int_ids(
            additive_inbound_ids if explicit_plan_scope else preserved_inbound_ids
        )
        ...
        def used_bytes(snapshot: dict[str, Any] | None) -> int:
            snapshot = snapshot or {}
            for key in ("used_traffic", "usedTraffic"):
                if key in snapshot:
                    try:
                        return max(int(snapshot.get(key) or 0), 0)
                    except Exception:
                        pass
            traffic = snapshot.get("traffic") or {}
            try:
                return max(int(traffic.get("up") or 0) + int(traffic.get("down") or 0), 0)
            except Exception:
                return 0
        ...
        # Additional memberships are applied later through an attach endpoint.
        ...
        async def read_after_traffic_reset(delays: tuple[float, ...]) -> tuple[dict[str, Any], bool, int]:
            latest: dict[str, Any] = {}
            latest_used = before_used
            for delay in delays:
                if delay:
                    await asyncio.sleep(delay)
                candidate = await self.find_client(email)
                if candidate and candidate.get("client"):
                    latest = candidate
                    latest_used = used_bytes(candidate)
                    # Exact zero is ideal. A lower value also proves the old
                    # cycle was cleared and a few new bytes arrived meanwhile.
                    if before_used <= 0 or latest_used == 0 or latest_used < before_used:
                        return latest, True, latest_used
            return latest, False, latest_used

        traffic_reset_errors: list[str] = []
        try:
            await self.reset_client_traffic(email)
        except Exception as exc:
            # A POST can time out after Sanaei has already committed the reset.
            # Reconcile from the panel before retrying so a transient slow reply
            # does not abort the reseller renewal.
            if not self._is_transient_transport_error(exc):
                raise
            traffic_reset_errors.append(f"{type(exc).__name__}: {exc}")
        refreshed, traffic_reset_ok, after_reset_used = await read_after_traffic_reset(
            (0.0, 0.25, 0.6, 1.0)
        )

        # One retry handles both a real reset race and a timed-out request that
        # was not committed by the panel. Verify again after the retry.
        if before_used > 0 and not traffic_reset_ok:
            try:
                await self.reset_client_traffic(email)
            except Exception as exc:
                if not self._is_transient_transport_error(exc):
        ...
        if not refreshed or not refreshed.get("client"):
            raise RuntimeError("X-UI renew failed: client disappeared after resetTraffic")
        if before_used > 0 and not traffic_reset_ok:
            transport_detail = f", transient_errors={traffic_reset_errors}" if traffic_reset_errors else ""
            raise RuntimeError(
                "X-UI renew failed: consumed traffic did not reset "
                f"(email={email}, before_used={before_used}, after_reset_used={after_reset_used}{transport_detail})"
            )

        client = dict(refreshed["client"])
        ...
        # transport timeout is ambiguous: Sanaei may have committed the update
        # but failed to return the response. Verify first, then retry at most once.
        update_transport_error: Exception | None = None
        try:
            data = await self._update_client(client)
        except Exception as exc:
            if not self._is_transient_transport_error(exc):
                raise
            update_transport_error = exc
            data = {"success": False, "transport_timeout_reconciled": True}
        ...
        verified = await verify_direct((0.0, 0.35, 0.75, 1.25, 2.0))

        # If updateClient timed out and the desired state is not visible, retry
        # the idempotent set operation once. This is safe because quota/expiry
        # are absolute values, not increments.
        if not verified and update_transport_error is not None:
            try:
                data = await self._update_client(client)
                update_transport_error = None
            except Exception as exc:
                if not self._is_transient_transport_error(exc):
                    raise
                update_transport_error = exc
            verified = await verify_direct((0.2, 0.6, 1.2, 2.0))

        # Some 3x-ui versions accept quota/expiry but keep enable=false. Enable
        # through the dedicated endpoint, then verify again. Neither fallback
        # receives inbound IDs, so membership remains untouched.
        enable_error: Exception | None = None
        if not verified and not real_enabled:
            try:
                await self.bulk_set_enabled([email], True)
            except Exception as exc:
                enable_error = exc
                try:
                    await self.set_client_enabled(email, True)
                    enable_error = None
                except Exception as fallback_exc:
                    enable_error = fallback_exc
            if enable_error is None:
                verified = await verify_direct((0.0, 0.3, 0.7, 1.2))
        ...
        if not verified:
            enable_detail = f", enable_error={enable_error}" if enable_error else ""
            update_detail = f", update_transport_error={update_transport_error}" if update_transport_error else ""
            raise RuntimeError(
        ...
        verified_found = await self.find_client(email) or latest_found
        panel_inbound_ids = self._clean_int_ids((verified_found or {}).get("inbound_ids") or [])

        # Reassert the configured scope after the client is enabled. Sanaei can
        # detach memberships while disabling an expired client, and some builds
        # briefly return stale membership data after re-enable. Therefore every
        # explicitly configured ID is sent to attach again, even when the lookup
        # says it is already present. Existing extras are never detached.
        if explicit_plan_scope:
            # Reassert only the current plan scope.  Historical hints are not
            # authoritative across server/inbound migrations and may reference
            # IDs that no longer exist on this panel.
            attach_ids = list(additive_inbound_ids)
        else:
            # Generic reset without an explicit plan keeps the old preservation
            # behavior and restores locally-known memberships if Sanaei detached
            # them while disabling the client.
            attach_ids = self._clean_int_ids(
                iid for iid in preserved_inbound_ids if iid not in panel_inbound_ids
            )
        if attach_ids:
            attach_error: Exception | None = None
            try:
                await self.attach_client(email, attach_ids)
            except Exception as exc:
                attach_error = exc
                # If attach-all timed out, it may still have been committed.
                # Read the panel first and retry only memberships truly missing.
                try:
                    current_after_error = await self.find_client(email)
                except Exception:
                    current_after_error = None
                current_ids = self._clean_int_ids((current_after_error or {}).get("inbound_ids") or [])
                still_missing = [iid for iid in required_inbound_ids if iid not in current_ids]
                if not still_missing:
                    attach_error = None
                    panel_inbound_ids = current_ids
                    verified_found = current_after_error or verified_found
                else:
                    try:
                        await self.bulk_attach_clients([email], still_missing)
                        attach_error = None
                    except Exception as fallback_exc:
                        attach_error = fallback_exc
                        # bulkAttach can have the same ambiguous timeout. One
                        # final read decides whether the write actually landed.
                        if self._is_transient_transport_error(fallback_exc):
                            try:
                                await asyncio.sleep(0.4)
                                after_bulk_timeout = await self.find_client(email)
                            except Exception:
                                after_bulk_timeout = None
                            ids_after_timeout = self._clean_int_ids(
                                (after_bulk_timeout or {}).get("inbound_ids") or []
                            )
                            if ids_after_timeout and set(required_inbound_ids).issubset(set(ids_after_timeout)):
                                attach_error = None
                                panel_inbound_ids = ids_after_timeout
                                verified_found = after_bulk_timeout or verified_found
            if attach_error is not None:
                raise RuntimeError(
                    "X-UI renew failed: could not reattach required inbounds "
                    f"(email={email}, requested={attach_ids}, error={type(attach_error).__name__}: {attach_error})"
                ) from attach_error

            for delay in (0.0, 0.25, 0.6, 1.0):
                if delay:
                    await asyncio.sleep(delay)
                attached_found = await self.find_client(email)
                attached_ids = self._clean_int_ids((attached_found or {}).get("inbound_ids") or [])
                if attached_ids:
                    panel_inbound_ids = attached_ids
                    verified_found = attached_found or verified_found
                # Some older panels do not return inbound membership in the
                # client lookup response. In that case the successful attach
                # response is the best available confirmation.
                if not attached_ids or set(required_inbound_ids).issubset(set(attached_ids)):
                    break

        if panel_inbound_ids:
            missing_after = [iid for iid in required_inbound_ids if iid not in panel_inbound_ids]
            if missing_after:
                raise RuntimeError(
                    "X-UI renew failed: required inbound memberships are missing "
                    f"(required={required_inbound_ids}, after={panel_inbound_ids}, missing={missing_after})"
                )

        # A reseller in Manual mode is an exact allow-list, not merely an
        # additive list. If this service previously had all server inbounds,
        # remove memberships outside the reseller's selected scope. This runs
        # only when the caller explicitly opts into exact_inbound_scope; public
        # renewals and automatic reseller renewals keep their additive behavior.
        if exact_inbound_scope and required_inbound_ids and panel_inbound_ids:
            extra_ids = [iid for iid in panel_inbound_ids if iid not in required_inbound_ids]
            if extra_ids:
                detach_returned = False
                detach_error: Exception | None = None
                try:
                    await self.detach_client(email, extra_ids)
                    detach_returned = True
                except Exception as exc:
                    detach_error = exc
                    if not self._is_transient_transport_error(exc):
                        raise RuntimeError(
                            "X-UI renew failed: could not enforce reseller manual inbound scope "
                            f"(email={email}, extra={extra_ids}, error={type(exc).__name__}: {exc})"
                        ) from exc
                latest_scope = None
                for delay in (0.0, 0.3, 0.7):
                    if delay:
                        await asyncio.sleep(delay)
                    try:
                        latest_scope = await self.find_client(email)
                    except Exception:
                        latest_scope = None
                    ids_now = self._clean_int_ids((latest_scope or {}).get("inbound_ids") or [])
                    if ids_now:
                        still_extra = [iid for iid in ids_now if iid not in required_inbound_ids]
                        missing_required = [iid for iid in required_inbound_ids if iid not in ids_now]
                        if not still_extra and not missing_required:
                            panel_inbound_ids = ids_now
                            verified_found = latest_scope or verified_found
                            detach_error = None
                            break
                    elif detach_returned:
                        # Older Sanaei builds can omit inboundIds from client/get.
                        # A successful detach response is the strongest available
                        # confirmation; return the exact configured scope locally.
                        panel_inbound_ids = list(required_inbound_ids)
                        detach_error = None
                        break
                if detach_error is not None:
                    raise RuntimeError(
                        "X-UI renew failed: manual reseller inbound cleanup could not be confirmed "
                        f"(email={email}, required={required_inbound_ids}, extra={extra_ids})"
                    ) from detach_error
                if panel_inbound_ids:
                    still_extra = [iid for iid in panel_inbound_ids if iid not in required_inbound_ids]
                    if still_extra:
                        raise RuntimeError(
                            "X-UI renew failed: reseller manual inbound scope still has extra memberships "
                            f"(required={required_inbound_ids}, after={panel_inbound_ids}, extra={still_extra})"
                        )

        actual_inbound_ids = list(required_inbound_ids) if exact_inbound_scope and required_inbound_ids else (panel_inbound_ids or required_inbound_ids)

        return {
            "result": data,
            "client": real or client,
            "inbound_ids": actual_inbound_ids,
