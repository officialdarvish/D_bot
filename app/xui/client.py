...
class XUIClient:
    ...
    async def reset_client_plan(self, email: str, total_gb: int | float, expire_days: int, inbound_ids: Iterable[int] | None = None, current_inbound_ids_hint: Iterable[int] | None = None) -> dict[str, Any]:
        ...
        No inbound is ever detached by this renewal path.
        ``current_inbound_ids_hint`` can restore memberships recorded locally
        when an older buggy renewal already removed them from the panel.
        """
        found = await self.find_client(email)
        if not found or not found.get("client"):
            raise RuntimeError("X-UI client not found")
        ...
        required_inbound_ids = self._clean_int_ids(
            [*preserved_inbound_ids, *additive_inbound_ids]
        )
        ...
        # One retry handles panels where the first reset request races with a
        # traffic aggregation job. Do not silently start a new cycle with the
        # old usage counter still applied.
        ...
        if before_used > 0 and not traffic_reset_ok:
            raise RuntimeError(
                ...
                f"(email={email}, before_used={before_used}, after_reset_used={after_reset_used})"
            )
        ...
        # in place and keeps every inbound exactly as it was before renewal.
        ...
        if not verified:
            ...
            raise RuntimeError(
                ...
                f"after_enabled={real_enabled}{enable_detail}"
            )
        ...
        configured_reassert_ids = list(additive_inbound_ids)
        missing_preserved_ids = [
            iid for iid in required_inbound_ids
            if iid not in configured_reassert_ids and iid not in panel_inbound_ids
        ]
        attach_ids = self._clean_int_ids([*configured_reassert_ids, *missing_preserved_ids])
        if attach_ids:
            ...
            try:
                ...
                attach_error = exc
                # If attach-all failed because some memberships already exist,
                # read the panel and retry only the IDs that are still missing.
            ...
            if attach_error is not None:
                raise RuntimeError(
                    ...
                    f"(email={email}, requested={attach_ids}, error={attach_error})"
                ) from attach_error
        ...
        actual_inbound_ids = panel_inbound_ids or required_inbound_ids
