...
class XUIClient:
    ...
    async def add_client_to_inbounds(
        ...
        *,
        ...
        if deleted_targets:
            await self.purge_deleted_clients_everywhere(deleted_targets, inbound_ids=clean_ids, attempts=3)
        ...
        try:
            ...
            if self._is_success(data):
                ...
                if deleted_targets:
                    ...
                    if post_targets:
                        await self.purge_deleted_clients_everywhere(post_targets, inbound_ids=clean_ids, attempts=3)
    ...
    async def _update_inbound_settings(self, inbound: dict[str, Any], settings: dict[str, Any]) -> Any:
        ...
        # Sanaei / 3x-ui builds differ here: newer builds accept JSON, while some
        # older builds only persist inbound edits when sent as form data. Try both
        # and verify by re-reading the inbound settings so a stale client cannot be
        # silently resurrected later.
        for kwargs in ({"json": payload}, {"data": payload}):
            try:
                data = await self._request("POST", f"/panel/api/inbounds/update/{inbound_id}", **kwargs)
                if self._is_success(data):
                    return data
                last_error = RuntimeError(self._error_message(data) or f"X-UI inbound {inbound_id} cleanup failed")
            except Exception as exc:
                last_error = exc

        if last_error:
            raise RuntimeError(f"X-UI inbound {inbound_id} cleanup failed: {last_error}")
        raise RuntimeError(f"X-UI inbound {inbound_id} cleanup failed")

    async def _client_exists_in_inbound_settings(
        self,
    ...
    async def _legacy_delete_client_from_inbounds(
        self,
        identifiers: Iterable[str | None],
        *,
        inbound_ids: list[int] | None = None,
    ) -> int:
        """Best-effort legacy delete for stale inbound.settings clients.

        Some panels remove the canonical client row with /clients/del but keep the
        client object inside inbound.settings.clients[]. The next panel save/add can
        re-create the user. 3x-ui/x-ui variants commonly expose an inbound-scoped
        delClient route that removes exactly that stale object, so try it before
        falling back to full inbound settings rewrite.
        """
        targets = self._identifier_tokens(identifiers)
        if not targets:
            return 0
        ...
        for inbound in await self.get_inbounds():
            ...
            for client in clients:
                if not isinstance(client, dict):
                    continue
                if not self._client_identifier_values(client).intersection(targets):
                    continue
                values = self._identifier_tokens([
                    client.get("id"),
                    client.get("uuid"),
                    client.get("clientUuid"),
                    client.get("client_uuid"),
                    client.get("password"),
                    client.get("auth"),
                    client.get("email"),
                    client.get("subId"),
                ...
                for value in values:
                    ...
                    for path in (
                        f"/panel/api/inbounds/{inbound_id}/delClient/{encoded}",
                        f"/panel/api/inbounds/delClient/{encoded}",
                    ):
                        try:
    ...
    async def purge_deleted_clients_everywhere(
        ...
        *,
        ...
        for attempt in range(attempts):
            ...
            legacy_count += await self._legacy_delete_client_from_inbounds(expanded, inbound_ids=inbound_ids)
            purged_count += await self.purge_client_from_inbounds(*expanded, inbound_ids=inbound_ids)
            if attempt < attempts - 1:
                await asyncio.sleep(0.7)

        return {"deleted": deleted_count, "legacy_deleted": legacy_count, "purged": purged_count}
    ...
    async def delete_client(self, *identifiers: str | None) -> dict[str, Any]:
        ...
        for candidate in candidates:
            ...
            try:
                ...
                if self._is_success(data):
                    ...
                    await self.purge_deleted_clients_everywhere(candidates, attempts=4)
                    if (
                        not await self._client_exists_after_delete(candidate)
                        and not await self._client_exists_in_inbound_settings(candidates)
                    ):
                        return data
                    errors.append(f"{candidate}: delete returned success but stale client still exists")
                    continue
        ...
        for candidate in candidates:
            ...
            try:
                data = await self._request("POST", f"/panel/api/clients/del/{encoded}")
                if self._is_success(data):
                    await self.purge_deleted_clients_everywhere([email, *candidates], attempts=4)
                    if (
                        not await self._client_exists_after_delete(email)
                        and not await self._client_exists_in_inbound_settings([email, *candidates])
                    ):
                        return data
                errors.append(f"{email}: delete did not remove client or stale inbound copy")
            except Exception as exc:
        ...
        if email_like:
            try:
                ...
                if self._is_success(data):
                    await self.purge_deleted_clients_everywhere([*email_like, *candidates], attempts=4)
                    still_existing = []
                    for email in email_like:
                        if await self._client_exists_after_delete(email):
                            still_existing.append(email)
                    if not still_existing and not await self._client_exists_in_inbound_settings([*email_like, *candidates]):
                        return data
                    errors.append("bulkDel left clients/stale inbound copies: " + ", ".join(still_existing or email_like))
            except Exception as exc:
