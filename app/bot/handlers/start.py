...
from app.bot.keyboards.common import (
    CB_ADMIN, CB_BACK_MAIN, CB_BACK_ADMIN, CB_REFERRAL, main_menu_inline, admin_panel_inline, rules_keyboard, back_button, report_home_inline
)
...
def _extract_start_payload(message: Message | None) -> str:
...
def _event_bot(event: Message | CallbackQuery):
    bot = getattr(event, 'bot', None)
    if bot:
        return bot
    event_message = getattr(event, 'message', None)
    return getattr(event_message, 'bot', None)
...
async def _notify_admins_about_new_user(bot, session, user: User, telegram_user, payload: str = '') -> None:
    """Notify owners/admins when a Telegram user is created for the first time.

    This must never break /start, so every send/query is best-effort.
    """
    if bot is None:
        return
    try:
    ...
    try:
        ...
        if getattr(user, 'referred_by_user_id', None):
            ref_q = await session.execute(select(User).where(User.id == user.referred_by_user_id))
            ref_user = ref_q.scalar_one_or_none()
            if ref_user:
                referrer_line = (
                    f'{ref_user.full_name or "-"} | '
                    f'{_display_username(ref_user.username)} | '
                    f'ID: {ref_user.telegram_id}'
                )
        ...
        joined_at = getattr(user, 'joined_at', None) or datetime.utcnow()
        joined_text = joined_at.strftime('%Y-%m-%d %H:%M UTC') if hasattr(joined_at, 'strftime') else str(joined_at)
        start_payload_line = f'\n🎯 Start: {payload}' if payload else ''
        referrer_short = '' if referrer_line == '-' else f'\n👥 Ref: {referrer_line}'

        text_msg = (
            '👤 کاربر جدید\n'
            '━━━━━━━━━━━━━━\n'
            f'🆔 ID: {user.telegram_id}\n'
            f'👤 User: {_display_username(user.username)}\n'
            f'📝 Name: {user.full_name or "-"}\n'
            f'🔗 Link: tg://user?id={user.telegram_id}'
            f'{start_payload_line}'
            f'{referrer_short}\n'
            '━━━━━━━━━━━━━━\n'
            f'📊 Total: {total_users if total_users is not None else "-"}\n'
            f'⏰ {joined_text}'
        )

        for admin_id in admin_ids:
            try:
                await bot.send_message(admin_id, text_msg, reply_markup=report_home_inline())
            except Exception:
                pass
    except Exception:
        # Do not interrupt user registration if notification formatting fails.
        pass

async def get_or_create_user(event: Message | CallbackQuery) -> User:
    telegram_user = getattr(event, 'from_user', None)
    if telegram_user is None:
        raise ValueError('Cannot resolve Telegram user from event')

    telegram_id = int(telegram_user.id)
    username = telegram_user.username
    full_name = telegram_user.full_name
    payload = _clean_referral_payload(_extract_start_payload(event if isinstance(event, Message) else None))
    ...
    async with SessionLocal() as session:
        ...
        if user:
            ...
            if changed or not getattr(user, 'referral_code', None):
                try:
                    await session.commit()
                    if linked_to_referrer and bot:
                        await notify_referrer_about_new_subset(session, bot, user)
                except Exception:
                    await session.rollback()
            return user
        ...
        try:
            ...
            try:
                if bot:
                    await _notify_admins_about_new_user(bot, session, user, telegram_user, payload)
                if bot and user.referred_by_user_id:
                    await notify_referrer_about_new_subset(session, bot, user)
                if bot:
                    await maybe_grant_invite_reward(session, bot, user)
            except Exception:
                pass
            return user
        except Exception:
...
async def send_main_menu(target, user_id: int):
    ...
    if hasattr(target, 'answer'):
        await ui_page(target, welcome_text, reply_markup=await main_menu_inline(is_admin, await is_test_visible(), (is_admin or await is_reseller_menu_unlocked(user_id))))
    ...
    if bot_enabled != '1' and message.from_user.id not in settings.admin_ids:
        ...
        if not ok:
            ...
            sent = await _safe_send_start_page(
                ...
                reply_markup=await main_menu_inline(message.from_user.id in settings.admin_ids, await is_test_visible(), (message.from_user.id in settings.admin_ids or await is_reseller_menu_unlocked(message.from_user.id))),
            )
...
async def accept_rules(callback: CallbackQuery):
    ...
    await edit_or_answer(
        callback,
        await get_setting_value('welcome_text', WELCOME_TEXT_DEFAULT),
        reply_markup=await main_menu_inline(callback.from_user.id in settings.admin_ids, await is_test_visible(), (callback.from_user.id in settings.admin_ids or await is_reseller_menu_unlocked(callback.from_user.id))),
    )
...
@router.callback_query(F.data == 'report:home')
async def persistent_report_home(callback: CallbackQuery, state: FSMContext):
    """Keep the audit/notification message, remove its Home button, open main menu."""
    try:
        if callback.message:
