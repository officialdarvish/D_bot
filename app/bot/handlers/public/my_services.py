...
from app.services.renewal_scope import renewal_plan_allowed, service_purchase_category_id
...
async def _deny_invalid_renewal_plan(callback: CallbackQuery):
    await callback.answer(
        'این تعرفه متعلق به دسته‌ای نیست که سرویس از آن خریداری شده است.',
        show_alert=True,
    )
...
async def sync_service_from_panel(session, svc, *, delete_missing: bool = False) -> bool | None:
    ...
    if server.server_type == 'mikrotik':
        try:
            ...
            total = MikroTikService.logical_total_bytes(found, baseline, svc.total_bytes or 0)
...
def detail_kb(svc, plan, server_type: str = 'xui'):
    ...
    rows += [
        [InlineKeyboardButton(text='🔁 تمدید سرویس یا تغییر تعرفه', callback_data=f'svc:renew_menu:{svc.id}'), InlineKeyboardButton(text='🗑 حذف کانفیگ', callback_data=f'svc:delete:{svc.id}')],
...
async def my_services(event):
    ...
    async with SessionLocal() as session:
        ...
        # Keep this screen fast: never contact MikroTik/3x-ui while rendering the
        # services list. OpenVPN status and usage are synchronized only after the
        # user opens that specific service detail page.
...
async def renew_menu(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        purchase_category_id = await service_purchase_category_id(
            session, svc, persist_fallback=True
        )
        if not purchase_category_id:
            await session.rollback()
            await edit_or_answer(
                callback,
                'دسته خرید این سرویس مشخص نیست و امکان نمایش تعرفه‌های تمدید وجود ندارد. لطفاً با پشتیبانی تماس بگیرید.',
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button(f'svc:{sid}')]]),
            )
            await callback.answer()
            return
        plans = (await session.execute(
            select(Plan).where(
                Plan.server_id == svc.server_id,
                Plan.category_id == purchase_category_id,
                Plan.is_active == True,
            )
        )).scalars().all()
    ...
    if not plans:
        rows.append([InlineKeyboardButton(text='هیچ تعرفه فعالی در این دسته ثبت نشده است', callback_data='noop')])
...
async def show_renew_payment_methods(
    ...
    *,
    ...
    async with SessionLocal() as session:
        ...
        server = await session.get(Server, svc.server_id) if svc else None
        if not ok_owner or not svc or not plan or not server:
            await _deny_not_owned(callback)
            return False
        plan_allowed = await renewal_plan_allowed(
            session, svc, plan, require_active=True, persist_fallback=True
        )
        if not plan_allowed:
            await _deny_invalid_renewal_plan(callback)
            return False
        current_plan = await session.get(Plan, svc.plan_id) if svc.plan_id else None
...
async def renew_discount_start(callback: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        ok_owner, _user = await _owned_public_service(session, svc, callback.from_user.id)
        plan = await session.get(Plan, pid)
        if not ok_owner or not svc or not plan:
            await _deny_not_owned(callback)
            return
        plan_allowed = await renewal_plan_allowed(
            session, svc, plan, require_active=True, persist_fallback=True
...
async def renew_discount_apply(message: Message, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        server = await session.get(Server, svc.server_id) if svc else None
        plan_allowed = await renewal_plan_allowed(session, svc, plan, require_active=True, persist_fallback=True)
        if not ok_owner or not user or not svc or not plan or not server or not plan_allowed:
            await state.clear()
            await ui_message(message, 'این تعرفه مربوط به دسته خرید این سرویس نیست یا دیگر فعال نیست.', reply_markup=back_main_inline())
            return
...
async def _apply_service_renewal(session, svc: ClientService, user: User, plan: Plan, server: Server, *, amount_irt: int, payment_method: str) -> Order:
    if not svc.purchase_category_id:
        svc.purchase_category_id = plan.category_id
    order = Order(
        user_id=user.id,
        plan_id=plan.id,
        service_id=svc.id,
    ...
    await session.flush()
    if server.server_type == 'xui':
        renew_result = await XuiService().renew_client_on_plan(server, plan, svc.xui_email, current_inbound_ids=list(svc.inbound_ids or []))
        ...
        renew_result = await MikroTikService().renew_user(
            ...
            previous_used_bytes=svc.used_bytes,
        )
        svc.traffic_baseline_bytes = int(renew_result.get('_traffic_baseline_bytes') or 0)
    svc.plan_id = plan.id
...
async def admin_free_renew(callback: CallbackQuery, sid: int, pid: int) -> bool:
    ...
    async with SessionLocal() as session:
        ...
        plan = await session.get(Plan, pid)
        server = await session.get(Server, svc.server_id) if svc else None
        if not ok_owner or not user or not svc or not plan or not server:
            await _deny_not_owned(callback)
            return False
        plan_allowed = await renewal_plan_allowed(
            session, svc, plan, require_active=True, persist_fallback=True
...
async def delete_service(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        svc=await session.get(ClientService,sid)
        ok_owner, owner_user = await _owned_public_service(session, svc, callback.from_user.id)
        if not ok_owner:
            await _deny_not_owned(callback); return
        server=await session.get(Server,svc.server_id)
        ...
        if server and server.server_type == 'xui':
            ...
            try:
                ...
                if 'not found' in msg or 'not exist' in msg or 'not exists' in msg:
                    ...
                    deletion_task_id = deletion_task.id
                    panel_delete_queued = True
        if not panel_delete_queued:
            # The panel deletion succeeded (or the client was already absent),
            # so only the local tombstone/history cleanup remains.
            panel_status = (
    ...
    result_intro = (
        f'🕓 یوزرنیم {deleted_username or deleted_email or "-"} از ربات حذف شد و حذف از پنل در صف تلاش مجدد قرار گرفت.'
    ...
    deleted_text = (
        ...
        + ('\n\n🔁 به‌محض در دسترس شدن پنل، حذف نهایی خودکار انجام می‌شود.' if panel_delete_queued else '')
    )
    await edit_or_answer(
        callback,
        deleted_text,
