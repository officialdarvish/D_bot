...
from app.bot.service_presenter import mikrotik_guide_text
...
def detail_kb(svc, plan):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text='♻️ بروزرسانی کانفیگ', callback_data=f'svc:refresh:{svc.id}'), InlineKeyboardButton(text='🔄 باطل کردن و ارسال جدید', callback_data=f'svc:revoke:{svc.id}')],
        [InlineKeyboardButton(text='📥 دریافت پروفایل سرور', callback_data=f'svc:profile:{svc.id}')],
...
def service_detail_text(svc, plan, server_type: str = 'xui') -> str:
    ...
    if (server_type or '').lower() == 'mikrotik':
        ...
        body = (
            '🔐 اطلاعات اتصال (MikroTik)\n'
            f'├ 👤 نام کاربری: {svc.client_username}\n'
            f'╰ 🔑 رمز عبور: {password}\n\n'
            + mikrotik_guide_text() + '\n\n'
        )
    else:
...
async def render_detail(callback: CallbackQuery, sid: int):
    ...
    await edit_or_answer(callback, service_detail_text(svc, plan, server_type), reply_markup=detail_kb(svc, plan))
    ...
    try:
        for _ in range(20):
            ...
            async with SessionLocal() as session:
                ...
                server = await session.get(Server, svc.server_id)
                server_type = server.server_type if server else 'xui'
                text = service_detail_text(svc, plan, server_type)
...
async def revoke_service(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        try:
            ...
            sub=svc.sub_link
            plan = await session.get(Plan, svc.plan_id) if svc.plan_id else None
            detail_text = service_detail_text(svc, plan, server_type)
...
async def send_openvpn_profile(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if not prof and server:
            prof = (await session.execute(select(OpenVPNProfile).where(OpenVPNProfile.server_id == server.id, OpenVPNProfile.is_active == True).order_by(OpenVPNProfile.id.desc()))).scalar_one_or_none()
        if not prof:
            await callback.answer('برای این سرور هنوز پروفایل OpenVPN ثبت نشده است.', show_alert=True); return
    ...
    await callback.message.answer_document(
        ...
        caption='📥 پروفایل OpenVPN سرور\n\nفایل را داخل برنامه OpenVPN Import کنید. برای L2TP، اطلاعات یوزرنیم/پسورد را از همین کانفیگ داخل Settings آیفون وارد کنید.'
    )
    await callback.answer('پروفایل ارسال شد')
