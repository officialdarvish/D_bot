...
async def delete_local_service_records(session, svc):
    """Delete one local service safely while keeping accounting history.

    This is used when the client has already been removed directly from 3x-ui,
    so the bot must clean its own database without trying to delete from panel again.
    """
    if not svc:
        return
...
async def my_services(event):
    ...
    async with SessionLocal() as session:
        ...
        # Clean stale services that were deleted manually from 3x-ui before showing the list.
        for svc in list(services):
            await sync_service_from_panel(session, svc, delete_missing=True)
        services = (await session.execute(
            select(ClientService)
            .where(ClientService.user_id == user.id)
            .where(ClientService.reseller_id.is_(None))
...
async def revoke_service(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        try:
            ...
            svc.xui_uuid=new.get('uuid'); svc.sub_link=new.get('sub_link'); await session.commit(); sub=svc.sub_link
        except Exception as e:
            await edit_or_answer(callback, f'❌ تغییر لینک روی پنل با خطا مواجه شد:\n{e}', reply_markup=back_main_inline()); await callback.answer(); return
    # Show the new account data immediately, then open the main menu in a fresh message.
    if sub:
        img = qrcode.make(sub)
...
async def change_plan(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if server.server_type == 'xui':
            ...
            except Exception as e: await callback.answer(f'خطا در پنل: {type(e).__name__}', show_alert=True); return
...
async def renew_wallet(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        try:
            ...
            await callback.answer('تمدید روی پنل ناموفق بود.', show_alert=True)
            await edit_or_answer(callback, f'❌ تمدید سرویس روی پنل با خطا مواجه شد:\n{e}', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button(f'svc:renew_menu:{sid}')]]))
            return
    await render_detail(callback, sid)
...
async def delete_service(callback: CallbackQuery):
        ...
        if server and server.server_type == 'xui':
            try:
                ...
                if 'not found' in msg or 'not exist' in msg or 'not exists' in msg:
                    ...
                    await callback.answer(f'حذف از پنل ناموفق بود: {type(e).__name__}', show_alert=True); return
    ...
    await edit_or_answer(callback, deleted_text, reply_markup=main_menu_inline(callback.from_user.id in settings.admin_ids))
