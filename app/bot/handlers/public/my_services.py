...
def service_detail_text(svc, plan, server_type: str = 'xui', server=None) -> str:
    ...
    if (server_type or '').lower() == 'mikrotik':
        ...
        meta = getattr(server, 'meta', None) or {}
        l2tp_server = str(meta.get('l2tp_server') or 'de.rgiftshop.ir').strip() or 'de.rgiftshop.ir'
        private_key = str(meta.get('l2tp_ipsec_secret') or '123456789').strip() or '123456789'
        lines += [
            '🔐 اطلاعات ورود',
            f'├ Username: {username}',
            f'├ Password: {password}',
...
async def send_openvpn_profile(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        server = await session.get(Server, svc.server_id) if svc else None
        if not svc or not server:
            await callback.message.answer('این سرویس پیدا نشد.', reply_markup=back_main_inline())
            return
