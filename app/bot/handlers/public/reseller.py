...
import qrcode, tempfile, re, logging, random, string, asyncio, uuid
from app.database.session import SessionLocal
from app.database.models import User, Server, Plan, PaymentCard, ResellerAccount, ResellerPackage, ResellerTopupRequest, ResellerAccessRequest, ClientService, ResellerServiceActivity, DiscountCode, DiscountUsage, ResellerUsernameCounter
from app.bot.keyboards.common import CB_RESELLER, back_button, back_main_inline, main_menu_inline, get_user_button, report_home_inline
...
from app.services.reseller_service import (
    gb_to_bytes, bytes_to_gb, get_user_reseller, is_reseller_access_active,
    reseller_stats, remaining_bytes, reserve_volume_for_service,
    refund_unused_volume, drop_inactive_reservation, create_reseller_access_request,
    ensure_reseller_for_user, apply_package, apply_reseller_usage_delta,
    reconcile_reseller_accounting, sync_reseller_service_panel_snapshot,
)
...
from app.services.service_grace import is_tombstone, visible_in_my_services
from app.jobs.service_cleanup import (
    enqueue_service_deletion,
    notify_service_deletion_task_admin,
    record_completed_service_deletion,
)
from app.services.service_deletion_audit import build_service_deletion_snapshot
...
def _activity_error_message(exc: BaseException | None) -> str:
    if exc is None:
        return ''
    text = str(exc).strip()
    if not text:
...
async def _record_reseller_failure_activity(
    *,
    reseller_id: int,
    service_id: int | None,
    server_id: int | None,
    ...
    event_key: str,
    username: str,
    requested_volume_bytes: int,
    previous_volume_bytes: int = 0,
    duration_days: int = 0,
    expires_at: datetime | None = None,
    quota_before_bytes: int | None = None,
    error: BaseException | None = None,
    extra_meta: dict | None = None,
) -> None:
    """Persist a failed reseller operation after the main transaction rolls back.

    Failure logging uses a fresh transaction so a panel error cannot erase its own
    audit record.  The current reseller quota is read again after rollback, which
    makes accidental deductions visible in the admin activity list.
    """
    try:
        async with SessionLocal() as log_session:
            reseller = await log_session.get(ResellerAccount, reseller_id)
            quota_after = int(reseller.total_bytes or 0) if reseller else 0
            meta = {
                'source': 'reseller_bot',
                'status': 'failed',
                'requested_volume_bytes': int(requested_volume_bytes or 0),
                'deducted_bytes': max((int(quota_before_bytes or 0) - quota_after), 0) if quota_before_bytes is not None else 0,
                'quota_before_bytes': int(quota_before_bytes or 0),
                'quota_after_bytes': quota_after,
                'quota_delta_bytes': quota_after - int(quota_before_bytes or 0),
                'error_type': type(error).__name__ if error is not None else 'Error',
                'error_message': _activity_error_message(error),
                'rolled_back': True,
            }
            if extra_meta:
                meta.update(extra_meta)
            log_session.add(ResellerServiceActivity(
                ...
                volume_bytes=int(requested_volume_bytes or 0),
                previous_volume_bytes=int(previous_volume_bytes or 0),
        ...
        logger.exception('Failed to write reseller failure activity action=%s reseller_id=%s service_id=%s', action, reseller_id, service_id)
...
async def send_home(bot, chat_id:int, is_admin: bool=False):
    ...
    await send_single_message(bot, chat_id, text, reply_markup=await main_menu_inline(is_admin))
...
def reseller_owns_visible_service(svc: ClientService | None, reseller: ResellerAccount | None) -> bool:
    return bool(
        svc
        and reseller
        and svc.reseller_id == reseller.id
        and not is_tombstone(svc)
        and visible_in_my_services(svc, datetime.utcnow())
    )
...
def reseller_create_prompt_text(remaining_gb: float) -> str:
    return '👤 لطفاً یوزرنیم اختصاصی مشتری را ارسال کنید.'
...
RESELLER_USERNAME_FORMAT_ERROR_TEXT = '❌ یوزرنیم معتبر نیست. لطفاً فقط حروف انگلیسی، عدد یا آندرلاین ارسال کنید.'

RESELLER_USERNAME_TAKEN_TEXT = '⚠️ یوزرنیم یا اسم اختصاصی‌ای که انتخاب کرده‌اید تکراری است.'
...
async def _next_reseller_duplicate_number() -> int:
    """Allocate an independent sequential suffix for reseller duplicate names.

    The counter is separate from normal/public sales, so reseller alternatives
    are generated as 0001, 0002, ... regardless of duplicate names elsewhere.
    """
    async with SessionLocal() as counter_session:
        dialect_name = counter_session.get_bind().dialect.name
        if dialect_name == 'postgresql':
            stmt = (pg_insert(ResellerUsernameCounter).values(id=1, next_number=2)
                    .on_conflict_do_update(index_elements=[ResellerUsernameCounter.id], set_={'next_number': ResellerUsernameCounter.next_number + 1})
                    .returning(ResellerUsernameCounter.next_number - 1))
            allocated = int((await counter_session.execute(stmt)).scalar_one())
        elif dialect_name == 'sqlite':
            stmt = (sqlite_insert(ResellerUsernameCounter).values(id=1, next_number=2)
                    .on_conflict_do_update(index_elements=[ResellerUsernameCounter.id], set_={'next_number': ResellerUsernameCounter.next_number + 1})
                    .returning(ResellerUsernameCounter.next_number - 1))
            allocated = int((await counter_session.execute(stmt)).scalar_one())
        else:
            counter = await counter_session.get(ResellerUsernameCounter, 1, with_for_update=True)
            if counter is None:
                allocated = 1
                counter_session.add(ResellerUsernameCounter(id=1, next_number=2))
            else:
                allocated = max(int(counter.next_number or 1), 1)
                counter.next_number = allocated + 1
        await counter_session.commit()
        return allocated


async def allocate_reseller_duplicate_username(session, server: Server | None, username: str) -> str:
    """Return the next unique reseller name using a 4+ digit sequential suffix."""
    username = normalize_reseller_username(username)
    base_source = re.sub(r'_\d{4,}$', '', username) or username or 'user'
    for _ in range(500):
        number = await _next_reseller_duplicate_number()
        suffix = str(number).zfill(4)
        max_base = max(1, 32 - len(suffix) - 1)
        base = base_source[:max_base].rstrip('_') or 'user'
        candidate = f'{base}_{suffix}'
        if not await reseller_username_exists(session, server, candidate):
            return candidate
    raise RuntimeError('امکان ساخت نام یکتای جدید برای سرویس نماینده وجود نداشت.')


async def resolve_reseller_username(session, server: Server | None, username: str) -> tuple[str, bool]:
    username = normalize_reseller_username(username)
    if not valid_reseller_username(username):
        raise ValueError(RESELLER_USERNAME_FORMAT_ERROR_TEXT)
    if not await reseller_username_exists(session, server, username):
        return username, False
    return await allocate_reseller_duplicate_username(session, server, username), True


def is_reseller_duplicate_username_error(exc: BaseException) -> bool:
    text = str(exc or '').lower()
    return any(token in text for token in (
        'email already in use', 'email is already in use', 'email already exists',
        'duplicate email', 'username already in use', 'username already exists',
        'duplicate username', 'already used', 'already exists',
    ))
...
def reseller_inbound_mode(reseller: ResellerAccount | None) -> str:
    """Return the per-account inbound policy for reseller-created users."""
    return 'manual' if str(getattr(reseller, 'inbound_mode', '') or '').strip().lower() == 'manual' else 'automatic'


def reseller_manual_inbound_ids(reseller: ResellerAccount | None) -> list[int]:
    return normalize_inbound_ids(getattr(reseller, 'inbound_ids', None) or [])
...
async def resolve_reseller_build_target(session, reseller: ResellerAccount) -> tuple[Server | None, list[int]]:
    ...
    Inbound policy is stored on the reseller account itself. Automatic mode
    follows all active inbounds on the assigned reseller server; manual mode uses
    only the administrator-selected reseller.inbound_ids. Public plan inbound
    settings are never used for reseller-created users.
    """
    ...
    mode = reseller_inbound_mode(reseller)
    if server.server_type != 'xui':
        return server, []
    if mode == 'manual':
        return server, reseller_manual_inbound_ids(reseller)
    # This cached list is used only for readiness/UI checks. The actual create
    # path fetches the live panel list again when automatic=True.
    meta = server.meta or {}
    rows = meta.get('inbounds') or []
    live_cached = normalize_inbound_ids([
        row.get('id') for row in rows
        if isinstance(row, dict) and row.get('enable', row.get('enabled', True)) is not False
    ] or meta.get('inbound_ids') or [])
    return server, live_cached


async def pick_available_reseller_username(session, server: Server, base_username: str) -> str:
    resolved, _ = await resolve_reseller_username(session, server, base_username)
    return resolved
...
async def sync_reseller_service_from_panel(session, svc) -> bool | None:
    ...
    """
    server = await session.get(Server, svc.server_id) if svc and getattr(svc, 'server_id', None) else None
    if not svc:
        return None
    if not server:
        return await sync_reseller_service_panel_snapshot(session, svc)
    if server.server_type == 'mikrotik':
        try:
            found = await MikroTikService().get_user(server, svc.xui_email or svc.client_username)
            if not found:
    ...
    try:
        ...
        await apply_reseller_usage_delta(session, svc, _reseller_effective_used_bytes(svc, int(used or 0)))
        if total:
...
async def sync_all_reseller_services(session, reseller: ResellerAccount, *, commit: bool = True) -> None:
...
async def reseller_dashboard_text(session, reseller: ResellerAccount) -> str:
    await sync_all_reseller_services(session, reseller, commit=False)
    stats = await reseller_stats(session, reseller)
    return (
        '💼 <b>منو نمایندگی D Bot</b>\n'
        '━━━━━━━━━━━━━━━━━━━━━━\n\n'
        '📊 <b>وضعیت لحظه‌ای نمایندگی</b>\n\n'
        f'👥 <b>تعداد یوزرهای فعال:</b> {stats["total_users"]}\n'
        f'📦 <b>Total / حجم قابل ساخت:</b> {bytes_to_gb(stats["total_bytes"])} گیگ\n'
        f'🔒 <b>Reserved / رزرو فعال:</b> {bytes_to_gb(stats["reserved_bytes"])} گیگ\n'
        f'📉 <b>Used / مصرف تاریخی:</b> {bytes_to_gb(stats["used_bytes"])} گیگ\n'
        f'💾 <b>Remaining / باقی‌مانده:</b> {bytes_to_gb(stats["remaining_bytes"])} گیگ\n\n'
...
async def reseller_home(callback: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        button_name = 'reseller_menu' if (callback.from_user.id in settings.admin_ids or (reseller and is_reseller_access_active(reseller))) else 'reseller_request'
        _, button_enabled = await get_user_button(button_name)
        if not button_enabled:
            await edit_or_answer(callback, 'این بخش در حال حاضر توسط مدیریت غیرفعال شده است.', reply_markup=back_main_inline())
...
async def reseller_pkg(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.from_user.id in settings.admin_ids:
        async with SessionLocal() as session:
            ...
            await apply_package(session, req, source='admin_free_package', actor_telegram_id=callback.from_user.id)
            await session.commit()
...
async def reseller_create_start(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear(); await state.update_data(operation_id=uuid.uuid4().hex); await state.set_state(ResellerCreateUser.username)
    async with SessionLocal() as session:
        fresh_reseller = await session.get(ResellerAccount, reseller.id)
        if fresh_reseller:
            await sync_all_reseller_services(session, fresh_reseller, commit=True)
            await session.refresh(fresh_reseller)
            remaining = bytes_to_gb(remaining_bytes(fresh_reseller))
        else:
            remaining = 0
...
async def reseller_create_username(message: Message, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        inbound_mode = reseller_inbound_mode(reseller)
        if not server or (server.server_type == 'xui' and inbound_mode == 'manual' and not inbounds):
            await ui_message(message, 'سرور نمایندگی یا تنظیمات ساخت یوزر آماده نیست. در حالت Manual باید حداقل یک Inbound برای همین نماینده انتخاب شده باشد.', reply_markup=reseller_menu())
            await state.clear()
            return
        ...
        if username_error:
            ...
            return
        username, username_was_changed = await resolve_reseller_username(session, server, username)
    async with SessionLocal() as session:
        _, reseller = await get_user_reseller(session, message.from_user.id)
        if reseller:
            await sync_all_reseller_services(session, reseller, commit=True)
            await session.refresh(reseller)
            max_create_gb = bytes_to_gb(remaining_bytes(reseller))
        else:
            max_create_gb = 0
    await state.update_data(username=username, username_auto_adjusted=username_was_changed, max_create_gb=max_create_gb)
    await state.set_state(ResellerCreateUser.volume)
    auto_note = (f'\n\n⚠️ یوزرنیم یا اسم اختصاصی‌ای که انتخاب کرده‌اید تکراری است.\n' f'✅ نام جدید به‌صورت خودکار انتخاب شد: <code>{username}</code>') if username_was_changed else ''
    await ui_message(
        message,
        f'💾 لطفاً حجم کانفیگ را به گیگ ارسال کنید.\nحداکثر قابل ساخت: {max_create_gb:g} گیگ' + auto_note,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('reseller:create')]])
    )
...
async def reseller_create_volume(message: Message, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        await sync_all_reseller_services(session, reseller, commit=True)
        await session.refresh(reseller)
        available_bytes = remaining_bytes(reseller)
        if available_bytes < gb_to_bytes(volume):
            await ui_message(
                message,
                f'❌ حجم کافی نیست. حداکثر حجمی که الان می‌توانید بسازید: {bytes_to_gb(available_bytes):g} گیگ',
                reply_markup=reseller_menu()
            )
            await state.clear(); return
    await state.update_data(volume=volume)
    await state.set_state(ResellerCreateUser.duration)
    await ui_message(message, '📅 لطفاً مدت اعتبار کانفیگ را به روز ارسال کنید.', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('reseller:create_back_volume')]]))
...
async def reseller_create_back_volume(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    max_create_gb = data.get('max_create_gb')
    if max_create_gb is None:
        async with SessionLocal() as session:
            _, reseller = await get_user_reseller(session, callback.from_user.id)
            if reseller:
                await sync_all_reseller_services(session, reseller, commit=True)
                await session.refresh(reseller)
                max_create_gb = bytes_to_gb(remaining_bytes(reseller))
    ...
    await edit_or_answer(
        callback,
        f'💾 لطفاً حجم کانفیگ را به گیگ ارسال کنید.\nحداکثر قابل ساخت: {float(max_create_gb):g} گیگ',
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('reseller:create')]])
    )
    await callback.answer()
...
async def reseller_create_duration(message: Message, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        if not reseller or not is_reseller_access_active(reseller):
            await ui_message(message, 'دسترسی نمایندگی فعال نیست.', reply_markup=back_main_inline()); await state.clear(); return
        server, inbounds = await resolve_reseller_build_target(session, reseller)
        reseller_mode = reseller_inbound_mode(reseller) if server and server.server_type == 'xui' else 'manual'
        if not server or (server.server_type == 'xui' and reseller_mode == 'manual' and not inbounds):
            await ui_message(message, 'سرور نمایندگی یا تنظیمات ساخت یوزر آماده نیست. در حالت دستی X-UI باید Inbound ID معتبر ثبت شود و برای MikroTik باید Router/API آماده باشد.', reply_markup=reseller_menu()); await state.clear(); return
        ...
        try:
            ...
            if username_error:
                ...
                await ui_message(message, username_error, reply_markup=reseller_back_kb())
                return
            username, _ = await resolve_reseller_username(session, server, username)
            await state.update_data(username=username)
            await sync_all_reseller_services(session, reseller, commit=False)
            reseller = (await session.execute(
                select(ResellerAccount).where(ResellerAccount.id == reseller_id_value).with_for_update()
            )).scalar_one_or_none()
            if not reseller:
                await ui_message(message, 'دسترسی نمایندگی فعال نیست.', reply_markup=back_main_inline()); await state.clear(); return
            # Re-read the per-reseller inbound policy after locking the account so
            # an admin edit made while this flow was open takes effect immediately.
            if server.server_type == 'xui':
                reseller_mode = reseller_inbound_mode(reseller)
                inbounds = reseller_manual_inbound_ids(reseller) if reseller_mode == 'manual' else []
                if reseller_mode == 'manual' and not inbounds:
                    raise RuntimeError('Manual reseller inbound mode has no selected inbound. Edit the reseller and select at least one inbound.')
            await reconcile_reseller_accounting(session, reseller)
            quota_before_bytes = int(reseller.total_bytes or 0)
            available_bytes = remaining_bytes(reseller)
            if available_bytes < volume_bytes:
                await ui_message(
                    message,
                    f'❌ حجم کافی نیست. حداکثر حجمی که الان می‌توانید بسازید: {bytes_to_gb(available_bytes):g} گیگ',
                    reply_markup=reseller_menu(),
            ...
            # The panel can still report a duplicate after our pre-check (for
            # example because another worker created the same name in between).
            # Do not fail the reseller purchase in that case: allocate the next
            # reseller-only 0001/0002/... suffix and retry provisioning.
            duplicate_notice_sent = False
            for duplicate_attempt in range(8):
                try:
                    if server.server_type == 'mikrotik':
                        ...
                        result = await asyncio.wait_for(
                            xui_service.create_client_on_inbounds(
                                server,
                                inbounds if reseller_mode == 'manual' else [],
                                XuiClientPayload(email=username, total_gb=volume, expire_days=days),
                                automatic=(reseller_mode == 'automatic'),
                            ),
                            timeout=60,
                        )
                    break
                except Exception as panel_exc:
                    if not is_reseller_duplicate_username_error(panel_exc) or duplicate_attempt >= 7:
                        raise
                    previous_username = username
                    username = await allocate_reseller_duplicate_username(session, server, previous_username)
                    await state.update_data(username=username, username_auto_adjusted=True)
                    logger.info(
                        'Reseller duplicate username auto-renamed telegram_id=%s old=%s new=%s',
                        message.from_user.id, previous_username, username,
                    )
                    if not duplicate_notice_sent:
                        try:
                            await message.answer(
                                '⚠️ یوزرنیم یا اسم اختصاصی‌ای که انتخاب کرده‌اید تکراری است.\n'
                                f'✅ نام جدید به‌صورت خودکار انتخاب شد: <code>{username}</code>',
                                parse_mode='HTML',
                            )
                        except Exception:
                            pass
            ...
            svc = ClientService(
                ...
                inbound_ids=normalize_inbound_ids((result or {}).get('inbound_ids') or inbounds),
            ...
            # Freeze the reseller's real sellable quota *after* the service row
            # exists and all accounting changes are applied.  The activity card
            # must keep this historical value even if the reseller buys more
            # quota or creates other users later.
            await reconcile_reseller_accounting(session, reseller)
            await session.flush()
            quota_after_bytes = int(remaining_bytes(reseller))
            session.add(ResellerServiceActivity(
                reseller_id=reseller_id_value,
                service_id=svc.id,
                server_id=server_id_value,
                action='create',
                ...
                meta={
                    ...
                    'deducted_bytes': volume_bytes,
                    'added_bytes': 0,
                    'quota_before_bytes': int(quota_before_bytes or 0),
                    'quota_after_bytes': quota_after_bytes,
                    'quota_remaining_after_bytes': quota_after_bytes,
                    'quota_delta_bytes': quota_after_bytes - int(quota_before_bytes or 0),
                    'quota_snapshot_version': 2,
                },
            ...
            await _record_reseller_failure_activity(
                reseller_id=reseller_id_value,
                service_id=None,
                server_id=server_id_value,
                action='create_failed',
                event_key=f'create_failed:{reseller_id_value}:{operation_id}:{datetime.utcnow().strftime("%Y%m%d%H%M%S%f")}',
                username=username,
                requested_volume_bytes=volume_bytes,
                duration_days=days,
                expires_at=datetime.utcnow() + timedelta(days=days),
                quota_before_bytes=quota_before_bytes,
                error=e,
                extra_meta={'operation_id': operation_id, 'failure_stage': 'panel_create_timeout'},
            )
            try:
            ...
            await _record_reseller_failure_activity(
                ...
                quota_before_bytes=quota_before_bytes,
                error=e,
                extra_meta={'operation_id': operation_id, 'failure_stage': 'panel_create'},
            )
            try:
                if server and getattr(server, 'server_type', None) == 'xui':
                    await XuiService().delete_client(server, username)
    ...
    async with SessionLocal() as session:
        ...
        if r_after:
            await reconcile_reseller_accounting(session, r_after)
            remaining_after = bytes_to_gb(remaining_bytes(r_after))
...
async def reseller_users(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        now = datetime.utcnow()
        services = [svc for svc in services if visible_in_my_services(svc, now)]
    if not services:
...
async def reseller_user_detail(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        user, reseller = await get_user_reseller(session, callback.from_user.id)
        if not reseller_owns_visible_service(svc, reseller):
...
async def reseller_add_volume_start(callback: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        await sync_all_reseller_services(session, reseller, commit=True)
        await session.refresh(reseller)
        reseller_remaining = remaining_bytes(reseller)
    ...
    await state.clear()
    await state.update_data(service_id=sid, max_volume_gb=max_gb, operation_id=uuid.uuid4().hex)
...
async def reseller_add_volume_apply(message: Message, state: FSMContext):
    ...
    try:
        async with SessionLocal() as session:
            ...
            await reserve_volume_for_service(session, reseller, add_bytes)
            try:
                if server.server_type == 'xui':
                ...
                server_id_value = int(svc.server_id or server.id)
                username_value = svc.client_username or svc.xui_email or '-'
                await session.rollback()
                await _record_reseller_failure_activity(
                    reseller_id=reseller_id_value,
                    service_id=sid,
                    server_id=server_id_value,
                    action='add_volume_failed',
                    event_key=f'add_volume_failed:{sid}:{operation_id}:{datetime.utcnow().strftime("%Y%m%d%H%M%S%f")}',
                    username=username_value,
                    requested_volume_bytes=add_bytes,
                    previous_volume_bytes=current_total,
                    quota_before_bytes=quota_before_bytes,
                    error=e,
                    extra_meta={'operation_id': operation_id, 'failure_stage': 'panel_add_volume'},
                )
            ...
            await reconcile_reseller_accounting(session, reseller)
            remaining_quota = remaining_bytes(reseller)
            ...
            session.add(ResellerServiceActivity(
                ...
                action='add_volume',
                event_key=f'add_volume:{svc.id}:{operation_id}',
                username=username,
                volume_bytes=add_bytes,
                previous_volume_bytes=current_total,
                duration_days=0,
                expires_at=svc.expires_at,
                meta={
                    ...
                    'status': 'success',
                    'operation_id': operation_id,
                    'added_to_service_bytes': add_bytes,
                    'deducted_bytes': add_bytes,
                    'quota_before_bytes': quota_before_bytes,
                    'quota_after_bytes': int(remaining_quota),
                    'quota_remaining_after_bytes': int(remaining_quota),
                    'quota_delta_bytes': int(remaining_quota) - quota_before_bytes,
                    'quota_snapshot_version': 2,
                    'service_total_before_bytes': current_total,
                    'service_total_after_bytes': new_total,
                },
...
async def reseller_add_days_start(callback: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        expire_text = fa_date(svc.expires_at, empty='-') if svc.expires_at else '-'
    await state.clear()
    await state.update_data(service_id=sid, operation_id=uuid.uuid4().hex)
        ...
        async with SessionLocal() as session:
            ...
            try:
                ...
                await _record_reseller_failure_activity(
                    ...
                    service_id=sid,
                    server_id=server_id_value,
                    action='add_days_failed',
                    event_key=f'add_days_failed:{sid}:{operation_id}:{datetime.utcnow().strftime("%Y%m%d%H%M%S%f")}',
                    username=username_value,
                    requested_volume_bytes=0,
                    duration_days=days,
                    expires_at=new_expires_at,
                    quota_before_bytes=quota_before_bytes,
                    error=e,
                    extra_meta={'operation_id': operation_id, 'failure_stage': 'panel_add_days'},
                )
            ...
            session.add(ResellerServiceActivity(
                ...
                server_id=svc.server_id,
                action='add_days',
                event_key=f'add_days:{svc.id}:{operation_id}',
                username=username,
                volume_bytes=0,
                previous_volume_bytes=int(svc.total_bytes or 0),
                duration_days=days,
                expires_at=new_expires_at,
                meta={
                    'source': 'reseller_bot',
                    ...
                    'quota_after_bytes': quota_after_bytes,
                    'quota_remaining_after_bytes': quota_after_bytes,
                    'quota_delta_bytes': quota_after_bytes - quota_before_bytes,
                    'quota_snapshot_version': 2,
                    'old_expires_at': old_expire.isoformat() if old_expire else None,
                },
                created_at=activity_at,
            ))
            await session.commit()
...
async def reseller_renew_start(callback: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        if not reseller_owns_visible_service(svc, reseller):
            await callback.answer('یوزر پیدا نشد.', show_alert=True)
            return
        # Refresh all reseller services first so Total/Reserved/Remaining and Used display
        # are calculated from the latest traffic, not stale local values.
        await sync_all_reseller_services(session, reseller, commit=False)
        await session.refresh(svc)
        await session.refresh(reseller)
        current_reserved = int(svc.reseller_reserved_bytes or svc.total_bytes or 0)
        current_used = int(svc.used_bytes or 0)
        release_bytes = max(current_reserved - current_used, 0)
        await reconcile_reseller_accounting(session, reseller)
        reseller_remaining = remaining_bytes(reseller)
        max_gb = bytes_to_gb(reseller_remaining + release_bytes)
        current_gb = bytes_to_gb(current_reserved)
        used_gb = bytes_to_gb(current_used)
        release_gb = bytes_to_gb(release_bytes)
    ...
    await edit_or_answer(
        ...
        'در تمدید، رزرو دوره قبلی آزاد می‌شود و حجم دوره جدید دوباره رزرو می‌شود.\n'
        'Used فقط نمایش مصرف است و از ظرفیت باقی‌مانده کم نمی‌شود.\n\n'
        f'حجم دوره فعلی این یوزر: {current_gb:g} گیگ\n'
        f'مصرف‌شده تا الان: {used_gb:g} گیگ\n'
        f'رزرو آزادشونده: {release_gb:g} گیگ\n\n'
        f'سهمیه کل نماینده: {total_gb:g} گیگ\n'
        f'رزروشده از سهمیه: {reserved_gb:g} گیگ\n'
        f'حجم قابل تمدید بعد از آزادسازی رزرو: {max_gb:g} گیگ\n\n'
...
async def reseller_renew_duration(message: Message, state: FSMContext):
    ...
    try:
        async with SessionLocal() as session:
            ...
            renew_event_key = f'renew:{sid}:op:{operation_id}'
            existing_success = (await session.execute(
                select(ResellerServiceActivity).where(ResellerServiceActivity.event_key == renew_event_key)
            )).scalar_one_or_none()
            if existing_success:
                # The same FSM operation may be submitted again if Telegram/UI
                # delivery failed after commit. Never charge reseller quota twice.
                await reconcile_reseller_accounting(session, reseller)
                duplicate_remaining = remaining_bytes(reseller)
                await state.clear()
                await ui_message(
                    message,
                    '✅ این تمدید قبلاً با موفقیت ثبت شده است و هیچ حجم اضافه‌ای دوباره از سهمیه شما کم نشد.\n\n'
                    f'📦 سهمیه فعلی نماینده: {bytes_to_gb(duplicate_remaining):g} گیگ',
                    reply_markup=reseller_back_kb(f'reseller:user:{sid}')
                )
                return

            # Refresh all reseller services before settlement so the quota uses
            # the latest traffic counters before calculating unused return.
            await sync_all_reseller_services(session, reseller, commit=False)
            await session.refresh(svc)
            await session.refresh(reseller)
            ...
            old_expires_at = svc.expires_at
            old_unused_bytes = max(old_reserved_bytes - old_used_bytes, 0)
            await reconcile_reseller_accounting(session, reseller)
            quota_before_bytes = int(reseller.total_bytes or 0)
            reseller_remaining = remaining_bytes(reseller)
            effective_remaining = reseller_remaining + old_unused_bytes
            # Renew is a NEW reseller cycle with settlement of the previous cycle.
            # Only the unused part of the old service returns to the reseller pool;
            # consumed traffic stays spent and only appears in Used statistics.
            if effective_remaining < new_bytes:
                await ui_message(
                    ...
                    f'حجم مصرف‌نشده قابل برگشت از دوره قبلی: {bytes_to_gb(old_unused_bytes):g} گیگ\n'
                    f'حجم قابل تمدید بعد از تسویه دوره قبلی: {bytes_to_gb(effective_remaining):g} گیگ',
                    reply_markup=reseller_back_kb(f'reseller:user:{sid}')
                )
                return
            server = await session.get(Server, svc.server_id)
            ...
            released_bytes = await refund_unused_volume(session, svc, reseller)
            await reserve_volume_for_service(session, reseller, new_bytes)
            renew_baseline_bytes = 0
            try:
                if server.server_type == 'xui':
                    identifiers = [svc.xui_email, svc.client_username, svc.xui_uuid, svc.sub_link]
                    # Renewal re-applies THIS reseller's inbound policy after
                    # enable. Automatic means every live inbound; Manual means the
                    # exact administrator-selected set. This repairs Sanaei's
                    # auto-detach-on-expiry behavior without leaking another
                    # reseller's/server-wide policy into this account.
                    xui_service = XuiService()
                    reseller_mode = reseller_inbound_mode(reseller)
                    reseller_inbounds = reseller_manual_inbound_ids(reseller) if reseller_mode == 'manual' else []
                    if reseller_mode == 'manual' and not reseller_inbounds:
                        raise RuntimeError('Manual reseller inbound mode has no selected inbound. Edit the reseller and select at least one inbound.')
                    reset_info = await xui_service.renew_reseller_client_any(
                        server,
                        identifiers,
                        volume,
                        expire_days,
                        inbound_mode=reseller_mode,
                        inbound_ids=reseller_inbounds,
                    )
                    ...
                    renewed_inbound_ids = normalize_inbound_ids(
                        ((reset_info or {}).get('result') or {}).get('inbound_ids')
                        or (found_after or {}).get('inbound_ids')
                    )
                    if renewed_inbound_ids:
                        svc.inbound_ids = renewed_inbound_ids
                    ...
                    renew_result = await MikroTikService().renew_user(
                        server,
                        svc.xui_email or svc.client_username,
                        volume_gb=volume,
                        expire_days=expire_days,
                        previous_used_bytes=old_used_bytes,
                    )
                    renew_baseline_bytes = int(renew_result.get('_traffic_baseline_bytes') or 0)
            except Exception as e:
                await session.rollback()
                await _record_reseller_failure_activity(
                    reseller_id=reseller_id_value,
                    ...
                    server_id=server_id_value,
                    action='renew_failed',
                    event_key=f'renew_failed:{sid}:{operation_id}:{datetime.utcnow().strftime("%Y%m%d%H%M%S%f")}',
                    username=username_value,
                    requested_volume_bytes=new_bytes,
                    previous_volume_bytes=old_reserved_bytes,
                    ...
                    extra_meta={
                        'operation_id': operation_id,
                        'failure_stage': 'panel_renew',
                        'old_used_bytes': old_used_bytes,
                        'old_unused_bytes': old_unused_bytes,
                        'old_expires_at': old_expires_at.isoformat() if old_expires_at else None,
                        'requested_deduction_bytes': new_bytes,
                    },
                )
            ...
            username = svc.client_username or svc.xui_email or '-'
            activity_at = datetime.utcnow()
            # Renewal first settles the old cycle (unused return), then reserves
            # the new cycle.  Snapshot only after both steps and after the service
            # row contains the new allocation, otherwise the List can show 0 GB
            # or a stale pre-renew value.
            await reconcile_reseller_accounting(session, reseller)
            await session.flush()
            quota_after_bytes = int(remaining_bytes(reseller))
            session.add(ResellerServiceActivity(
                ...
                server_id=svc.server_id,
                action='renew',
                event_key=renew_event_key,
                username=username,
                volume_bytes=new_bytes,
                previous_volume_bytes=old_reserved_bytes,
                duration_days=expire_days,
                expires_at=expires_at,
                meta={
            ...
            remaining_quota = quota_after_bytes
            await session.commit()
    finally:
        pass
    await state.clear()
    ...
    await ui_message(
        ...
        f'↩️ حجم مصرف‌نشده برگشتی از دوره قبلی: {bytes_to_gb(released_bytes):g} گیگ\n'
        f'📦 باقی‌مانده سهمیه نماینده: {bytes_to_gb(remaining_quota):g} گیگ\n'
        '♻️ ترافیک مصرفی دوره جدید ریست شد.\n'
        '📉 حجم تمدید جدید از سهمیه قابل فروش نماینده کم شد.\n'
        '🟢 وضعیت: فعال\n\n'
...
async def reseller_delete_user(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        server = await session.get(Server, svc.server_id)
        try:
            await sync_reseller_service_panel_snapshot(session, svc)
        except Exception:
            pass
        ...
        if server and server.server_type == 'xui':
            try:
                ...
                if 'not found' in msg_l or 'not exist' in msg_l or 'not exists' in msg_l or 'record not found' in msg_l:
                    ...
                    panel_delete_queued = True
        elif server and server.server_type in ('mikrotik', 'openvpn', 'l2tp'):
            try:
        ...
        if panel_delete_queued:
            deletion_task = await enqueue_service_deletion(
                session,
                svc,
                now=datetime.utcnow(),
                reason='reseller_delete',
            ...
            panel_status = (
                'already_missing' if panel_already_missing else
                'deleted' if server and server.server_type in ('xui', 'mikrotik', 'openvpn', 'l2tp') else
                'local_deleted'
            )
            deletion_task = await record_completed_service_deletion(
                session,
                svc,
                now=datetime.utcnow(),
                reason='reseller_delete',
        ...
        await session.commit()
    await notify_service_deletion_task_admin(
        callback.message.bot,
        task_id=deletion_task_id,
        limit=1,
    )
    result_title = (
        '🕓 سرویس از ربات حذف شد و پاک‌سازی پنل در صف تلاش مجدد قرار گرفت'
        if panel_delete_queued else
    ...
    msg = (
        ...
        f'↩️ حجم مصرف‌نشده برگشتی به سهمیه نماینده: {bytes_to_gb(refund):g} گیگ'
        + ('\n🔁 حذف از پنل پس از برقراری اتصال خودکار انجام می‌شود.' if panel_delete_queued else '')
    )
    await edit_or_answer(
        callback,
        msg,
    ...
    if panel_delete_queued:
        await callback.answer('از ربات حذف شد؛ پاک‌سازی پنل در صف قرار گرفت')
    elif panel_already_missing:
        await callback.answer('کانفیگ داخل پنل نبود؛ از ربات حذف شد')
    else:
