...
async def reseller_create_duration(message: Message, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        if not reseller or not is_reseller_access_active(reseller):
            await ui_message(message, 'دسترسی نمایندگی فعال نیست.', reply_markup=back_main_inline()); await state.clear(); return
        server, inbounds = await resolve_reseller_build_target(session, reseller)
        reseller_inbound_mode = XuiService().server_inbound_mode(server) if server and server.server_type == 'xui' else 'manual'
        if not server or (server.server_type == 'xui' and reseller_inbound_mode == 'manual' and not inbounds):
            ...
            if server.server_type == 'mikrotik':
                ...
                result = await asyncio.wait_for(
                    xui_service.create_client_on_inbounds(
                        server,
                        inbounds if reseller_inbound_mode == 'manual' else [],
                        XuiClientPayload(email=username, total_gb=volume, expire_days=days),
                        automatic=(reseller_inbound_mode == 'automatic'),
                    ),
                    timeout=60,
                )
...
async def reseller_renew_start(callback: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        release_bytes = current_reserved
        await reconcile_reseller_accounting(session, reseller)
        reseller_remaining = remaining_bytes(reseller)
        ...
        async with SessionLocal() as session:
            ...
            try:
                if server.server_type == 'xui':
                    ...
                    # Reseller renewal is additive-only for inbound membership:
                    # preserve everything already attached/stored and attach all
                    # currently configured reseller-server inbounds. Never
                    # detach an older inbound just because settings changed.
                    current_service_inbounds = normalize_inbound_ids(svc.inbound_ids or [])
                    xui_service = XuiService()
                    reseller_inbound_mode = xui_service.server_inbound_mode(server)
                    configured_reseller_inbounds = normalize_inbound_ids(
                        (server.meta or {}).get('inbound_ids') or []
                    )
                    reset_info = await xui_service.reset_client_plan_any(
                        server,
                        identifiers,
                        volume,
                        expire_days,
                        inbound_ids=configured_reseller_inbounds,
                        inbound_mode=reseller_inbound_mode,
                        current_inbound_ids=current_service_inbounds,
                    )
                    ...
                    renewed_inbound_ids = normalize_inbound_ids(
                        ...
                        or [*current_service_inbounds, *configured_reseller_inbounds]
                    )
            ...
            session.add(ResellerServiceActivity(
                ...
                action='renew',
                event_key=f"renew:{svc.id}:{activity_at.strftime('%Y%m%d%H%M%S%f')}",
                username=username,
