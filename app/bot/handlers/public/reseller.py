...
async def reseller_user_detail(callback: CallbackQuery):
    ...
    online_text = '✅ بله' if online else '❌ خیر'
...
async def reseller_renew_back_volume(callback: CallbackQuery, state: FSMContext):
    ...
    if max_gb is not None:
        text += f'سقف حجم قابل تنظیم برای این یوزر: {float(max_gb):g} گیگ\n\n'
    text += 'چند گیگ برای این یوزر ست شود؟\nمثال: 100'
    await edit_or_answer(
        callback,
        text,
