...
from app.database.models import User, Server, Plan, PaymentCard, ResellerAccount, ResellerPackage, ResellerTopupRequest, ResellerAccessRequest, ClientService, ResellerServiceActivity, DiscountCode, DiscountUsage, ServiceUsernameCounter
from app.bot.keyboards.common import CB_RESELLER, back_button, back_main_inline, main_menu_inline, get_user_button
...
RESELLER_USERNAME_TAKEN_TEXT = '❌ این یوزرنیم قبلاً ثبت شده است. لطفاً یک یوزرنیم اختصاصی دیگر ارسال کنید.'
...
async def _next_reseller_duplicate_number() -> int:
    async with SessionLocal() as counter_session:
        ...
        if dialect_name == 'postgresql':
            stmt = (pg_insert(ServiceUsernameCounter).values(id=1, next_number=2)
                    .on_conflict_do_update(index_elements=[ServiceUsernameCounter.id], set_={'next_number': ServiceUsernameCounter.next_number + 1})
                    .returning(ServiceUsernameCounter.next_number - 1))
            allocated = int((await counter_session.execute(stmt)).scalar_one())
        elif dialect_name == 'sqlite':
            stmt = (sqlite_insert(ServiceUsernameCounter).values(id=1, next_number=2)
                    .on_conflict_do_update(index_elements=[ServiceUsernameCounter.id], set_={'next_number': ServiceUsernameCounter.next_number + 1})
                    .returning(ServiceUsernameCounter.next_number - 1))
            allocated = int((await counter_session.execute(stmt)).scalar_one())
        else:
            counter = await counter_session.get(ServiceUsernameCounter, 1, with_for_update=True)
            if counter is None:
                allocated = 1
                counter_session.add(ServiceUsernameCounter(id=1, next_number=2))
            else:
...
async def resolve_reseller_username(session, server: Server | None, username: str) -> tuple[str, bool]:
    ...
    base = username[:25].rstrip('_') or 'user'
    for _ in range(250):
        number = await _next_reseller_duplicate_number()
        candidate = f'{base}_{number:06d}'
        if not await reseller_username_exists(session, server, candidate):
            return candidate, True
    raise RuntimeError('Could not allocate a unique reseller username')
...
async def reseller_create_username(message: Message, state: FSMContext):
    ...
    auto_note = f'\n\nℹ️ نام تکراری بود و <code>{username}</code> به‌صورت خودکار جایگزین شد.' if username_was_changed else ''
    await ui_message(
        message,
