    ...
    async with SessionLocal() as session:
        ...
        try:
            ...
            svc = ClientService(
                ...
                xui_email=username,
                xui_uuid=(str(result.get('uuid')) if result.get('uuid') is not None else None),
...
async def reseller_users(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        services = (await session.execute(select(ClientService).where(ClientService.reseller_id == reseller.id, ClientService.is_active == True).order_by(ClientService.id.desc()))).scalars().all()
    if not services:
