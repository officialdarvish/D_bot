...
from app.database.models import User, ClientService, PaymentCard, Order
...
async def account(callback: CallbackQuery):
    ...
    kb=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=str(user.telegram_id), callback_data='noop'), InlineKeyboardButton(text='آیدی عددی 🆔:', callback_data='noop')],
        [InlineKeyboardButton(text=f'{(user.wallet_v2ray_balance or 0):,} تومان', callback_data='noop'), InlineKeyboardButton(text='کیف پول V2Ray 💎:', callback_data='noop')],
        [InlineKeyboardButton(text=f'{(user.wallet_openvpn_balance or 0):,} تومان', callback_data='noop'), InlineKeyboardButton(text='کیف پول OpenVPN 🌐:', callback_data='noop')],
        ...
        [InlineKeyboardButton(text=BTN_WALLET_TOPUP, callback_data='wallet:topup')],
        [back_button('back:main')]
    ])
...
async def wallet_topup(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    kb=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='شارژ کیف پول V2Ray', callback_data='wallet:type:xui')],
        [InlineKeyboardButton(text='شارژ کیف پول OpenVPN - L2TP', callback_data='wallet:type:openvpn')],
        [back_button('menu:account')]
    ])
...
async def wallet_receipt(message: Message, state: FSMContext):
    ...
    await state.clear(); await ui_message(message, '✅ رسید شارژ کیف پول ارسال شد. بعد از تایید مدیر، موجودی شما افزایش پیدا می‌کند.'); await send_home(message.bot, message.from_user.id, message.from_user.id in settings.admin_ids)
...
async def wallet_topup_approve(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        order=await session.get(Order, oid); user=await session.get(User, order.user_id)
