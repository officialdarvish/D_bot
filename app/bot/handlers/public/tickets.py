...
from app.bot.keyboards.common import CB_TICKETS, back_main_inline, main_menu_inline, back_button
...
async def send_home(bot, chat_id:int, is_admin=False):
    await bot.send_message(chat_id, await get_setting_value('welcome_text', WELCOME_TEXT_DEFAULT), reply_markup=await main_menu_inline(is_admin))
...
async def ticket_new(callback: CallbackQuery, state: FSMContext):
    ...
    await edit_or_answer(callback, '📝 موضوع تیکت را ارسال کنید:', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('menu:tickets')]]))
...
async def ticket_subject(message: Message, state: FSMContext):
    ...
    await ui_message(message, 'متن تیکت را ارسال کنید:', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('ticket:back_subject')]]))
