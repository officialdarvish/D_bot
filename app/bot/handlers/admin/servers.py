...
from app.database.models import Server, Plan, ServerCategory, ClientService, PaymentCard, Order, TestAccountUsage
...
def router_connection_text(server: Server | None) -> str:
...
async def refresh_server(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if s.server_type == 'mikrotik':
            ...
            await session.commit()
            connection = router_connection_text(s)
            ...
            if not ok:
                await edit_or_answer(
                    ...
                    reply_markup=await servers_keyboard(),
                )
                await callback.answer('اتصال موقتاً ناموفق بود.', show_alert=True)
                return
            await edit_or_answer(
                callback,
                f'✅ وضعیت MikroTik بروزرسانی شد.\n\nوضعیت در D Bot: {admin_state}\nوضعیت Router: {connection}\n\nآنلاین/آفلاین شدن Router وضعیت فعال سرور را تغییر نمی‌دهد.',
                reply_markup=await servers_keyboard(),
            )
...
async def delete_server_ask(callback: CallbackQuery):
    ...
    await edit_or_answer(callback, '⚠️ مطمئنی می‌خواهی این سرور حذف شود؟\nپلن‌ها و کارت‌های متصل حذف/غیرفعال می‌شوند، اما یوزرهای ساخته‌شده از دیتابیس حذف نمی‌شوند و فقط از سرور جدا می‌شوند.', reply_markup=kb); await callback.answer()
...
async def delete_server(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if is_public_server(s):
            ...
            await session.execute(update(ClientService).where(ClientService.server_id == sid).values(server_id=None))
            await session.execute(update(Plan).where(Plan.server_id == sid).values(server_id=None, is_active=False))
            await session.execute(update(ServerCategory).where(ServerCategory.server_id == sid).values(server_id=None))
            await session.delete(s); await session.commit()
    await edit_or_answer(callback, '✅ سرور حذف شد، اما یوزرهای ساخته‌شده در دیتابیس نگهداری شدند.\n\n📋 لیست سرورها:', reply_markup=await servers_keyboard()); await callback.answer()
