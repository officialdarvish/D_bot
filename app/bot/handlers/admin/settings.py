...
from aiogram.types import Message, CallbackQuery, InlineKeyboardMarkup, InlineKeyboardButton, FSInputFile, ChatMemberUpdated
from aiogram.fsm.context import FSMContext
from sqlalchemy import select, func, delete
from app.core.config import settings
from app.core.roles import is_owner
from app.database.session import SessionLocal
from app.database.models import User, Setting, PaymentCard, ClientService, Server, ServerCategory, Plan, Order, Ticket, TicketMessage, WalletTransaction, TestAccountUsage, DiscountCode, ResellerAccount
from app.bot.states.admin_states import TestAccountConfig, PaymentCardConfig, PaymentTextConfig, RulesTextConfig, BroadcastFlow, RestoreBackup, ServiceTypeConfig, AddDiscountCode, EditDiscountCode, WebsiteSettings, WebsiteCommandSetup, ReferralSettingsConfig, UserInfoLookup, PrivateMessageFlow
from app.bot.keyboards.common import BTN_USERS, BTN_PAYMENT_CHANNEL, BTN_RULES, BTN_BUTTONS, BTN_BROADCAST, BTN_PRIVATE_MESSAGE, BTN_DISCOUNTS, CB_USERS, CB_PAYMENT_CHANNEL, CB_RULES, CB_BUTTONS, CB_BROADCAST, CB_PRIVATE_MESSAGE, CB_DISCOUNTS, CB_BACKUP, CB_RESTORE, back_button, back_admin_inline, main_menu_inline, private_message_user_actions
from app.database.defaults import get_setting_value, set_setting_value, WELCOME_TEXT_DEFAULT, RULES_TEXT_DEFAULT
from app.bot.utils import edit_or_answer, ui_message, ui_callback_message, ui_page, delete_active_ui_message, remember_ui_message
from app.bot.error_reporting import handle_user_facing_error, report_bot_error
from app.services.xui_service import XuiService
from app.services.web_credentials import save_web_credentials, validate_web_admin_username

router = Router()

@router.my_chat_member()
async def remember_backup_target(event: ChatMemberUpdated):
    """Remember the real numeric ID when this bot is promoted in a channel/group.

    A private invite link is never a valid Bot API chat ID. Telegram does expose
    the real ID in my_chat_member updates, so the web backup page can use it
    automatically after the bot is made administrator.
    """
    chat_type = str(getattr(event.chat.type, 'value', event.chat.type))
    if chat_type not in {'channel', 'group', 'supergroup'}:
        return
    new_status = str(getattr(event.new_chat_member.status, 'value', event.new_chat_member.status))
    if new_status not in {'administrator', 'creator'}:
        return
    detected_id = str(event.chat.id)
    detected_title = str(event.chat.title or '')
    destination = 'channel' if chat_type == 'channel' else 'group'
    current_target = await get_setting_value('backup_chat_id', '')
    current_destination = await get_setting_value('backup_destination', destination)
    values = {
        'backup_last_detected_chat_id': detected_id,
        'backup_last_detected_chat_type': chat_type,
        'backup_last_detected_chat_title': detected_title,
        'backup_last_detected_at': datetime.utcnow().isoformat(),
    }
    # Replace empty/invite-link configuration automatically. Do not overwrite a
    # different explicit numeric/public target merely because the bot is admin
    # in another channel too.
    from app.services.telegram_target import is_private_invite_link
    if (not current_target or is_private_invite_link(current_target)) and current_destination == destination:
        values['backup_chat_id'] = detected_id
        values['backup_admin_ok'] = '1'
        values['backup_last_test_message'] = f'Private {destination} detected automatically: {detected_title or detected_id}'
    await save_web_settings_bulk(values)
...
@router.channel_post()
async def detect_existing_private_backup_channel(message: Message):
    """Resolve an already-configured private invite from a new channel post.

    This is useful when the bot had been made administrator before this version
    was installed, so no fresh my_chat_member update is available. The handler
    stores the numeric ID silently and never edits or deletes the channel post.
    """
    if str(getattr(message.chat.type, 'value', message.chat.type)) != 'channel':
        return
    current_destination = await get_setting_value('backup_destination', 'channel')
    current_target = await get_setting_value('backup_chat_id', '')
    from app.services.telegram_target import is_private_invite_link
    if current_destination != 'channel' or (current_target and not is_private_invite_link(current_target)):
        return
    detected_id = str(message.chat.id)
    detected_title = str(message.chat.title or '')
    await save_web_settings_bulk({
        'backup_chat_id': detected_id,
        'backup_last_detected_chat_id': detected_id,
        'backup_last_detected_chat_type': 'channel',
        'backup_last_detected_chat_title': detected_title,
        'backup_last_detected_at': datetime.utcnow().isoformat(),
        'backup_admin_ok': '1',
        'backup_last_test_message': f'Private channel detected automatically: {detected_title or detected_id}',
    })
...
async def payment_settings(callback: CallbackQuery):
    ...
    payment_text=await get_setting_value('payment_text', 'Not set')
    info='💳 تنظیمات درگاه و Channel\n━━━━━━━━━━━━━━━━\n\n'
    info += f'📢 Channel اجباری: {ch or "Not set"}\n'
    info += f'👁 وضعیت عضویت اجباری: {"On" if channel_required == "1" else "Off"}\n\n'
    info += f'📝 متن پرداخت: {payment_text[:120]}\n\n'
    if cards:
        info += '💳 کارت‌های ثبت‌شده:\n'
        for c in cards:
            status='🟢 Enabled' if c.is_active else '🔴 Disabled'
        ...
        info += 'هنوز شماره کارتی Not set است.'
    rows=[
        ...
        [InlineKeyboardButton(text='📢 تغییر Channel اجباری', callback_data='channel:set'), InlineKeyboardButton(text='👁 On/Off عضویت', callback_data='channel:toggle_required')],
...
async def paycard_owner(message: Message, state: FSMContext):
    ...
    if not servers:
        await ui_message(message, 'هیچ سروری Not set است.', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:payment_channel')]])); await state.clear(); return
...
async def set_channel(message: Message):
    ...
    await ui_message(message, '✅ Channel ذخیره شد.', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:user_interaction')]]))
...
async def paycard_detail(callback: CallbackQuery):
    ...
    kb=InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text='👁 Enabled/Disabled', callback_data=f'paycard:toggle:{cid}')],
...
def _private_message_confirm_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text='✅ ارسال پیام', callback_data='private_message:send'),
            InlineKeyboardButton(text='✏️ تغییر پیام', callback_data='private_message:edit_message'),
        ],
        [InlineKeyboardButton(text='🔢 تغییر آیدی عددی', callback_data='private_message:edit_target')],
        [back_button('admin:user_interaction')],
    ])


async def _private_target_label(telegram_id: int) -> str:
    async with SessionLocal() as session:
    ...
    return f'{name} | {username} | {telegram_id}'


@router.message(F.text == BTN_PRIVATE_MESSAGE)
@router.callback_query(F.data == CB_PRIVATE_MESSAGE)
async def private_message_start(event, state: FSMContext):
    if not admin(event.from_user.id): return
    await state.clear()
    await state.set_state(PrivateMessageFlow.telegram_id)
    text = (
        '✉️ ارسال پیام خصوصی\n\n'
        'آیدی عددی تلگرام کاربر را وارد کنید.\n'
        'برای ارسال پیام، وجود یوزرنیم لازم نیست.\n\n'
        'مثال: 123456789'
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:user_interaction')]])
    if isinstance(event, CallbackQuery):
        await edit_or_answer(event, text, reply_markup=kb)
        await event.answer()
    else:
        await ui_message(event, text, reply_markup=kb)


@router.message(PrivateMessageFlow.telegram_id)
async def private_message_target(message: Message, state: FSMContext):
    if not admin(message.from_user.id): return
    raw = (message.text or '').strip()
    if not raw.isdigit() or int(raw) <= 0:
        await ui_message(
            message,
            '❌ آیدی نامعتبر است. فقط آیدی عددی مثبت تلگرام را وارد کنید.\nمثال: 123456789',
    ...
    await state.set_state(PrivateMessageFlow.message)
    label = await _private_target_label(telegram_id)
    await ui_message(
        message,
        f'✅ گیرنده: {label}\n\nحالا پیامی که می‌خواهید برای کاربر ارسال شود را بفرستید.\nمتن، عکس، ویدیو، فایل و سایر پیام‌های قابل Copy در تلگرام پشتیبانی می‌شوند.',
...
@router.message(PrivateMessageFlow.message)
async def private_message_capture(message: Message, state: FSMContext):
    if not admin(message.from_user.id): return
    data = await state.get_data()
    telegram_id = data.get('target_telegram_id')
    if not telegram_id:
        await state.set_state(PrivateMessageFlow.telegram_id)
        await ui_message(message, '❌ آیدی گیرنده پیدا نشد. دوباره آیدی عددی کاربر را وارد کنید.')
        return
    await state.update_data(source_chat_id=message.chat.id, source_message_id=message.message_id)
    label = await _private_target_label(int(telegram_id))
    preview = ''
    if message.text:
        compact = message.text.strip()
        if len(compact) > 500:
            compact = compact[:500] + '…'
        preview = f'\n\n📝 متن:\n{compact}'
    elif message.caption:
        compact = message.caption.strip()
        if len(compact) > 500:
            compact = compact[:500] + '…'
        preview = f'\n\n📝 کپشن:\n{compact}'
    else:
        preview = '\n\n📎 پیام غیرمتنی ثبت شد.'
    # Keep the admin's source message intact until confirmation because the
    # delivery step uses Telegram copy_message with this exact message_id.
    # The confirmation itself MUST become the active UI message. Otherwise
    # FreshCallbackOnlyMiddleware treats its inline buttons as stale and drops
    # private_message:send before this module ever receives the callback.
    await delete_active_ui_message(message.bot, message.chat.id, exclude_message_id=message.message_id)
    confirmation = await message.answer(
        f'✉️ پیام خصوصی آماده ارسال است.\n\n👤 گیرنده: {label}{preview}\n\nارسال شود؟',
        reply_markup=_private_message_confirm_kb(),
    )
    remember_ui_message(confirmation.chat.id, confirmation.message_id)


@router.callback_query(F.data == 'private_message:edit_target')
async def private_message_edit_target(callback: CallbackQuery, state: FSMContext):
    if not admin(callback.from_user.id): return
    await state.set_state(PrivateMessageFlow.telegram_id)
    await edit_or_answer(
        callback,
        '🔢 آیدی عددی جدید کاربر را وارد کنید:',
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:user_interaction')]]),
    )
    await callback.answer()


@router.callback_query(F.data == 'private_message:edit_message')
async def private_message_edit_message(callback: CallbackQuery, state: FSMContext):
    if not admin(callback.from_user.id): return
    data = await state.get_data()
    if not data.get('target_telegram_id'):
        await state.set_state(PrivateMessageFlow.telegram_id)
        await edit_or_answer(callback, '🔢 ابتدا آیدی عددی کاربر را وارد کنید:')
    else:
        await state.set_state(PrivateMessageFlow.message)
        await edit_or_answer(
            callback,
...
@router.callback_query(F.data.startswith('private_admin:reply:'))
async def private_message_quick_reply(callback: CallbackQuery, state: FSMContext):
    if not admin(callback.from_user.id):
        return
    try:
        telegram_id = int((callback.data or '').rsplit(':', 1)[-1])
    except (TypeError, ValueError):
        await callback.answer('آیدی کاربر نامعتبر است.', show_alert=True)
        return
    if telegram_id <= 0:
        await callback.answer('آیدی کاربر نامعتبر است.', show_alert=True)
        return
    ...
    # The reply notification is proactive and may not be the owner's active UI
    # message. Create a fresh active prompt instead of editing the notification.
    await delete_active_ui_message(callback.bot, callback.from_user.id)
    prompt = await callback.bot.send_message(
        callback.from_user.id,
        f'✍️ پاسخ خصوصی برای کاربر:\n{label}\n\nپیام جدید را ارسال کنید:',
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text='🔢 تغییر آیدی عددی', callback_data='private_message:edit_target')],
            [back_button('admin:user_interaction')],
        ]),
    )
    remember_ui_message(prompt.chat.id, prompt.message_id)
    await callback.answer('پیام پاسخ را ارسال کنید')


@router.callback_query(F.data == 'private_message:send')
async def private_message_send(callback: CallbackQuery, state: FSMContext):
    if not admin(callback.from_user.id): return
    data = await state.get_data()
    telegram_id = data.get('target_telegram_id')
    source_chat_id = data.get('source_chat_id')
    source_message_id = data.get('source_message_id')
    if not telegram_id or not source_chat_id or not source_message_id:
        await callback.answer('اطلاعات پیام کامل نیست. دوباره تلاش کنید.', show_alert=True)
        return
    try:
        await callback.bot.copy_message(
            ...
            reply_markup=private_message_user_actions(callback.from_user.id),
        )
    except Exception as exc:
        await report_bot_error(
            callback.bot,
            exc,
            context=f'Admin private message delivery failed target_id={telegram_id}',
            event=callback,
        )
        err_text = str(exc).strip()
        if len(err_text) > 350:
            err_text = err_text[:350] + '…'
        await edit_or_answer(
            callback,
            '❌ ارسال پیام ناموفق بود.\n\n'
            'اگر کاربر ربات را Start نکرده باشد، ربات را Block کرده باشد یا Chat ID معتبر نباشد، تلگرام اجازه ارسال نمی‌دهد.\n\n'
            f'خطا: {err_text or type(exc).__name__}',
            reply_markup=_private_message_confirm_kb(),
        )
    ...
    await edit_or_answer(
        callback,
        f'✅ پیام خصوصی با موفقیت ارسال شد.\n\n👤 گیرنده: {label}',
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:user_interaction')]]),
    )
...
async def user_info_hint(event, state: FSMContext):
    if not admin(event.from_user.id): return
    await state.clear()
    await state.set_state(UserInfoLookup.telegram_id)
    target = event.message if isinstance(event, CallbackQuery) else event
    await ui_page(
        target,
        '👥 اطلاعات کاربران\n\nفقط آیدی عددی تلگرام کاربر را وارد کنید تا اطلاعات کامل نمایش داده شود.\nمثال: 123456789',
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:user_interaction')]])
    )
    if isinstance(event, CallbackQuery): await event.answer()


async def _send_user_info_by_numeric_id(message: Message, telegram_id: int, state: FSMContext | None = None):
    async with SessionLocal() as session:
        user = (await session.execute(select(User).where(User.telegram_id == telegram_id))).scalar_one_or_none()
        if not user:
            await ui_message(message, '❌ کاربری با این آیدی عددی پیدا نشد.', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:user_interaction')]]))
            return
        active = (await session.execute(select(func.count(ClientService.id)).where(ClientService.user_id == user.id, ClientService.is_active == True))).scalar() or 0
        total_services = (await session.execute(select(func.count(ClientService.id)).where(ClientService.user_id == user.id))).scalar() or 0
    ...
    reseller_text = 'ندارد'
    if reseller:
        reseller_text = 'فعال' if reseller.is_active else 'غیرفعال'
        if reseller.expires_at:
            reseller_text += f' | انقضا: {fa_date(reseller.expires_at)}'
    ...
    text = (
        ...
        f'✅ قوانین: {"پذیرفته" if user.accepted_rules else "نپذیرفته"}\n'
        f'📅 عضویت: {joined}\n\n'
        '💰 کیف پول‌ها\n'
        f'▫️ اصلی: {int(user.wallet_balance or 0):,} تومان\n'
        f'▫️ V2Ray: {int(user.wallet_v2ray_balance or 0):,} تومان\n'
        f'▫️ OpenVPN: {int(user.wallet_openvpn_balance or 0):,} تومان\n\n'
        '📦 سرویس‌ها و خریدها\n'
        f'▫️ سرویس فعال: {active}\n'
        f'▫️ کل سرویس‌ها: {total_services}\n'
        f'▫️ تعداد سفارش‌ها: {orders_count}\n'
...
async def user_info_by_state(message: Message, state: FSMContext):
    if not admin(message.from_user.id): return
    raw = (message.text or '').strip()
    if not raw.isdigit():
        await ui_message(message, '❌ فقط آیدی عددی تلگرام کاربر را وارد کنید. جستجو با یوزرنیم غیرفعال شد.')
        return
    await _send_user_info_by_numeric_id(message, int(raw), state)


@router.message(F.text.startswith('user '))
async def user_info(message: Message):
    if not admin(message.from_user.id): return
    key = message.text.replace('user ', '').strip()
    if not key.isdigit():
        await ui_message(message, '❌ اطلاعات کاربران فقط با آیدی عددی قابل جستجو است. مثال:\nuser 123456789', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:user_interaction')]]))
        return
...
def buttons_kb(bot_enabled='1', test_enabled='1', test_visible='1'):
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text=f'🤖 ربات: {"On" if bot_enabled == "1" else "Off"}', callback_data='buttons:toggle_bot'),
            InlineKeyboardButton(text=f'🎁 اکانت تست: {"On" if test_enabled == "1" else "Off"}', callback_data='buttons:toggle_test'),
        ],
        [
...
async def buttons_menu(event):
    ...
    text = '🔘 مدیریت دکمه‌ها و اکانت تست\n\nاز این بخش می‌توانید اکانت تست را On/Off کنید و مشخص کنید از کدام سرور و Inbound ساخته شود.'
    if isinstance(event, CallbackQuery):
...
async def service_types_text() -> str:
    lines = [
        ...
        '━━━━━━━━━━━━━━━━',
        '',
        'از این بخش می‌توانید گزینه‌های نوع سرویس در خرید را On/Off کنید یا متن دکمه‌ها را تغییر دهید.',
        '',
    ]
    for key, item in SERVICE_TYPE_ITEMS.items():
        ...
        lines.append(f'• {"🟢 Enabled" if enabled == "1" else "🔴 Disabled"} | {item["title"]}')
...
async def service_type_detail_text(kind: str) -> str:
    ...
    return (
        ...
        f'👁 وضعیت نمایش در خرید: {"🟢 Enabled" if enabled == "1" else "🔴 Disabled"}\n\n'
        f'با Disabled کردن این گزینه، کاربر دیگر هنگام خرید این نوع سرویس را نمی‌بیند.'
    )

def service_type_detail_kb(kind: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='👁 Enabled / Disabled', callback_data=f'stype:toggle:{kind}')],
...
async def test_config_text() -> str:
    ...
    return (
        ...
        '━━━━━━━━━━━━━━━━\n\n'
        f'🖥 سرور: {server_name}\n'
        f'🔢 Inbound ID ها: {inbounds or "Not set"}\n'
        f'💾 حجم: {volume} گیگ\n'
        f'📅 مدت: {duration} روز'
    )
...
async def test_config_server_start(callback: CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        servers = [s for s in all_servers if (s.meta or {}).get('scope') != 'reseller']
    if not servers:
        await callback.answer('هیچ سرور X-UI Enabledی Not set است.', show_alert=True); return
...
async def finish_home_later(bot, chat_id: int, delay: int = 3):
    ...
    await bot.send_message(chat_id, await get_setting_value('welcome_text', WELCOME_TEXT_DEFAULT), reply_markup=await main_menu_inline(True))
...
async def run_broadcast(bot, admin_chat_id: int, text: str):
    ...
    await bot.send_message(admin_chat_id, await get_setting_value('welcome_text', WELCOME_TEXT_DEFAULT), reply_markup=await main_menu_inline(True))
...
async def broadcast_send(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    if not text:
        await callback.answer('متنی Not set است.', show_alert=True); return
...
async def channel_save_state(message: Message, state: FSMContext):
    ...
    await set_value('channel_url', message.text.strip().replace(' ', ''))
    await state.clear()
    await ui_message(message, '✅ Channel اجباری ذخیره شد.', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:user_interaction')]]))
...
async def backup_send(callback: CallbackQuery):
    ...
    path = ''
    try:
        # Use the same portable v4 exporter as the web panel. This keeps every
        # database table and every encrypted credential cross-installation safe.
        from app.api.admin_web import _write_backup_file

        path = await _write_backup_file()
        await callback.message.answer_document(
            FSInputFile(path, filename='dbot_portable_backup_v4.json'),
            caption=(
                '📦 بک‌آپ پرتابل D BOT آماده شد.\n\n'
                '✅ قابل ری‌استور روی VPS و نصب‌های دیگر\n'
                '✅ شامل اطلاعات کامل دیتابیس و Credentials Center'
            ),
        )
        await ui_callback_message(
            callback,
            await get_setting_value('welcome_text', WELCOME_TEXT_DEFAULT),
            reply_markup=await main_menu_inline(True),
        )
        await callback.answer('بک‌آپ پرتابل ساخته شد')
    except Exception as exc:
        await handle_user_facing_error(
            callback.message,
            exc,
            context='Admin portable backup failed',
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]),
        )
        await callback.answer('ساخت بک‌آپ ناموفق بود', show_alert=True)
    finally:
        if path:
            try:
                os.unlink(path)
...
async def _delete_restore_temp_later(path: str, delay_seconds: int = 900) -> None:
...
async def restore_backup_start(callback: CallbackQuery, state: FSMContext):
    ...
    previous_path = str(previous.get('legacy_backup_path') or '')
    if previous_path:
        try:
    ...
    await edit_or_answer(
        callback,
        '♻️ فایل بک‌آپ پرتابل JSON را ارسال کنید.\n\n'
        '✅ فایل را می‌توانید از VPS یا نصب دیگری گرفته باشید.\n'
        '⚠️ اطلاعات فعلی دیتابیس مقصد با اطلاعات بک‌آپ جایگزین می‌شود.',
        reply_markup=kb,
    )
    await callback.answer()
...
async def restore_backup_file(message: Message, state: FSMContext):
    ...
    if not (doc.file_name or '').lower().endswith('.json'):
        await ui_message(
            message,
            '❌ لطفاً فایل بک‌آپ با فرمت JSON ارسال کنید.',
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:user_interaction')]]),
        )
        return
    ...
    try:
        ...
        await message.bot.download_file(telegram_file.file_path, tmp.name)
        with open(tmp.name, 'r', encoding='utf-8') as fh:
            data = json.load(fh)

        # Use the web panel's shared validator/importer so Telegram and web
        # restore have exactly the same trust, checksum and portability rules.
        from app.api.admin_web import _restore_backup_payload, LegacyBackupSecretRequired

        try:
            restored = await _restore_backup_payload(data)
        except LegacyBackupSecretRequired as exc:
            keep_tmp = True
            await state.update_data(legacy_backup_path=tmp.name)
            await state.set_state(RestoreBackup.legacy_secret)
            asyncio.create_task(_delete_restore_temp_later(tmp.name))
            await ui_message(
                message,
                '🔐 این بک‌آپ از نوع قدیمی است و رمز سرورها با کلید VPS مبدا ذخیره شده.\n\n'
                'کلید قدیمی را در پیام بعدی ارسال کنید:\n'
                '• FERNET_KEY نمایش‌داده‌شده در دستور dbot credentials روی VPS قبلی\n'
                '• یا اگر FERNET_KEY خالی بوده، توکن قدیمی ربات تلگرام\n\n'
                'این مقدار فقط برای همین Restore استفاده می‌شود و ذخیره نخواهد شد.\n'
                f'جزئیات: {exc}',
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]),
            )
            return
        ...
        await ui_message(
            message,
            '✅ بک‌آپ پرتابل با موفقیت ری‌استور و سینک شد.\n\n'
            f'📊 تعداد رکوردهای بازیابی‌شده: {total}\n'
            '🔐 نام کاربری و رمز وب نیز در Credentials Center به‌روز شد.',
            reply_markup=await main_menu_inline(True),
        )
    except Exception as exc:
        await state.clear()
        await handle_user_facing_error(
            message,
            exc,
            context='Admin portable restore failed',
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]),
        )
    finally:
        if not keep_tmp:
...
@router.message(RestoreBackup.legacy_secret, F.text)
async def restore_backup_legacy_secret(message: Message, state: FSMContext):
    if not admin(message.from_user.id):
        return
    ...
    try:
        ...
        pass
    if not secret:
        await ui_message(message, '❌ کلید خالی است. FERNET_KEY یا توکن قدیمی ربات را ارسال کنید.')
        return

    state_data = await state.get_data()
    path = str(state_data.get('legacy_backup_path') or '')
    if not path or not os.path.exists(path):
        await state.clear()
        await ui_message(
            message,
            '❌ فایل موقت Restore پیدا نشد. لطفاً دوباره فایل بک‌آپ را ارسال کنید.',
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]),
        )
        return
    ...
    try:
        ...
        kind = 'bot_token' if ':' in secret else 'fernet'
        restored = await _restore_backup_payload(
            data,
            legacy_secret=secret,
            legacy_secret_kind=kind,
        )
        total = sum(restored.values())
        completed = True
        await state.clear()
        await ui_message(
            message,
            '✅ بک‌آپ قدیمی با کلید VPS مبدا باز شد و کامل روی این نصب سینک شد.\n\n'
            f'📊 تعداد رکوردهای بازیابی‌شده: {total}\n'
            '🔐 تمام رمزهای سرورها برای FERNET_KEY نصب جدید دوباره رمزگذاری شدند.',
            reply_markup=await main_menu_inline(True),
        )
    except ValueError as exc:
        await ui_message(
            message,
            f'❌ کلید ارسال‌شده با بک‌آپ مطابقت ندارد یا معتبر نیست.\n\n{exc}\n\nدوباره FERNET_KEY یا توکن قدیمی را ارسال کنید.',
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]),
        )
    except Exception as exc:
        await state.clear()
        await handle_user_facing_error(
            message,
            exc,
            context='Admin legacy portable restore failed',
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]),
        )
    finally:
        if completed:
...
async def discount_list(callback: CallbackQuery):
    ...
    if not items:
        await edit_or_answer(callback, 'هنوز کد تخفیفی Not set است.', reply_markup=discounts_kb()); await callback.answer(); return
    ...
    for d in items:
        ...
        status = '🟢 Enabled' if d.is_active else '🔴 Disabled'
...
async def discount_detail(callback: CallbackQuery, state: FSMContext):
    ...
    text = (
        ...
        f'⚙️ وضعیت: {"Enabled" if d.is_active else "Disabled"}'
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text='🟢/🔴 Enabled یا Disabled', callback_data=f'discount:toggle:{did}')],
...
async def _get_web_settings_text() -> str:
    ...
    password_status = 'ثبت شده' if await get_setting_value('web_admin_password', '') else 'Not set'
    return (
        '🌐 تنظیمات وب سایت\n━━━━━━━━━━━━━━━━\n\n'
        f'🌍 دامنه: {domain or "Not set"}\n'
        f'👤 Username: {username or "Not set"}\n'
        f'🔐 Password: {password_status}\n'
        f'⏱ زمان توکن: {timeout} minutes\n'
...
async def website_command_username_save(message: Message, state: FSMContext):
    ...
    try:
        ...
        await ui_message(message, f'❌ {exc}')
        return
...
async def website_command_password_save(message: Message, state: FSMContext):
    ...
    if len(password) < 8:
        await ui_message(message, '❌ Password باید حداقل ۸ کاراکتر باشد.')
        return
    data = await state.get_data()
    ...
    await ui_message(
        message,
        f'''✅ تنظیمات ورود سایت ذخیره و با Credentials Center همگام شد.
...
async def website_username_save(message: Message, state: FSMContext):
    ...
    username = (message.text or '').strip()
    try:
        await save_web_credentials(username=username, updated_by='telegram-settings')
    except ValueError as exc:
        await ui_message(message, f'❌ {exc}')
        return
    await state.clear()
    await ui_message(message, f'''✅ Username سایت همان لحظه به‌روزرسانی و با Credentials Center همگام شد.
👤 Username فعلی: {username}

برای ورود بعدی از همین Username استفاده کنید.''', reply_markup=_website_kb())
...
async def website_password_save(message: Message, state: FSMContext):
    ...
    password = message.text or ''
    if len(password) < 8:
        await ui_message(message, '❌ Password باید حداقل ۸ کاراکتر باشد.', reply_markup=_website_kb())
        return
    try:
        await save_web_credentials(password=password, updated_by='telegram-settings')
    except ValueError as exc:
        await ui_message(message, f'❌ {exc}', reply_markup=_website_kb())
        return
    await state.clear()
    await ui_message(message, '''✅ Password سایت همان لحظه به‌روزرسانی و با Credentials Center همگام شد.
🔐 برای Login بعدی از Password جدید استفاده کنید.''', reply_markup=_website_kb())
...
async def website_timeout_start(callback: CallbackQuery, state: FSMContext):
    ...
    await edit_or_answer(callback, '⏱ زمان اعتبار توکن را به minutes وارد کنید. مثال: 30', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:website_settings')]]))
    await callback.answer()
...
async def website_timeout_save(message: Message, state: FSMContext):
    ...
    await state.clear()
    await ui_message(message, f'✅ زمان توکن سایت روی {value} minutes تنظیم شد.', reply_markup=_website_kb())
...
async def website_domain_save(message: Message, state: FSMContext):
    ...
    try:
        ...
        if proc.returncode == 0:
            await set_value('web_ssl_status', 'active')
            await _notify_admins(message.bot, f'✅ SSL سایت با موفقیت Enabled شد.\n🌍 Domain: {domain}', reply_markup=_website_back_kb())
        else:
...
def referral_settings_kb(reward_enabled: str = '0', commission_enabled: str = '0') -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f'1️⃣ ارسال یوزر به ازای دعوت | {"🟢 On" if reward_enabled == "1" else "🔴 Off"}', callback_data='refset:reward')],
        [InlineKeyboardButton(text=f'2️⃣ پورسانت به ازای خرید | {"🟢 On" if commission_enabled == "1" else "🔴 Off"}', callback_data='refset:commission')],
        [back_button('admin:user_interaction')],
    ])
