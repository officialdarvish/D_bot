...
from app.bot.states.admin_states import TestAccountConfig, PaymentCardConfig, PaymentTextConfig, RulesTextConfig, PaygRate, BroadcastFlow, RestoreBackup, ServiceTypeConfig, AddDiscountCode, EditDiscountCode
...
async def set_channel(message: Message):
    ...
    await ui_message(message, '✅ کانال ذخیره شد.', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]))
...
async def paycard_detail(callback: CallbackQuery):
    ...
    text=(
        ...
        f'🖥 کاربرد: {'نماینده‌ها' if c.server_type == 'reseller' else f'سرور {c.server_id or c.server_type}'}\n'
        f'👁 وضعیت: {status}'
    )
...
async def payg_change(callback: CallbackQuery, state: FSMContext):
    ...
    await edit_or_answer(callback, 'چه مقداری می‌خواهید بابت هر 1 گیگ بگذارید؟\n\nفقط رقم را به تومان وارد کنید:', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]))
    await callback.answer()
...
def buttons_kb(bot_enabled='1', test_enabled='1', test_visible='1'):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text='🧩 نوع سرویس‌ها', callback_data='buttons:service_types')],
...
def service_type_detail_kb(kind: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [back_button('buttons:service_types')],
    ])
...
async def channel_set_start(callback: CallbackQuery, state: FSMContext):
    ...
    await ui_callback_message(callback, 'لینک کانال اجباری را ارسال کنید. مثال:\nhttps://t.me/yourchannel', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]))
    await callback.answer()
...
async def channel_save_state(message: Message, state: FSMContext):
    ...
    await set_value('channel_url', message.text.strip().replace(' ', ''))
    await state.clear()
    await ui_message(message, '✅ کانال اجباری ذخیره شد.', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]))
...
async def restore_backup_start(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]])
...
async def restore_backup_file(message: Message, state: FSMContext):
    ...
    if not doc.file_name.lower().endswith('.json'):
        await ui_message(message, '❌ لطفاً فایل بک‌آپ با فرمت JSON ارسال کنید.', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]))
        return
    ...
    try:
        ...
        await state.clear()
        await ui_message(message, f'❌ ری‌استور بک‌آپ ناموفق بود:\n{e}', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('admin:bot_settings')]]))
