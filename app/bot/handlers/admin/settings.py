...
async def backup_send(callback: CallbackQuery):
    ...
    await ui_callback_message(callback, await get_setting_value('welcome_text', WELCOME_TEXT_DEFAULT), reply_markup=await main_menu_inline(True))
    await callback.answer()
    ...
    try:
        ...
        async with SessionLocal() as session:
            ...
            for model, key in [
                ...
                await session.commit()
        await state.clear()
        await ui_message(message, '✅ بک‌آپ با موفقیت ری‌استور شد.', reply_markup=await main_menu_inline(True))
...
async def website_command_password_save(message: Message, state: FSMContext):
    ...
    await save_web_settings_bulk({
        ...
        'web_admin_password': hash_password(password) if password else '',
...
async def website_password_save(message: Message, state: FSMContext):
    ...
    if len(password) < 4:
        ...
        return
    await save_web_setting('web_admin_password', hash_password(password))
    await state.clear()
