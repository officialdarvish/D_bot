...
from app.database.models import User, Server, ResellerAccount, ResellerPackage, ResellerTopupRequest, ResellerAccessRequest, ClientService
...
async def reseller_delete(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if r:
            r.is_active=False
            await session.commit()
    await edit_or_answer(callback,'✅ دسترسی نماینده حذف/غیرفعال شد. سرویس‌های ساخته‌شده دست‌نخورده باقی ماندند.', reply_markup=admin_reseller_menu()); await callback.answer()
...
def _reseller_server_confirm_kb(mode: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f'✅ {label}', callback_data='resadmin:server_confirm')],
        [InlineKeyboardButton(text='❌ اصلاح اطلاعات', callback_data='resadmin:server_restart')],
...
async def reseller_server_password(message: Message, state: FSMContext):
    ...
    if not ok:
        await ui_message(
            message,
            f'❌ اتصال به پنل یا دریافت Inboundها ناموفق بود.\n\nدلیل خطا:\n{err}\n\nاطلاعات را بررسی کنید و دوباره اضافه/ویرایش سرور را شروع کنید.',
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('resadmin:servers')]])
        )
        await state.clear()
        return
...
async def reseller_server_preview(message: Message, state: FSMContext):
    # سازگاری با stateهای قدیمی؛ در فلو جدید Inboundها خودکار از پنل گرفته می‌شوند.
    if not admin(message.from_user.id): return
    data = await state.get_data()
    ok, inbound_ids, err = await _fetch_reseller_panel_inbounds(data)
    if not ok:
        await ui_message(message, f'❌ دریافت Inboundها ناموفق بود.\n{err}', reply_markup=InlineKeyboardMarkup(inline_keyboard=[[back_button('resadmin:servers')]]))
...
async def reseller_server_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        err=str(exc)
    if not ok:
        await edit_or_answer(callback, f'❌ اتصال به پنل نماینده ناموفق بود.\n\nدلیل خطا:\n{err or "Login/List inbounds failed"}\n\nاطلاعات را اصلاح کنید و دوباره تایید بزنید.', reply_markup=_reseller_server_confirm_kb(data.get('mode') or 'add'))
...
async def packages_list(callback: CallbackQuery):
    if not admin(callback.from_user.id): return
    async with SessionLocal() as session:
        packs=(await session.execute(select(ResellerPackage).order_by(ResellerPackage.id.desc()))).scalars().all()
    if not packs:
