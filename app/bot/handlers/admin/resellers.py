...
from app.database.models import User, Server, ResellerAccount, ResellerPackage, ResellerTopupRequest, ResellerAccessRequest, ClientService, ResellerBuildConfig, PaymentCard, ResellerServiceActivity
from app.bot.keyboards.common import CB_RESELLERS, back_button, back_admin_inline, main_menu_inline
from app.bot.states.admin_states import AddResellerPackage, EditResellerPackage, ExtendReseller, ResellerServerForm, AdjustResellerVolume, ResellerReceiptReject
...
from app.services.reseller_service import gb_to_bytes, bytes_to_gb, reseller_stats, remaining_bytes, reconcile_reseller_accounting, apply_package, approve_reseller_access, reject_reseller_access
from app.services.xui_service import XuiService
from app.services.plan_order import saved_plan_order, sort_by_saved_order
from app.jobs.server_sync import refresh_server_inbounds, refresh_mikrotik_server
...
def reseller_router_connection_text(server: Server | None) -> str:
    if not server or server.server_type != 'mikrotik':
        return '-'
    meta = server.meta or {}
    if 'router_online' not in meta:
        return '🟡 نامشخص'
    return '🟢 آنلاین' if bool(meta.get('router_online')) else '🔴 آفلاین موقت'
...
async def send_reseller_home(bot, chat_id: int, is_admin_user: bool = False) -> None:
    ...
    await send_single_message(
        ...
        reply_markup=await main_menu_inline(is_admin_user, is_reseller=bool(reseller and reseller.is_active)),
    )
...
async def reseller_server_detail(callback: CallbackQuery):
    ...
    detail_text = (
        ...
        f'└ {status_text}\n\n'
        '📶 وضعیت Router\n'
        f'└ {reseller_router_connection_text(s)}\n\n'
        '📥 Inbounds\n'
...
async def reseller_server_refresh(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if not is_reseller_server(s):
            await callback.answer('سرور نماینده پیدا نشد.', show_alert=True); return
        if s.server_type == 'mikrotik':
            ok, err = await refresh_mikrotik_server(session, s)
            await session.commit()
            items = await all_reseller_servers(session)
            connection = reseller_router_connection_text(s)
            admin_state = 'فعال' if s.is_active else 'غیرفعال دستی'
            if not ok:
                await edit_or_answer(
                    callback,
                    f'⚠️ اتصال لحظه‌ای MikroTik برقرار نشد.\n\nوضعیت در D Bot: {admin_state}\nوضعیت Router: {connection}\nخطا: {err or "نامشخص"}\n\nاین خطا سرور را خودکار غیرفعال نمی‌کند.',
                    reply_markup=reseller_servers_keyboard(items),
                )
                await callback.answer('اتصال موقتاً ناموفق بود.', show_alert=True)
                return
            await edit_or_answer(
                callback,
                f'✅ وضعیت MikroTik بروزرسانی شد.\n\nوضعیت در D Bot: {admin_state}\nوضعیت Router: {connection}',
                reply_markup=reseller_servers_keyboard(items),
            )
...
async def reseller_server_toggle(callback: CallbackQuery):
    ...
    async with SessionLocal() as session:
        ...
        if s and is_reseller_server(s):
            ...
            if s.server_type == 'mikrotik':
                meta = dict(s.meta or {})
                meta['admin_enabled'] = bool(s.is_active)
                s.meta = meta
            await session.commit()
    async with SessionLocal() as session:
        items=await all_reseller_servers(session)
...
async def reseller_adjust_volume_save(message: Message, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        await reconcile_reseller_accounting(session, r)
        ...
        await session.commit()
        await reconcile_reseller_accounting(session, r)
        total=bytes_to_gb(r.total_bytes); remaining=bytes_to_gb(remaining_bytes(r))
    await state.clear()
...
async def reseller_access_approve(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.bot.send_message(
            u.telegram_id,
            await get_setting_value('welcome_text', WELCOME_TEXT_DEFAULT),
            reply_markup=await main_menu_inline(False, True, True),
        )
    except Exception:
        pass
    ...
    await callback.message.answer('🏠 صفحه اصلی', reply_markup=await main_menu_inline(True))
...
async def reseller_access_reject(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.bot.send_message(
            u.telegram_id,
            await get_setting_value('welcome_text', WELCOME_TEXT_DEFAULT),
            reply_markup=await main_menu_inline(False, True, False),
        )
    except Exception:
        pass
...
async def reseller_request_approve(callback: CallbackQuery):
    ...
    req_id=int(callback.data.split(':')[-1])
    async with SessionLocal() as session:
        # Lock the receipt row so two admins/double-clicks cannot recharge the
        # same reseller package twice.
        r=(await session.execute(
            select(ResellerTopupRequest).where(ResellerTopupRequest.id == req_id).with_for_update()
        )).scalar_one_or_none()
        if not r or r.status!='pending': await callback.answer('درخواست قابل تایید نیست.', show_alert=True); return
        reseller=await apply_package(
            session, r, source='admin_topup_approval', actor_telegram_id=callback.from_user.id
        )
        u=await session.get(User,r.user_id); await session.commit()
    await mark_message_status(callback, '✅ تایید شد')
    try:
...
async def reseller_request_reject(callback: CallbackQuery, state: FSMContext):
    if not admin(callback.from_user.id):
        return
    req_id = int(callback.data.split(':')[-1])
    async with SessionLocal() as session:
        req = await session.get(ResellerTopupRequest, req_id)
        if not req:
            await mark_message_status(callback, '❌ درخواست پیدا نشد')
            await callback.answer('درخواست پیدا نشد.', show_alert=True)
            return
    ...
    await state.clear()
    await state.update_data(
        reseller_reject_request_id=req_id,
        reseller_receipt_chat_id=(callback.message.chat.id if callback.message else callback.from_user.id),
        reseller_receipt_message_id=(callback.message.message_id if callback.message else None),
    )
    await state.set_state(ResellerReceiptReject.reason)
    await mark_message_status(callback, '✍️ دلیل رد را ارسال کنید')
    await callback.message.answer(
        f"""✍️ دلیل رد رسید شارژ نمایندگی #{req_id} را بنویسید.

همین متن برای نماینده ارسال و در سابقه درخواست ذخیره می‌شود."""
    )
    await callback.answer('دلیل رد را ارسال کنید.')


@router.message(ResellerReceiptReject.reason)
async def reseller_request_reject_reason(message: Message, state: FSMContext):
    ...
    if len(reason) > 1500:
        ...
        return
    data = await state.get_data()
    req_id = int(data.get('reseller_reject_request_id') or 0)
    chat_id = data.get('reseller_receipt_chat_id')
    message_id = data.get('reseller_receipt_message_id')
    async with SessionLocal() as session:
        req = await session.get(ResellerTopupRequest, req_id)
        if not req:
            await state.clear(); await message.answer('❌ درخواست پیدا نشد.'); return
        user = await session.get(User, req.user_id)
        pkg = await session.get(ResellerPackage, req.package_id) if req.package_id else None
        if req.status == 'approved':
            await state.clear()
    ...
    if user:
        rejection_text = (
            f"""❌ رسید شارژ نمایندگی شما رد شد.

🧾 شماره درخواست: #{req_id}
📦 بسته: {pkg.title if pkg else '-'}
💾 حجم: {bytes_to_gb(req.volume_bytes):g} گیگ
💰 مبلغ: {int(req.amount_irt or 0):,} تومان
...
{reason}"""
    ...
    await send_reseller_home(message.bot, message.from_user.id, True)
