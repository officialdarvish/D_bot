...
def _is_result_message(text: str) -> bool:
    value = (text or '').strip()
    # Service detail pages are normal navigable pages and must be edited in-place,
    # especially when the user presses the refresh/update button.
    if value.startswith('✅ کانفیگ با مشخصات زیر پیدا شد') or value.startswith('📌 کانفیگ با مشخصات زیر'):
        return False
