...
BTN_PAYG = 'سیستم Pay As You Go ⚖️'
BTN_ANTI_SHARING = 'محدودیت کاربر 🛡'
BTN_BROADCAST = 'ارسال پیام همگانی 📢'
BTN_WALLET_ADJUST = 'کاهش یا اضافه موجودی 💰'
...
CB_PAYG = 'admin:payg'
CB_ANTI_SHARING = 'admin:anti_sharing'
...
def main_menu_inline(is_admin: bool = False, show_test: bool = True, is_reseller: bool = False) -> InlineKeyboardMarkup:
    ...
    buttons.append(InlineKeyboardButton(text=(BTN_RESELLER_MENU if is_reseller else BTN_RESELLER_REQUEST), callback_data=CB_RESELLER))
    rows = _pair_rows(buttons)
...
def user_interaction_inline() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=BTN_USERS, callback_data=CB_USERS), InlineKeyboardButton(text=BTN_RULES, callback_data=CB_RULES)],
        [back_button(CB_BACK_ADMIN)],
    ])

def bot_settings_inline() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=BTN_ANTI_SHARING, callback_data=CB_ANTI_SHARING), InlineKeyboardButton(text=BTN_BUTTONS, callback_data=CB_BUTTONS)],
        [InlineKeyboardButton(text=BTN_BACKUP, callback_data=CB_BACKUP), InlineKeyboardButton(text=BTN_RESTORE, callback_data=CB_RESTORE)],
        [back_button(CB_BACK_ADMIN)],
    ])
