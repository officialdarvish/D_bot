...
BTN_PRIVATE_MESSAGE = 'ارسال پیام خصوصی ✉️'
...
CB_PRIVATE_MESSAGE = 'admin:private_message'
...
def report_home_inline() -> InlineKeyboardMarkup:
    """Home button for persistent audit/notification messages.

    The report itself must remain in chat. The callback handler removes only
    this keyboard and opens a fresh main-menu page for the person who clicked.
    """
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text='🏠 خانه', callback_data='report:home')
    ]])


def private_message_user_actions(admin_id: int) -> InlineKeyboardMarkup:
    """Actions shown under an admin private message delivered to a user."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='✍️ پاسخ به پیام', callback_data=f'private_chat:reply:{int(admin_id)}')],
        [InlineKeyboardButton(text='✅ خاتمه گفتگو', callback_data='private_chat:end')],
    ])


def private_message_admin_actions(user_id: int) -> InlineKeyboardMarkup:
    """Quick reply action shown to the owner when a user answers a private message."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='✍️ پاسخ به کاربر', callback_data=f'private_admin:reply:{int(user_id)}')],
    ])
...
def user_interaction_inline() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=BTN_PRIVATE_MESSAGE, callback_data=CB_PRIVATE_MESSAGE), InlineKeyboardButton(text=BTN_BROADCAST, callback_data=CB_BROADCAST)],
        [InlineKeyboardButton(text=BTN_TICKETS, callback_data=CB_TICKETS), InlineKeyboardButton(text=BTN_USERS, callback_data=CB_USERS)],
        [InlineKeyboardButton(text=BTN_WALLET_ADJUST, callback_data=CB_WALLET_ADJUST), InlineKeyboardButton(text=BTN_RULES, callback_data=CB_RULES)],
        [InlineKeyboardButton(text=BTN_REFERRAL_SETTINGS, callback_data=CB_REFERRAL_SETTINGS)],
        [back_button(CB_BACK_ADMIN)],
    ])
