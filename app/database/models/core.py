...
class Plan(Base):
    ...
    category_id: Mapped[int] = mapped_column(ForeignKey('server_categories.id'))
    ...
    anti_sharing_enabled: Mapped[bool] = mapped_column(Boolean, default=True)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
...
class ClientService(Base):
    ...
    last_payg_used_bytes: Mapped[int] = mapped_column(BigInteger, default=0)
    expires_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    is_payg: Mapped[bool] = mapped_column(Boolean, default=False)
    ip_limit: Mapped[int] = mapped_column(Integer, default=0)
    anti_share_violation_count: Mapped[int] = mapped_column(Integer, default=0)
    anti_share_banned_until: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    anti_share_banned_permanent: Mapped[bool] = mapped_column(Boolean, default=False)
    anti_share_last_alert_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    anti_share_last_ips: Mapped[list] = mapped_column(JSON, default=list)
...
class AntiSharingViolation(Base):
    __tablename__ = 'anti_sharing_violations'
    id: Mapped[int] = mapped_column(primary_key=True)
    service_id: Mapped[int] = mapped_column(ForeignKey('client_services.id', ondelete='CASCADE'))
    user_id: Mapped[int | None] = mapped_column(ForeignKey('users.id'), nullable=True)
    server_id: Mapped[int | None] = mapped_column(ForeignKey('servers.id'), nullable=True)
    email: Mapped[str] = mapped_column(String(150), index=True)
    ip_count: Mapped[int] = mapped_column(Integer, default=0)
    allowed_ip: Mapped[int] = mapped_column(Integer, default=0)
    ips: Mapped[list] = mapped_column(JSON, default=list)
    action: Mapped[str] = mapped_column(String(32), default='warning')
...
class ResellerTopupRequest(Base):
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey('users.id'))
    reseller_id: Mapped[int | None] = mapped_column(ForeignKey('reseller_accounts.id', ondelete='SET NULL'), nullable=True)
    package_id: Mapped[int] = mapped_column(ForeignKey('reseller_packages.id'))
...
class PaygUsageLog(Base):
    __tablename__ = 'payg_usage_logs'
    id: Mapped[int] = mapped_column(primary_key=True)
    service_id: Mapped[int] = mapped_column(ForeignKey('client_services.id', ondelete='CASCADE'))
    used_delta_bytes: Mapped[int] = mapped_column(BigInteger)
    charged_irt: Mapped[int] = mapped_column(BigInteger)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
