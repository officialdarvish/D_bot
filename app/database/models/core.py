...
class ServiceDeletionTask(Base):
    """Persistent panel-cleanup queue for locally removed services.

    Panel APIs can be temporarily unreachable (especially MikroTik/OpenVPN).
    The service is removed from the user's lists once the 72-hour grace period
    ends, while this row keeps the minimum identifiers needed to retry the
    upstream deletion without keeping the full service record alive.
    """

    __tablename__ = 'service_deletion_tasks'

    id: Mapped[int] = mapped_column(primary_key=True)
    source_service_id: Mapped[int] = mapped_column(BigInteger, unique=True, index=True)
...
class ResellerAccount(Base):
    ...
    server_id: Mapped[int | None] = mapped_column(ForeignKey('servers.id'), nullable=True)
    # Per-reseller X-UI inbound policy. Automatic means every currently active
    # inbound on the reseller's server; manual means only inbound_ids below.
    inbound_mode: Mapped[str] = mapped_column(String(16), default='automatic')
    inbound_ids: Mapped[list] = mapped_column(JSON, default=list)
    total_bytes: Mapped[int] = mapped_column(BigInteger, default=0)
    used_bytes: Mapped[int] = mapped_column(BigInteger, default=0)
...
class ResellerUsernameCounter(Base):
    """Independent four-digit sequence for duplicated reseller service names."""
    __tablename__ = 'reseller_username_counters'
    id: Mapped[int] = mapped_column(primary_key=True, default=1)
    next_number: Mapped[int] = mapped_column(BigInteger, default=1, nullable=False)
