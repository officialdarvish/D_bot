...
from app.database.models import ResellerAccount, ResellerPackage, ResellerTopupRequest, ResellerAccessRequest, ClientService, User, Server, ResellerServiceActivity
        ...
        try:
            ...
            if server.server_type == 'mikrotik':
                for ident in identifiers:
                    ...
                    baseline = int(getattr(service, 'traffic_baseline_bytes', 0) or 0)
                    effective_used = MikroTikService.logical_used_bytes(found, baseline, service.used_bytes or 0)
                    await apply_reseller_usage_delta(session, service, effective_used)
                    if found.get('volume_bytes') is not None:
                        service.total_bytes = MikroTikService.logical_total_bytes(found, baseline, service.total_bytes or 0)
...
async def apply_package(
    ...
    request: ResellerTopupRequest,
    *,
    source: str = 'reseller_topup',
    actor_telegram_id: int | None = None,
) -> ResellerAccount:
    """Apply a reseller quota package exactly once and record the recharge.

    ``ResellerTopupRequest.status`` plus the unique activity event make repeated
    approval callbacks idempotent, so a double-click cannot recharge the quota
    twice.
    """
    ...
    # A second approval callback must never add the same package again.
    if str(request.status or '').lower() == 'approved':
        await reconcile_reseller_accounting(session, reseller)
        return reseller

    before_total = int(reseller.total_bytes or 0)
    added_bytes = int(request.volume_bytes or 0)
    reseller.total_bytes = before_total + added_bytes
    ...
    event_key = f'topup:{request.id}' if request.id else None
    existing = None
    if event_key:
        existing = (await session.execute(
            select(ResellerServiceActivity).where(ResellerServiceActivity.event_key == event_key)
        )).scalar_one_or_none()
    if not existing:
        ...
        username = (
            ...
            or (str(getattr(user, 'telegram_id', '')) if user else '')
            or '-'
        )
        after_total = int(reseller.total_bytes or 0)
        session.add(ResellerServiceActivity(
            reseller_id=reseller.id,
            service_id=None,
            server_id=(pkg.server_id if pkg and pkg.server_id else reseller.server_id),
            action='topup',
            event_key=event_key or f'topup:pending:{request.user_id}:{now.strftime("%Y%m%d%H%M%S%f")}',
            username=username,
            volume_bytes=added_bytes,
            previous_volume_bytes=before_total,
            duration_days=int(add_days or 0),
            ...
            meta={
                ...
                'deducted_bytes': 0,
                'quota_before_bytes': before_total,
                'quota_after_bytes': after_total,
                'quota_remaining_after_bytes': after_total,
                'quota_delta_bytes': after_total - before_total,
