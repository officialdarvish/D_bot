...
class MikroTikService:
        ...
        if preferred:
            for row in routers:
                if str(row.get('name') or '').strip().lower() == preferred:
                    return row
        for row in routers:
            if self.router_is_online(row, True):
                return row
    ...
    async def test_server(self, server: Server) -> tuple[bool, list[dict[str, Any]]]:
        try:
            ...
            if router_name:
                ...
                return bool(matched and self.router_is_online(matched[0], True)), matched or routers
    ...
    def router_is_online(self, router: dict[str, Any] | None, fallback: bool = True) -> bool:
        """Parse router online state without treating the string ``false`` as true."""
        if not isinstance(router, dict):
            return bool(fallback)
        for key in ('online', 'is_online', 'connected', 'reachable'):
            parsed = self._as_bool(router.get(key))
            if parsed is not None:
                return parsed
        status = str(router.get('status') or '').strip().lower()
        if status in {'offline', 'disconnected', 'unreachable', 'down', 'error'}:
            return False
        if status in {'online', 'connected', 'reachable', 'up', 'active'}:
            return True
        return bool(fallback)
    ...
    async def renew_user(
        ...
        *,
        ...
        """Renew a MikroTik/OpenVPN user through the panel's native endpoint.

        The current panel exposes the atomic renewal action documented as::

            POST /api/routers/{router}/users/{username}/renew
            {"expire_days": 30}

        That endpoint resets the user's traffic counter, enables the PPP secret,
        and starts a fresh expiry period. After the native renewal, D Bot applies
        the selected plan quota and verifies the remote enabled/expiry/reset
        state before the local service is marked renewed.
        """
        ...
        # Native panel renewal: reset usage + enable user + fresh expiry.
        renew_payload = {'expire_days': days}
        native = await self._request(
            server,
            'POST',
            f'/routers/{quote(router)}/users/{quote(username)}/renew',
            json=renew_payload,
        )

        # The renewal endpoint may preserve the previous quota. Apply the plan
        # chosen in D Bot after the reset so upgrades/downgrades are respected.
        plan_bytes = int(round(float(volume_gb or 0) * (1024 ** 3))) if volume_gb else 0
        if volume_gb is not None:
            await self.update_user(
                server,
                username,
                volume_bytes=plan_bytes if plan_bytes > 0 else 0,
            )

        after = await self.get_user(server, username) or {}
        if not self.user_is_active(after, False):
            # The native endpoint is expected to enable the user. Keep this
            # explicit recovery for panels where active sessions delay the flag.
            await self.enable_user(server, username)
            after = await self.get_user(server, username) or after

        raw_after = self.raw_used_bytes(after, 0)
        remote_total = self._int_value(after.get('volume_bytes'))

        # A connected user can consume a small amount immediately after renewal,
        # so allow a tiny post-reset window. A counter that remains at/above the
        # old value means the native reset did not happen and must not be accepted.
        reset_tolerance = 10 * 1024 ** 2
        if raw_after > reset_tolerance:
            raise RuntimeError(
                'MikroTik renewal failed: native /renew did not reset usage '
                f'(before={raw_before}, after={raw_after})'
            )

        quota_tolerance = 1024 ** 2
        if plan_bytes > 0 and remote_total + quota_tolerance < plan_bytes:
            raise RuntimeError(
                'MikroTik renewal failed: quota was not updated on panel '
                f'(expected at least {plan_bytes} bytes, got {remote_total})'
            )
        if not self.user_is_active(after, False):
        ...
        if days:
            ...
            expected_min = date.today() + timedelta(days=max(days - 1, 0))
        ...
        result = dict(after)
        result['_native_renew_response'] = native.get('data') or native
        result['_traffic_baseline_bytes'] = 0
        result['_logical_used_bytes'] = int(raw_after)
        result['_logical_total_bytes'] = int(plan_bytes if plan_bytes > 0 else remote_total)
        result['_remote_total_bytes'] = int(remote_total)
        result['_remote_used_bytes'] = int(raw_after)
        result['_usage_reset'] = True
        return result
