...
class NowPaymentsService:
    ...
    @staticmethod
    def validate_payment_details(details: dict[str, Any], *, order_id: int, payment_id: str, amount_irt: int) -> None:
        if str(details.get('order_id') or '') != str(order_id):
            raise ValueError('NOWPayments order_id mismatch')
        if payment_id and str(details.get('payment_id') or '') != str(payment_id):
            raise ValueError('NOWPayments payment_id mismatch')
        status = str(details.get('payment_status') or '').lower()
        if status not in {'finished', 'confirmed', 'sending'}:
            raise ValueError('NOWPayments payment is not confirmed')
        currency = str(details.get('price_currency') or '').lower()
        if currency and currency != 'irr':
            raise ValueError('NOWPayments price currency mismatch')
        try:
            paid_amount = float(details.get('price_amount') or 0)
            expected = float(amount_irt or 0)
            if expected > 0 and paid_amount + max(1.0, expected * 0.01) < expected:
                raise ValueError('NOWPayments price amount mismatch')
        except TypeError:
            raise ValueError('NOWPayments price amount is invalid')
