...
def _simple_delete_reason(snapshot: dict) -> str:
    source = str(snapshot.get('deletion_source') or '').lower()
    reason = str(snapshot.get('deletion_reason') or '').lower()
    if source in {'user_manual', 'reseller_manual'} or reason in {'user_delete', 'reseller_delete'}:
        return 'دستی'
    return 'ربات'
...
def admin_deletion_message(
    ...
    *,
    ...
    """Short admin report requested for manual/automatic config deletion.

    panel_status/task_id stay in the signature for existing callers, but the
    notification intentionally omits queue IDs and technical cleanup details.
    """
    ...
    return (
        '🗑 گزارش حذف کانفیگ\n'
        '━━━━━━━━━━━━━━\n'
        f'📌 دلیل: {_simple_delete_reason(snapshot)}\n\n'
        '👤 مالک سرویس\n'
        f'• یوزرنیم: {_display_username(snapshot.get("owner_username"))}\n'
        f'• آیدی عددی: {snapshot.get("owner_telegram_id") or "-"}\n\n'
        '🔐 مشخصات کانفیگ\n'
        f'• یوزرنیم پنل: {snapshot.get("panel_username") or "-"}\n'
        f'• حجم کل: {_compact_gb(total)} گیگ\n'
        f'• حجم مانده: {_compact_gb(remaining)} گیگ\n'
...
async def notify_admins_service_deletion(
    ...
    *,
    ...
    for admin_id in admin_ids:
        try:
            from app.bot.keyboards.common import report_home_inline
