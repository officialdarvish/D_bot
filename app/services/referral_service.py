...
async def notify_referrer_about_new_subset(session, bot, invited_user: User) -> None:
    ...
    if not referrer:
        return
    try:
        joined = invited_user.referral_joined_at.strftime('%Y-%m-%d %H:%M') if invited_user.referral_joined_at else '-'
    except Exception:
        joined = '-'
    text = (
...
async def apply_purchase_commission(session, buyer: User, amount_irt: int, bot=None, server_type: str | None = 'xui') -> int:
    ...
    if bot:
        try:
            wallet_title = 'OpenVPN/L2TP' if field == 'wallet_openvpn_balance' else 'V2Ray'
