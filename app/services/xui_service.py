...
class XuiService:
    ...
    async def create_client_on_inbounds(self, server: Server, inbound_ids: list[int], payload: XuiClientPayload):
        ...
        try:
            ...
            return {'results': results, 'sub_id': sub_id, 'uuid': _as_text(uuid_val), 'sub_link': self.build_subscription_link(server, sub_id, payload.email)}
        finally: await xui.close()
    ...
    async def revoke_and_new_link(self, server: Server, email: str):
        ...
        try:
            ...
            return {'uuid': _as_text(uuid_val), 'sub_id': _as_text(sub_id), 'sub_link': self.build_subscription_link(server, sub_id, email)}
        finally: await xui.close()
