...
type FieldConfig = {
  name: string;
  label: string;
  type?: 'text' | 'number' | 'password' | 'select' | 'multiselect' | 'checkbox-group' | 'textarea' | 'time' | 'date' | 'user-search' | 'file';
  required?: boolean;
  full?: boolean;
...
function CategoriesSection({ query, reloadKey, setAuthRequired, openModal, runAction }: any) {
  ...
  return <><div className="filterbar"><button className="btn primary" onClick={() => openModal(categoryForm(options))}><Plus size={16} /> Add Category</button><span className="badge">{rows.length} categories</span></div><div className="section-grid">{rows.map((c) => {
...
function PlansSection({ query, reloadKey, setAuthRequired, openModal, runAction, show }: any) {
  ...
  return <>
    <div className="filterbar">
      <div className="actions"><button className="btn primary" onClick={() => openModal(planForm(catOpts, srvOpts))}><Plus size={16} /> Add Plan</button></div>
...
function FormModal({ modal, onClose, onSubmit, show }: { modal: ModalForm; onClose: () => void; onSubmit: (values: Record<string, any>) => void; show: (m: string, good?: boolean) => void }) {
  ...
  return <div className="modal-backdrop"><motion.div className="modal-card" initial={{ scale: .96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}><div className="modal-head"><h2>{modal.title}</h2><button className="icon-btn" onClick={onClose}><X size={18} /></button></div><form className="form-grid" onSubmit={(e) => { e.preventDefault(); onSubmit(values); }}>{modal.fields.filter((f) => !f.hidden).filter(visible).map((field) => <label key={field.name} className={`form-field ${field.full ? 'full' : ''}`}><span>{field.label}</span>{field.type === 'user-search' ? <UserSearchField onPick={mergeUser} /> : field.type === 'textarea' ? <textarea required={field.required} placeholder={field.placeholder} value={values[field.name] || ''} onChange={(e) => change(field.name, e.target.value)} /> : field.type === 'checkbox-group' ? <div className="checkbox-grid">{field.options?.filter((o) => String(o.value) !== '0').map((o) => { const value = String(o.value); const checked = csvValues(field.name).includes(value); return <label key={value} className={`checkline ${checked ? 'checked' : ''}`}><input type="checkbox" checked={checked} onChange={(e) => toggleCsvValue(field.name, value, e.currentTarget.checked)} /><span className="checkmark" aria-hidden="true">{checked ? '✓' : ''}</span><span className="checktext">{o.label}</span></label>; })}</div> : field.type === 'multiselect' ? <select multiple required={field.required} value={String(values[field.name] || '').split(',').filter(Boolean)} onChange={(e) => change(field.name, Array.from(e.currentTarget.selectedOptions).map((o) => o.value).filter((v) => v !== '0').join(','))}>{field.options?.map((o) => <option key={String(o.value)} value={String(o.value)}>{o.label}</option>)}</select> : field.type === 'select' ? <select required={field.required} value={values[field.name] || ''} onChange={(e) => change(field.name, e.target.value)}>{field.options?.map((o) => <option key={String(o.value)} value={String(o.value)}>{o.label}</option>)}</select> : field.type === 'file' ? <input type="file" required={field.required} accept={field.placeholder || '.ovpn'} onChange={(e) => change(field.name, e.target.files?.[0] || null)} /> : <input type={field.type || 'text'} required={field.required} placeholder={field.placeholder} value={values[field.name] || ''} onChange={(e) => change(field.name, e.target.value)} />}</label>)}<div className="form-actions">{isNewServer && <button type="button" className="btn success" disabled={testing} onClick={testServerConnection}><CheckCircle2 size={16} /> {testing ? 'Testing...' : 'Test & Auto Fill'}</button>}<button type="button" className="btn" onClick={onClose}>Cancel</button><button className="btn primary" type="submit">Save</button></div></form></motion.div></div>;
}
...
function serverForm(s?: ServerItem): ModalForm {
  ...
  const defaults = s ? {
    ...
    api_key: '',
    l2tp_server: s.l2tp_server || 'de.rgiftshop.ir',
    l2tp_ipsec_secret: s.l2tp_ipsec_secret || '123456789',
    ...
    detected_routers: '',
    api_key: '',
    l2tp_server: 'de.rgiftshop.ir',
    l2tp_ipsec_secret: '123456789',
    badge_color: '#2563eb',
  ...
  return {
    ...
    fields: [
      ...
      { name: 'l2tp_server', label: 'L2TP server shown in guides', placeholder: 'de.rgiftshop.ir', showWhen: { name: 'server_type', value: 'mikrotik' } },
      { name: 'l2tp_ipsec_secret', label: 'L2TP Secret shown in guides', placeholder: '123456789', showWhen: { name: 'server_type', value: 'mikrotik' } },
...
function planForm(catOptions: any[], serverOptionsList: any[], p?: PlanItem): ModalForm {
  return {
    ...
    defaults: p ? { ...p, plan_kind: 'public' } : { plan_kind: 'public', category_id: 0, server_id: 0, reseller_validity_days: 365 },
    fields: [
