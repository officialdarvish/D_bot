...
type FieldConfig = {
  name: string;
  label: string;
  type?: 'text' | 'number' | 'password' | 'select' | 'textarea' | 'time' | 'date' | 'user-search';
  required?: boolean;
  full?: boolean;
...
function BackupSection({ reloadKey, setAuthRequired, show }: any) {
  ...
  async function restoreBackup() {
    ...
    try {
      ...
      const res = await fetch('/admin/backup/restore', { method: 'POST', body: fd, credentials: 'include', headers: { Accept: 'application/json', 'X-Requested-With': 'fetch' } });
...
function FormModal({ modal, onClose, onSubmit, show }: { modal: ModalForm; onClose: () => void; onSubmit: (values: Record<string, FormDataEntryValue>) => void; show: (m: string, good?: boolean) => void }) {
  const [values, setValues] = useState<Record<string, string>>(() => Object.fromEntries(modal.fields.map((f) => [f.name, String(modal.defaults?.[f.name] ?? '')])));
  const [testing, setTesting] = useState(false);
  function change(name: string, value: string) { setValues((prev) => ({ ...prev, [name]: value })); }
...
function serverForm(s?: ServerItem): ModalForm { const defaults = s ? { ...s, panel_url: s.panel_base_url || s.panel_url, panel_path: s.panel_path || '/' } : { server_type: 'xui', scope: 'public', panel_path: '/' }; return { title: s ? 'Edit Server' : 'Add Server', action: s ? `/admin/servers/${s.id}/edit` : '/admin/servers/add', defaults, fields: [{ name: 'server_type', label: 'Profile', type: 'select', options: [{ value: 'xui', label: '3x-ui Sanaei' }] }, { name: 'scope', label: 'Show for', type: 'select', options: commonScopeOptions() }, { name: 'name', label: 'Server name', required: true }, { name: 'display_name', label: 'Display name' }, { name: 'panel_url', label: 'Panel URL / Origin', required: true, full: true, placeholder: 'https://domain.com:2053' }, { name: 'panel_path', label: 'Panel Web Path', placeholder: '/secretpath/' }, { name: 'subscription_url', label: 'Subscription URL', full: true }, { name: 'username', label: 'Username', required: true }, { name: 'password', label: s ? 'New password / API token' : 'Password / API token', type: 'password', required: !s }] }; }
