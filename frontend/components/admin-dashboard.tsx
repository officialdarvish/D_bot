...
function FormModal({ modal, onClose, onSubmit, show }: { modal: ModalForm; onClose: () => void; onSubmit: (values: Record<string, any>) => void; show: (m: string, good?: boolean) => void }) {
  const [values, setValues] = useState<Record<string, any>>(() => {
    ...
    modal.fields.forEach((f) => {
      if (base[f.name] === undefined) base[f.name] = String(modal.defaults?.[f.name] ?? '');
    });
    return base;
  });
  ...
  async function testServerConnection() {
    ...
    try {
      ...
      if (result?.suggested && typeof result.suggested === 'object') {
        setValues((prev) => {
          const next = { ...prev };
          Object.entries(result.suggested).forEach(([key, value]) => {
...
function serverForm(s?: ServerItem): ModalForm {
  const defaults = s
    ? { ...s, panel_url: s.panel_base_url || s.panel_url, panel_path: s.panel_path || '/', router_name: s.router_name || s.username || '' }
    : { server_type: 'xui', scope: 'public', panel_path: '/', default_protocol: 'openvpn', name: '', display_name: '', subscription_url: '' };
  return {
    title: s ? 'Edit Server' : 'Add Server',
    action: s ? `/admin/servers/${s.id}/edit` : '/admin/servers/add',
    defaults,
    fields: [
      { name: 'server_type', label: 'Panel type', type: 'select', options: [{ value: 'xui', label: '3x-ui Sanaei' }, { value: 'mikrotik', label: 'MikroTik' }] },
      { name: 'scope', label: 'Show this server for', type: 'select', options: commonScopeOptions() },

      { name: 'panel_url', label: '3x-ui Panel URL', required: true, full: true, placeholder: 'http://88.198.186.39:8090 or https://panel.example.com/secret/', help: 'Paste the panel address. If it includes the hidden web path, the system separates it automatically.', showWhen: { name: 'server_type', value: 'xui' } },
      { name: 'username', label: '3x-ui Username', required: true, placeholder: 'Panel login username', showWhen: { name: 'server_type', value: 'xui' } },
      { name: 'password', label: s ? 'New 3x-ui Password / API Token' : '3x-ui Password / API Token', type: 'password', required: !s, placeholder: 'Panel password or token:<API_TOKEN>', showWhen: { name: 'server_type', value: 'xui' } },
      { name: 'panel_path', label: '3x-ui Web Path (optional)', placeholder: '/secret-path/', help: 'Leave / when the panel has no hidden path. Test & Auto Fill normalizes it.', showWhen: { name: 'server_type', value: 'xui' } },
      { name: 'subscription_url', label: 'Subscription URL base (optional)', full: true, placeholder: 'https://sub.example.com/sub/ or leave empty for panel /sub/', help: 'If empty, D BOT builds links from the panel URL and the real subId returned by 3x-ui.', showWhen: { name: 'server_type', value: 'xui' } },
      { name: 'name', label: '3x-ui Server key (auto if empty)', placeholder: 'Auto from inbound remark or panel host', showWhen: { name: 'server_type', value: 'xui' } },
      { name: 'display_name', label: '3x-ui Display name (auto if empty)', placeholder: 'Name shown inside bot/admin', showWhen: { name: 'server_type', value: 'xui' } },

      { name: 'panel_url', label: 'MikroTik API Base URL', required: true, full: true, placeholder: 'https://mikrotik-api.example.com', showWhen: { name: 'server_type', value: 'mikrotik' } },
      { name: 'router_name', label: 'MikroTik Router name', required: true, placeholder: 'Router name from your API', showWhen: { name: 'server_type', value: 'mikrotik' } },
      { name: 'password', label: s ? 'New MikroTik API key / Password' : 'MikroTik API key / Password', type: 'password', required: !s, showWhen: { name: 'server_type', value: 'mikrotik' } },
      { name: 'name', label: 'MikroTik Server key (auto if empty)', placeholder: 'Auto from router name or host', showWhen: { name: 'server_type', value: 'mikrotik' } },
      { name: 'display_name', label: 'MikroTik Display name', placeholder: 'Name shown inside bot/admin', showWhen: { name: 'server_type', value: 'mikrotik' } },
      { name: 'default_protocol', label: 'MikroTik protocol', type: 'select', options: [{ value: 'openvpn', label: 'OpenVPN' }, { value: 'l2tp', label: 'L2TP' }, { value: 'both', label: 'OpenVPN + L2TP' }], showWhen: { name: 'server_type', value: 'mikrotik' } },
      { name: 'l2tp_server', label: 'L2TP Server address', showWhen: { name: 'server_type', value: 'mikrotik' } },
      { name: 'l2tp_ipsec_secret', label: 'L2TP IPSec Secret', type: 'password', showWhen: { name: 'server_type', value: 'mikrotik' } },
      { name: 'openvpn_profile_id', label: 'Default OpenVPN Profile ID', type: 'number', placeholder: 'Optional profile ID', showWhen: { name: 'server_type', value: 'mikrotik' } }
    ]
  };
}
