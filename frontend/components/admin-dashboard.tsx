...
type ResellerActivityItem = { id: number; action: 'create' | 'renew'; action_label?: string; service_id?: number | null; username: string; server_id?: number | null; server_name?: string; volume_bytes: number; previous_volume_bytes?: number; duration_days?: number; expires_at?: string; created_at?: string; released_bytes?: number; old_used_bytes?: number; source?: string };
...
const sectionTitles: Record<SectionKey, { title: string; subtitle: string }> = {
  ...
  settings: { title: 'Settings', subtitle: 'General bot settings, purchase texts, website login links and user-facing button controls.' }
};
  ...
  return <section className="card reseller-letter reseller-letter-page">
    ...
    {loading ? <p className="muted">Loading reseller configs...</p> : error ? <EmptyState message={error} /> : <>
      <div className="reseller-activity-head">
        <div><h3>Activity & renewal history</h3><p className="muted">Every created or renewed reseller user is recorded like an order.</p></div>
        <span className="badge">{activities.length} operations</span>
      </div>
      <div className="reseller-activity-list">
        {activities.length ? activities.map((activity) => {
          const isRenew = activity.action === 'renew';
          return <article className={`reseller-activity-order ${isRenew ? 'renew' : 'create'}`} key={activity.id}>
            <div className="activity-order-icon">{isRenew ? <RefreshCw size={19} /> : <Plus size={19} />}</div>
            <div className="activity-order-main">
              <div className="activity-order-title">
                <b>{activity.username}</b>
                <span className={`badge ${isRenew ? 'yellow' : 'green'}`}>{isRenew ? 'Renewed' : 'Created'}</span>
              </div>
              <p>{isRenew ? `User ${activity.username} was renewed on ${shortDate(activity.created_at)}.` : `User ${activity.username} was created on ${shortDate(activity.created_at)}.`}</p>
              <div className="activity-order-meta">
                <span><strong>Order:</strong> #RA-{activity.id}</span>
                <span><strong>Volume:</strong> {gbFromBytes(activity.volume_bytes)}</span>
                {isRenew && Number(activity.previous_volume_bytes || 0) > 0 && <span><strong>Previous:</strong> {gbFromBytes(activity.previous_volume_bytes)}</span>}
                <span><strong>Duration:</strong> {Number(activity.duration_days || 0)} days</span>
                <span><strong>Expires:</strong> {shortDate(activity.expires_at)}</span>
...
function SettingsSection({ reloadKey, setAuthRequired, openModal }: any) {
  ...
  return <div className="section-grid settings-grid">
    <EntityCard
      ...
      icon={<Settings />}
      badge="Bot"
      kvs={[["Bot Name", map.bot_name || 'D BOT'], ['Support', map.support_username || '-'], ['Description', map.admin_description || '-']]}
      actions={<button className="btn primary" onClick={() => openModal(settingsGeneralForm(map))}><Save size={16} /> Edit General</button>}
    />

    <EntityCard
      title="Bot Texts & Database"
      icon={<Database />}
      badge={map.bot_enabled === '0' ? 'Bot Off' : 'Bot On'}
      ...
      actions={<button className="btn primary" onClick={() => openModal(settingsBotCoreForm(map))}><Save size={16} /> Edit Bot Core</button>}
    />
    ...
    <EntityCard
      title="Bottom"
      icon={<ListChecks />}
      badge={`${enabledButtons}/${USER_BUTTON_FIELDS.length} active`}
      badgeClass="purple"
      kvs={[
        ['Purpose', 'Rename or enable/disable user-facing bot buttons'],
        ['Applies to', 'Main menu, reseller/admin entry and wallet top-up'],
        ['Update time', 'Applied from database on the next menu view'],
      ]}
...
function ServiceTypesSection({ reloadKey, setAuthRequired, openModal, runAction, show }: any) {
  const { data, loading, error, setData } = useApi<ApiList<ServiceTypeItem>>('/api/admin/service-types?v=1.1.31', reloadKey, setAuthRequired);
...
const USER_BUTTON_FIELDS = [
  { key: 'buy', label: 'Buy configuration' },
  { key: 'my_services', label: 'My configurations' },
  { key: 'account', label: 'User account' },
  { key: 'test_account', label: 'Test account' },
  { key: 'tickets', label: 'Tickets' },
  { key: 'referral', label: 'Referral' },
  { key: 'query', label: 'Configuration lookup' },
  { key: 'reseller_request', label: 'Reseller request' },
  { key: 'reseller_menu', label: 'Reseller menu' },
  { key: 'admin', label: 'Bot administration' },
  { key: 'wallet_topup', label: 'Wallet top-up' },
] as const;

function settingsGeneralForm(map: Record<string, string>): ModalForm {
  return {
    title: 'General Settings', action: '/admin/settings/save',
    defaults: { bot_name: map.bot_name || 'D BOT', support: map.support_username || '@support', description: map.admin_description || '' },
    fields: [
      { name: 'bot_name', label: 'Bot name', required: true },
      { name: 'support', label: 'Support username' },
      { name: 'description', label: 'Bot description', type: 'textarea', full: true },
    ],
  };
}
...
function settingsButtonsForm(map: Record<string, string>): ModalForm {
  const defaults: Record<string, string> = {};
  const fields: FieldConfig[] = [];
  for (const item of USER_BUTTON_FIELDS) {
    const textKey = `button_${item.key}_text`;
    const enabledKey = `button_${item.key}_enabled`;
    defaults[textKey] = map[textKey] || '';
    defaults[enabledKey] = map[enabledKey] || '1';
    fields.push({ name: textKey, label: `${item.label} — button name`, required: true });
    fields.push({
      name: enabledKey, label: `${item.label} — status`, type: 'select', required: true,
      options: [{ value: '1', label: 'Enabled' }, { value: '0', label: 'Disabled' }],
    });
  }
  return { title: 'Bottom — User Buttons', action: '/admin/settings/buttons', defaults, fields };
}
