...
type ResellerItem = { id: number; user: UserItem; server_id?: number | null; inbound_mode?: 'automatic' | 'manual'; inbound_ids?: number[]; total_bytes: number; used_bytes: number; reserved_bytes: number; remaining_bytes?: number; expires_at?: string; is_active: boolean; created_at?: string };
...
function ResellersSection({ query, reloadKey, setAuthRequired, openModal, runAction, show }: any) {
  ...
  if (loading || servers.loading) return <SkeletonGrid />;
...
function resellerForm(r?: ResellerItem, servers: ServerItem[] = []): ModalForm {
  ...
  const defaults = r ? { total_gb: Math.round(r.total_bytes / 1024 ** 3), expires_at: expiryDate, server_id: r.server_id || 0, inbound_mode: r.inbound_mode || 'automatic', inbound_ids: r.inbound_ids || [] } : { total_gb: 0, days: 30 };
  return {
    ...
    fields: r ? [
      ...
      { name: 'server_id', label: 'Assigned server', type: 'select', options: serverOptions(servers), hidden: true },
