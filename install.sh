...
collect_config(){
  ...
  DOMAIN_NAME="$(ask_required 'Enter website domain, example.com: ')"
  ...
  WEB_ADMIN_PASSWORD="$(openssl rand -base64 24 | tr -d '/+=' | cut -c1-24)"
  FERNET_KEY="$(python3 - <<'PYKEY'
import base64, os
...
case "${1:-menu}" in
  ...
  credentials|info)
    ...
    echo "Darvish D Bot Web Admin Access"
    echo "Login URL          : https://${DOMAIN_NAME:-your-domain.com}/login"
    echo "Alternative URL    : http://${DOMAIN_NAME:-your-domain.com}:${API_PORT:-8000}/login"
    echo "Web Admin Username : ${WEB_ADMIN_USERNAME:-admin}"
    echo "Web Admin Password : ${WEB_ADMIN_PASSWORD:-change_this_admin_password}"
    echo "Role               : Owner"
    ;;
  ...
  menu|*)
    ...
    echo "dbot credentials           Show web admin login URL, username and password"
...
show_web_credentials(){
  ...
  echo "        Darvish D Bot Web Admin Access"
  echo "================================================"
  echo "Login URL          : https://${DOMAIN_DISPLAY}/login"
  echo "Alternative URL    : http://${DOMAIN_DISPLAY}:${API_PORT:-8000}/login"
...
setup_https_admin(){
  ...
  info "Configuring nginx reverse proxy on 443 for https://${DOMAIN_NAME}/admin ..."
...
EOFNGINX
  ...
  certbot --nginx -d "$DOMAIN_NAME" --non-interactive --agree-tos --register-unsafely-without-email --redirect || warn "Certbot failed. Make sure DNS points to this VPS and ports 80/443 are open."
