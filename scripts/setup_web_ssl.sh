...
if [ "$NGINX_HTTP_PORT" != "80" ] || [ "$NGINX_HTTPS_PORT" != "443" ]; then
  log "WARNING: automatic Let’s Encrypt normally requires public ports 80 and 443. Custom Nginx ports may require manual SSL or proxy/firewall rules."
fi
...
if command -v curl >/dev/null 2>&1; then
  LOCAL_TEST="$(curl -fsS --max-time 5 "http://127.0.0.1:${NGINX_HTTP_PORT}/.well-known/acme-challenge/$TEST_FILE" -H "Host: ${DOMAIN}" 2>/dev/null || true)"
...
2) Public port 80 must be open from the internet for Let’s Encrypt HTTP validation.
