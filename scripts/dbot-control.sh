#!/usr/bin/env bash
set -e
APP_DIR="${DBOT_APP_DIR:-/opt/d-bot}"
BACKUP_KEEP_DIR="${DBOT_BACKUP_KEEP_DIR:-/root/d-bot-backups}"
ENV_FILE="$APP_DIR/.env"

if docker compose version >/dev/null 2>&1; then COMPOSE="docker compose"; else COMPOSE="docker-compose"; fi

C_GREEN='\033[1;32m'; C_BLUE='\033[1;34m'; C_YELLOW='\033[1;33m'; C_RED='\033[1;31m'; C_CYAN='\033[1;36m'; C_MAGENTA='\033[1;35m'; C_WHITE='\033[1;37m'; C_BOLD='\033[1m'; C_DIM='\033[2m'; C_NC='\033[0m'

cd_app(){
  [ -d "$APP_DIR" ] || { echo -e "${C_RED}D Bot is not installed at $APP_DIR${C_NC}"; exit 1; }
  cd "$APP_DIR"
}

load_env(){
  if [ -f "$ENV_FILE" ]; then
    set -a
    # shellcheck disable=SC1090
    . "$ENV_FILE"
    set +a
  fi
}

mask_value(){
  local v="${1:-}"
  if [ -z "$v" ]; then echo "not set"; return; fi
  local len=${#v}
  if [ "$len" -le 8 ]; then echo "********"; return; fi
  echo "${v:0:4}********${v: -4}"
}

line(){ printf '%b\n' "${C_CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${C_NC}"; }

header(){
  clear || true
  printf '%b\n' "${C_CYAN}╔══════════════════════════════════════════════════════════════╗${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC}                    ${C_GREEN}D Bot Control Center${C_NC}                    ${C_CYAN}║${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC}        ${C_DIM}Setup viewer, editor and VPS service manager${C_NC}        ${C_CYAN}║${C_NC}"
  printf '%b\n' "${C_CYAN}╚══════════════════════════════════════════════════════════════╝${C_NC}"
}

pause(){ echo; read -r -p "Press Enter to continue..." _ || true; }

confirm_action(){
  local prompt="$1" answer=""
  read -r -p "$prompt [y/N]: " answer || true
  case "$answer" in y|Y|yes|YES) return 0 ;; *) return 1 ;; esac
}

require_env(){
  [ -f "$ENV_FILE" ] || { echo -e "${C_RED}Missing .env: $ENV_FILE${C_NC}"; exit 1; }
}

set_env_value(){
  local key="$1" value="$2"
  require_env
  python3 - "$ENV_FILE" "$key" "$value" <<'PYENV'
import sys
from pathlib import Path
path = Path(sys.argv[1])
key = sys.argv[2]
value = sys.argv[3]
lines = path.read_text(encoding='utf-8').splitlines()
out = []
written = False
for line in lines:
    if line.startswith(key + '='):
        if not written:
            out.append(f'{key}={value}')
            written = True
        continue
    out.append(line)
if not written:
    out.append(f'{key}={value}')
path.write_text('\n'.join(out).rstrip() + '\n', encoding='utf-8')
PYENV
  chmod 600 "$ENV_FILE"
}

get_env_value(){
  local key="$1"
  [ -f "$ENV_FILE" ] || return 0
  python3 - "$ENV_FILE" "$key" <<'PYGET'
import sys
from pathlib import Path
path = Path(sys.argv[1])
key = sys.argv[2]
for line in path.read_text(encoding='utf-8').splitlines():
    if line.startswith(key + '='):
        print(line.split('=', 1)[1])
        break
PYGET
}

valid_domain(){
  local d="$1"
  [[ "$d" =~ ^[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?(\.[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?)+$ ]]
}

normalize_domain(){
  local raw="$1"
  raw="${raw#http://}"; raw="${raw#https://}"; raw="${raw%%/*}"; raw="${raw%%:*}"
  echo "$raw"
}

valid_port(){
  local p="$1"
  [[ "$p" =~ ^[0-9]+$ ]] && [ "$p" -ge 1 ] && [ "$p" -le 65535 ]
}

valid_ids(){
  local ids="$1"
  [[ "$ids" =~ ^[0-9]+([,[:space:]]*[0-9]+)*$ ]]
}

valid_bot_token(){
  local token="$1"
  [[ "$token" =~ ^[0-9]{6,}:[A-Za-z0-9_-]{20,}$ ]]
}

port_suffix(){
  local scheme="$1" p="$2"
  if [ "$scheme" = "https" ] && [ "$p" != "443" ]; then echo ":$p"; return; fi
  if [ "$scheme" = "http" ] && [ "$p" != "80" ]; then echo ":$p"; return; fi
  echo ""
}

https_redirect_target(){
  local https_port="${1:-443}"
  if [ "$https_port" = "443" ]; then
    echo 'https://$host$request_uri'
  else
    echo "https://$server_name:${https_port}"'$request_uri'
  fi
}


ask_nonempty(){
  local prompt="$1" value=""
  while [ -z "$value" ]; do read -r -p "$prompt" value || true; done
  echo "$value"
}

ask_secret_nonempty(){
  local prompt="$1" value=""
  while [ -z "$value" ]; do read -r -s -p "$prompt" value; echo >&2; done
  echo "$value"
}

ask_default(){
  local prompt="$1" default="$2" value=""
  read -r -p "$prompt [$default]: " value || true
  echo "${value:-$default}"
}

ask_yes_no(){
  local prompt="$1" default="${2:-y}" value="" hint="Y/n"
  [ "$default" = "n" ] && hint="y/N"
  while true; do
    read -r -p "$prompt [$hint]: " value || true
    value="${value:-$default}"
    case "$value" in y|Y|yes|YES) echo "true"; return ;; n|N|no|NO) echo "false"; return ;; *) echo "Please answer y or n." ;; esac
  done
}

generate_password(){ openssl rand -base64 32 | tr -d '/+=' | cut -c1-24; }

credentials_cli_read(){
  cd_app
  local out=""
  if out="$($COMPOSE exec -T api python scripts/web_credentials_cli.py "$@" 2>/dev/null)"; then
    printf '%s' "$out"
    return 0
  fi
  return 1
}

credentials_cli_write(){
  cd_app
  # Credential changes are database-backed. Make sure the API and its
  # dependencies are running, then execute the updater inside the trusted image.
  $COMPOSE up -d db redis api >/dev/null
  $COMPOSE exec -T api python scripts/web_credentials_cli.py "$@"
}

load_live_web_credentials(){
  load_env
  local payload=""
  LIVE_WEB_USERNAME="${WEB_ADMIN_USERNAME:-admin}"
  LIVE_WEB_PASSWORD="${WEB_ADMIN_PASSWORD:-}"
  LIVE_WEB_PASSWORD_AVAILABLE="false"
  [ -n "$LIVE_WEB_PASSWORD" ] && LIVE_WEB_PASSWORD_AVAILABLE="true"
  LIVE_WEB_UPDATED_AT=""
  LIVE_WEB_UPDATED_BY=""
  LIVE_WEB_SOURCE="environment-fallback"

  if payload="$(credentials_cli_read show --json)" && [ -n "$payload" ]; then
    local assignments=""
    assignments="$(python3 - "$payload" <<'PYCREDS'
import json, shlex, sys
obj = json.loads(sys.argv[1])
values = {
    'LIVE_WEB_USERNAME': str(obj.get('username') or 'admin'),
    'LIVE_WEB_PASSWORD': str(obj.get('password') or ''),
    'LIVE_WEB_PASSWORD_AVAILABLE': 'true' if obj.get('password_available') else 'false',
    'LIVE_WEB_UPDATED_AT': str(obj.get('updated_at') or ''),
    'LIVE_WEB_UPDATED_BY': str(obj.get('updated_by') or ''),
    'LIVE_WEB_SOURCE': str(obj.get('source') or 'database'),
}
for key, value in values.items():
    print(f'{key}={shlex.quote(value)}')
PYCREDS
)"
    eval "$assignments"
  fi
}

credentials_status_label(){
  case "${LIVE_WEB_SOURCE:-}" in
    database|legacy-database) echo "LIVE DATABASE" ;;
    environment-fallback) echo "ENV FALLBACK" ;;
    hash-only) echo "PASSWORD RESET NEEDED" ;;
    database-secret-unreadable) echo "SECRET KEY ERROR" ;;
    *) echo "NOT CONFIGURED" ;;
  esac
}

box_top(){ printf '%b\n' "${C_CYAN}╭────────────────────────────────────────────────────────────────────────────╮${C_NC}"; }
box_bottom(){ printf '%b\n' "${C_CYAN}╰────────────────────────────────────────────────────────────────────────────╯${C_NC}"; }
box_title(){
  local title="$1"
  printf '%b' "${C_CYAN}│${C_NC} ${C_BOLD}"
  printf '%-74.74s' "$title"
  printf '%b\n' "${C_NC} ${C_CYAN}│${C_NC}"
}
box_menu_row(){
  local text="$1"
  printf '%b' "${C_CYAN}│${C_NC} "
  printf '%-74.74s' "$text"
  printf '%b\n' " ${C_CYAN}│${C_NC}"
}
box_row(){
  local label="$1" value="${2:-not set}" color="${3:-$C_WHITE}"
  [ -n "$value" ] || value="not set"
  printf '%b' "${C_CYAN}│${C_NC} ${C_DIM}"
  printf '%-74.74s' "$label"
  printf '%b\n' "${C_NC} ${C_CYAN}│${C_NC}"
  # Always terminate the value with a newline. Without it, `read` reaches EOF
  # before executing the loop body for a single-line value, so secrets looked
  # hidden even though the values were loaded correctly.
  printf '%s\n' "$value" | fold -w 71 -s | while IFS= read -r part; do
    printf '%b' "${C_CYAN}│${C_NC}   ${color}"
    printf '%-71.71s' "$part"
    printf '%b\n' "${C_NC}  ${C_CYAN}│${C_NC}"
  done
}

refresh_credentials_services(){
  cd_app
  $COMPOSE restart api >/dev/null
  echo -e "${C_GREEN}Website authentication refreshed and previous web sessions were closed.${C_NC}"
}

panel_url(){
  load_env
  local domain="${DOMAIN_NAME:-not-set}" api_port="${API_PORT:-8000}" http_port="${NGINX_HTTP_PORT:-80}" https_port="${NGINX_HTTPS_PORT:-443}"
  if [ "${ENABLE_HTTPS:-false}" = "true" ] && [ "$domain" != "not-set" ]; then
    echo "https://${domain}$(port_suffix https "$https_port")/login"
  elif [ "$domain" != "not-set" ]; then
    echo "http://${domain}$(port_suffix http "$http_port")/login"
  else
    echo "http://SERVER_IP:${api_port}/login"
  fi
}

service_card(){
  load_env
  local domain="${DOMAIN_NAME:-not-set}" port="${API_PORT:-8000}" https="${ENABLE_HTTPS:-false}" http_port="${NGINX_HTTP_PORT:-80}" https_port="${NGINX_HTTPS_PORT:-443}"
  local url; url="$(panel_url)"
  printf '%b\n' "${C_CYAN}╔══════════════════════════════════════════════════════════════╗${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_GREEN}Project${C_NC} : D Bot"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_GREEN}Path${C_NC}    : ${APP_DIR}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_GREEN}Panel${C_NC}   : ${url}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_GREEN}Domain${C_NC}  : ${domain}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_GREEN}HTTPS${C_NC}   : ${https}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_GREEN}Nginx${C_NC}   : HTTP ${http_port} / HTTPS ${https_port}"
  printf '%b\n' "${C_CYAN}╚══════════════════════════════════════════════════════════════╝${C_NC}"
}

show_status(){
  header; service_card; echo
  printf '%b\n' "${C_BLUE}Docker services:${C_NC}"
  cd_app && $COMPOSE ps
}

show_setup_info(){
  header
  load_env
  load_live_web_credentials
  printf '%b\n' "${C_CYAN}╔══════════════════════════════════════════════════════════════╗${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC}                    ${C_GREEN}Saved Setup Information${C_NC}                 ${C_CYAN}║${C_NC}"
  printf '%b\n' "${C_CYAN}╚══════════════════════════════════════════════════════════════╝${C_NC}"
  echo
  printf '%b\n' "${C_YELLOW}Telegram${C_NC}"
  printf '  Bot Token        : %s\n' "$(mask_value "${BOT_TOKEN:-}")"
  printf '  Admin IDs        : %s\n' "${ADMIN_IDS:-not set}"
  printf '  Owner IDs        : %s\n' "${OWNER_IDS:-not set}"
  printf '  Channel URL      : %s\n' "${DEFAULT_CHANNEL_URL:-not set}"
  echo
  printf '%b\n' "${C_YELLOW}Website & SSL${C_NC}"
  printf '  Domain           : %s\n' "${DOMAIN_NAME:-not set}"
  printf '  HTTPS            : %s\n' "${ENABLE_HTTPS:-false}"
  printf '  SSL Email        : %s\n' "${LETSENCRYPT_EMAIL:-not set}"
  printf '  Web Login        : %s\n' "$(panel_url)"
  printf '  API Port         : %s\n' "${API_PORT:-8000}"
  printf '  Nginx HTTP Port  : %s\n' "${NGINX_HTTP_PORT:-80}"
  printf '  Nginx HTTPS Port : %s\n' "${NGINX_HTTPS_PORT:-443}"
  echo
  printf '%b\n' "${C_YELLOW}Web Admin — live database values${C_NC}"
  printf '  Username         : %s\n' "${LIVE_WEB_USERNAME:-not set}"
  printf '  Password         : %s\n' "$(mask_value "${LIVE_WEB_PASSWORD:-}")"
  printf '  Source           : %s\n' "$(credentials_status_label)"
  echo
  printf '%b\n' "${C_YELLOW}Database & Cache${C_NC}"
  printf '  Postgres DB      : %s\n' "${POSTGRES_DB:-d_bot}"
  printf '  Postgres User    : %s\n' "${POSTGRES_USER:-dbot}"
  printf '  Postgres Pass    : %s\n' "$(mask_value "${POSTGRES_PASSWORD:-}")"
  printf '  Postgres Host    : %s\n' "${POSTGRES_HOST:-db}"
  printf '  Postgres Port    : %s\n' "${POSTGRES_PORT:-5432}"
  printf '  Redis Host       : %s\n' "${REDIS_HOST:-redis}"
  printf '  Redis Port       : %s\n' "${REDIS_PORT:-6379}"
  echo
  printf '%b\n' "${C_YELLOW}Runtime${C_NC}"
  printf '  Timezone         : %s\n' "${TZ:-not set}"
  printf '  Server Sync      : %s seconds\n' "${SERVER_SYNC_SECONDS:-5}"
}

show_infrastructure_secrets(){
  header
  load_env
  box_top
  box_title "INFRASTRUCTURE SECRETS"
  box_row "Telegram bot token" "${BOT_TOKEN:-}" "$C_YELLOW"
  box_row "PostgreSQL password" "${POSTGRES_PASSWORD:-}" "$C_YELLOW"
  box_row "FERNET key" "${FERNET_KEY:-}" "$C_MAGENTA"
  box_row "JWT secret" "${JWT_SECRET:-}" "$C_MAGENTA"
  box_bottom
  echo
  echo -e "${C_RED}Do not copy these values into tickets, chats or screenshots.${C_NC}"
}

change_live_web_username(){
  load_live_web_credentials
  local value=""
  echo
  read -r -p "New web admin username [${LIVE_WEB_USERNAME:-admin}]: " value || true
  value="${value:-${LIVE_WEB_USERNAME:-admin}}"
  if ! credentials_cli_write set-username "$value" >/dev/null; then
    echo -e "${C_RED}Username update failed. Check the value and API logs.${C_NC}"
    return 1
  fi
  refresh_credentials_services
  echo -e "${C_GREEN}Username changed to: ${value}${C_NC}"
}

change_live_web_password(){
  local first="" second=""
  echo
  while true; do
    read -r -s -p "New web admin password (minimum 8 characters): " first || true; echo
    read -r -s -p "Repeat new password: " second || true; echo
    if [ "$first" != "$second" ]; then
      echo -e "${C_RED}Passwords do not match. Try again.${C_NC}"
      continue
    fi
    [ ${#first} -ge 8 ] || { echo -e "${C_RED}Password must contain at least 8 characters.${C_NC}"; continue; }
    break
  done
  if ! printf '%s' "$first" | credentials_cli_write set-password --stdin >/dev/null; then
    echo -e "${C_RED}Password update failed.${C_NC}"
    return 1
  fi
  refresh_credentials_services
  echo -e "${C_GREEN}Password changed and synchronized with the Credentials Center.${C_NC}"
}

generate_live_web_password(){
  local generated=""
  generated="$(credentials_cli_write generate-password --length 24)" || {
    echo -e "${C_RED}Password generation failed.${C_NC}"
    return 1
  }
  refresh_credentials_services
  echo
  box_top
  box_title "NEW GENERATED PASSWORD"
  box_row "Password" "$generated" "$C_GREEN"
  box_bottom
  echo -e "${C_YELLOW}Save it now. The Credentials Center can display it again later.${C_NC}"
}

credentials_center_screen(){
  header
  load_env
  load_live_web_credentials
  local password_display="${LIVE_WEB_PASSWORD:-}"
  [ "${LIVE_WEB_PASSWORD_AVAILABLE:-false}" = "true" ] || password_display="Not recoverable yet — change the password once to synchronize it"

  printf '%b\n' "${C_MAGENTA}╔════════════════════════════════════════════════════════════════════════════╗${C_NC}"
  printf '%b\n' "${C_MAGENTA}║${C_NC}                         ${C_WHITE}D BOT CREDENTIALS CENTER${C_NC}                         ${C_MAGENTA}║${C_NC}"
  printf '%b\n' "${C_MAGENTA}║${C_NC}              ${C_DIM}Live values from the shared D BOT database${C_NC}                 ${C_MAGENTA}║${C_NC}"
  printf '%b\n' "${C_MAGENTA}╚════════════════════════════════════════════════════════════════════════════╝${C_NC}"
  echo
  box_top
  box_title "WEBSITE OWNER ACCESS"
  box_row "Login URL" "$(panel_url)" "$C_CYAN"
  box_row "Username" "${LIVE_WEB_USERNAME:-admin}" "$C_GREEN"
  box_row "Password" "$password_display" "$C_GREEN"
  box_row "Sync status" "$(credentials_status_label)" "$C_YELLOW"
  box_row "Last updated" "${LIVE_WEB_UPDATED_AT:-Not recorded}" "$C_WHITE"
  box_row "Updated by" "${LIVE_WEB_UPDATED_BY:-Not recorded}" "$C_WHITE"
  box_bottom
  echo
  box_top
  box_title "QUICK ACTIONS"
  box_menu_row "1) Change web username        — update the live database immediately"
  box_menu_row "2) Change web password        — store hash + encrypted reveal copy"
  box_menu_row "3) Generate secure password    — create and apply a strong password"
  box_menu_row "4) Refresh live credentials    — reload current values from database"
  box_menu_row "5) Infrastructure secrets     — token, database and crypto keys"
  box_menu_row "0) Back"
  box_bottom
}

credentials_center(){
  if ! confirm_action "Open the Credentials Center and reveal the current website password?"; then return; fi
  while true; do
    credentials_center_screen
    echo
    read -r -p "Select an action: " c || true
    case "$c" in
      1|username) change_live_web_username; pause ;;
      2|password) change_live_web_password; pause ;;
      3|generate) generate_live_web_password; pause ;;
      4|refresh) continue ;;
      5|infrastructure|keys) show_infrastructure_secrets; pause ;;
      0|b|B|back|q|Q) return ;;
      *) echo -e "${C_RED}Invalid option.${C_NC}"; sleep 1 ;;
    esac
  done
}

reveal_secrets(){ credentials_center; }

restart_after_change(){
  echo
  if confirm_action "Restart D Bot services now to apply changes?"; then
    cd_app && $COMPOSE up -d
    echo -e "${C_GREEN}Services restarted/applied.${C_NC}"
  else
    echo -e "${C_YELLOW}Changes were saved. Run 'dbot restart' later.${C_NC}"
  fi
}

configure_nginx_ssl(){
  load_env
  local domain="${DOMAIN_NAME:-}" port="${API_PORT:-8000}" http_port="${NGINX_HTTP_PORT:-80}" https_port="${NGINX_HTTPS_PORT:-443}"
  local ssl_script="$APP_DIR/scripts/setup_web_ssl.sh"

  if [ -z "$domain" ] || [ "$domain" = "not-set" ]; then
    echo -e "${C_RED}DOMAIN_NAME is empty. Set domain first.${C_NC}"
    return 1
  fi
  if ! valid_port "$http_port" || ! valid_port "$https_port" || ! valid_port "$port"; then
    echo -e "${C_RED}Invalid port value in .env.${C_NC}"
    return 1
  fi

  if [ "${ENABLE_HTTPS:-true}" = "true" ]; then
    [ -f "$ssl_script" ] || { echo -e "${C_RED}Missing $ssl_script${C_NC}"; return 1; }
    chmod 755 "$ssl_script"

    if [ ! -f "/etc/letsencrypt/live/${domain}/fullchain.pem" ]; then
      echo -e "${C_YELLOW}No certificate exists for ${domain}; starting the safe HTTPS bootstrap flow first.${C_NC}"
      DOMAIN_NAME="$domain" LETSENCRYPT_EMAIL="${LETSENCRYPT_EMAIL:-}" \
      NGINX_HTTP_PORT="$http_port" NGINX_HTTPS_PORT="$https_port" \
        bash "$ssl_script" "$domain" bootstrap || return 1
    fi

    if ! curl -fsS --max-time 5 "http://127.0.0.1:${port}/health" >/dev/null 2>&1; then
      echo -e "${C_RED}The D Bot API is not healthy on 127.0.0.1:${port}.${C_NC}"
      echo -e "${C_YELLOW}Nginx was not switched to the API, so users will not see a 502 page.${C_NC}"
      echo "Fix/restart the API, then run this option again."
      return 1
    fi

    DOMAIN_NAME="$domain" LETSENCRYPT_EMAIL="${LETSENCRYPT_EMAIL:-}" API_PORT="$port" \
    NGINX_HTTP_PORT="$http_port" NGINX_HTTPS_PORT="$https_port" \
      bash "$ssl_script" "$domain" proxy
    return $?
  fi

  if ! command -v nginx >/dev/null 2>&1; then
    echo -e "${C_YELLOW}nginx is not installed.${C_NC}"
    return 1
  fi
  cat > /etc/nginx/sites-available/d-bot.conf <<EOFNGINX
server {
    listen ${http_port};
    server_name ${domain};
    client_max_body_size 100m;
    location / {
        proxy_pass http://127.0.0.1:${port};
        proxy_http_version 1.1;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto \$scheme;
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_read_timeout 120s;
    }
}
EOFNGINX
  ln -sf /etc/nginx/sites-available/d-bot.conf /etc/nginx/sites-enabled/d-bot.conf
  nginx -t && systemctl reload nginx
}

edit_telegram(){
  header
  load_env
  echo -e "${C_YELLOW}Telegram Settings${C_NC}"
  echo "1) Change bot token"
  echo "2) Change admin/owner IDs"
  echo "3) Change default channel URL"
  echo "0) Back"
  read -r -p "Select: " c || true
  case "$c" in
    1)
      echo "The new Telegram Bot Token will be visible while typing."
      while true; do
        v="$(ask_nonempty 'New Telegram Bot Token: ')"
        valid_bot_token "$v" && break
        echo "Token format looks invalid. Example: 123456789:AAExample_Token-Value"
      done
      set_env_value BOT_TOKEN "$v"
      ;;
    2)
      while true; do
        v="$(ask_nonempty 'Admin Telegram IDs, comma separated: ')"; v="$(echo "$v" | tr -d ' ')"
        valid_ids "$v" && break
        echo "Invalid IDs. Example: 123456789 or 123456789,987654321"
      done
      set_env_value ADMIN_IDS "$v"; set_env_value OWNER_IDS "$v" ;;
    3) read -r -p "Default Telegram channel URL, empty to clear: " v || true; set_env_value DEFAULT_CHANNEL_URL "$v" ;;
    0) return ;;
  esac
  restart_after_change
}

edit_website(){
  header
  load_env
  echo -e "${C_YELLOW}Website & SSL Settings${C_NC}"
  echo "1) Change domain"
  echo "2) Toggle HTTPS"
  echo "3) Change Let's Encrypt email"
  echo "4) Change internal API port"
  echo "5) Change Nginx HTTP port"
  echo "6) Change Nginx HTTPS port"
  echo "7) Apply Nginx/SSL config"
  echo "0) Back"
  read -r -p "Select: " c || true
  case "$c" in
    1)
      while true; do
        v="$(ask_nonempty 'New domain, example: panel.example.com: ')"; v="$(normalize_domain "$v")"
        valid_domain "$v" && break
        echo "Invalid domain."
      done
      set_env_value DOMAIN_NAME "$v" ;;
    2) v="$(ask_yes_no 'Enable HTTPS?' 'y')"; set_env_value ENABLE_HTTPS "$v" ;;
    3) read -r -p "Let's Encrypt email, empty to clear: " v || true; set_env_value LETSENCRYPT_EMAIL "$v" ;;
    4)
      while true; do
        v="$(ask_default 'Internal API port' "${API_PORT:-8000}")"
        valid_port "$v" && break
        echo "Port must be between 1 and 65535."
      done
      set_env_value API_PORT "$v" ;;
    5)
      while true; do
        v="$(ask_default 'Nginx HTTP listen port' "${NGINX_HTTP_PORT:-80}")"
        valid_port "$v" && break
        echo "Port must be between 1 and 65535."
      done
      set_env_value NGINX_HTTP_PORT "$v" ;;
    6)
      while true; do
        v="$(ask_default 'Nginx HTTPS listen port' "${NGINX_HTTPS_PORT:-443}")"
        valid_port "$v" && break
        echo "Port must be between 1 and 65535."
      done
      set_env_value NGINX_HTTPS_PORT "$v" ;;
    7) configure_nginx_ssl; pause; return ;;
    0) return ;;
  esac
  configure_nginx_ssl || true
  restart_after_change
}

edit_web_admin(){
  credentials_center
}

edit_runtime(){
  header
  load_env
  echo -e "${C_YELLOW}Runtime Settings${C_NC}"
  echo "1) Change timezone"
  echo "2) Change server sync seconds"
  echo "0) Back"
  read -r -p "Select: " c || true
  case "$c" in
    1) v="$(ask_default 'Timezone' "${TZ:-Asia/Tehran}")"; set_env_value TZ "$v" ;;
    2)
      while true; do
        v="$(ask_default 'Server sync seconds' "${SERVER_SYNC_SECONDS:-5}")"
        [[ "$v" =~ ^[0-9]+$ ]] && [ "$v" -ge 1 ] && break
        echo "Enter a positive number."
      done
      set_env_value SERVER_SYNC_SECONDS "$v" ;;
    0) return ;;
  esac
  restart_after_change
}

edit_database(){
  header
  load_env
  echo -e "${C_RED}Advanced database settings${C_NC}"
  echo "Changing database values after data exists can break the app unless you migrate the database or recreate volumes."
  echo "A backup is recommended before changing these values."
  if ! confirm_action "Continue anyway?"; then return; fi
  echo "1) Change database name"
  echo "2) Change database username"
  echo "3) Change database password"
  echo "4) Change database host/port"
  echo "0) Back"
  read -r -p "Select: " c || true
  case "$c" in
    1) v="$(ask_default 'PostgreSQL database name' "${POSTGRES_DB:-d_bot}")"; set_env_value POSTGRES_DB "$v" ;;
    2) v="$(ask_default 'PostgreSQL username' "${POSTGRES_USER:-dbot}")"; set_env_value POSTGRES_USER "$v" ;;
    3) v="$(ask_secret_nonempty 'PostgreSQL password: ')"; set_env_value POSTGRES_PASSWORD "$v" ;;
    4)
      h="$(ask_default 'PostgreSQL host' "${POSTGRES_HOST:-db}")"
      p="$(ask_default 'PostgreSQL port' "${POSTGRES_PORT:-5432}")"
      set_env_value POSTGRES_HOST "$h"; set_env_value POSTGRES_PORT "$p" ;;
    0) return ;;
  esac
  # Keep DATABASE_URL aligned with the new values.
  load_env
  dburl="postgresql+asyncpg://${POSTGRES_USER:-dbot}:${POSTGRES_PASSWORD:-dbot}@${POSTGRES_HOST:-db}:${POSTGRES_PORT:-5432}/${POSTGRES_DB:-d_bot}"
  set_env_value DATABASE_URL "$dburl"
  restart_after_change
}

edit_setup_menu(){
  while true; do
    header
    echo -e "${C_YELLOW}Edit saved setup values${C_NC}"
    echo
    echo "1) Telegram bot settings"
    echo "2) Website, domain, HTTPS and port"
    echo "3) Web admin username/password"
    echo "4) Runtime settings"
    echo "5) Database settings (advanced)"
    echo "0) Back"
    echo
    read -r -p "Select: " c || true
    case "$c" in
      1) edit_telegram ;;
      2) edit_website ;;
      3) edit_web_admin ;;
      4) edit_runtime ;;
      5) edit_database ;;
      0|q|Q) return ;;
      *) echo -e "${C_RED}Invalid option.${C_NC}"; sleep 1 ;;
    esac
  done
}

install_manager_self(){
  local repair="$APP_DIR/scripts/repair-dbot-cli.sh"
  local launcher="$APP_DIR/scripts/dbot-launcher.sh"
  local control="$APP_DIR/scripts/dbot-control.sh"

  # Execute the repair helper from the freshly updated checkout. Keeping this
  # logic outside the currently-running CLI prevents stale function bodies
  # from reinstalling another stale copy after `git reset`.
  if [ -f "$repair" ]; then
    chmod 755 "$repair"
    bash "$repair"
    return 0
  fi

  local target=""
  if [ -f "$launcher" ]; then
    target="$launcher"
  elif [ -f "$control" ]; then
    target="$control"
  else
    echo -e "${C_RED}Cannot update dbot command: control script is missing.${C_NC}"
    return 1
  fi
  chmod 755 "$target"
  rm -f /usr/local/bin/dbot /usr/local/bin/d-bot
  ln -s "$target" /usr/local/bin/dbot
  ln -s /usr/local/bin/dbot /usr/local/bin/d-bot
  hash -r 2>/dev/null || true
  echo -e "${C_GREEN}Manager command updated: dbot -> $target${C_NC}"
}

backup_item_epoch(){
  local path="$1" base stamp y m d hh mm ss parsed=""
  base="$(basename -- "$path")"
  if [[ "$base" =~ ([0-9]{8})_([0-9]{6}) ]]; then
    stamp="${BASH_REMATCH[1]}${BASH_REMATCH[2]}"
    y="${stamp:0:4}"; m="${stamp:4:2}"; d="${stamp:6:2}"
    hh="${stamp:8:2}"; mm="${stamp:10:2}"; ss="${stamp:12:2}"
    parsed="$(date -d "$y-$m-$d $hh:$mm:$ss" +%s 2>/dev/null || true)"
  fi
  if [ -n "$parsed" ]; then
    printf '%s\n' "$parsed"
  else
    stat -c '%Y' -- "$path" 2>/dev/null || printf '0\n'
  fi
}

backup_item_size(){
  local path="$1"
  du -sh -- "$path" 2>/dev/null | awk '{print $1}' || printf '?\n'
}

backup_item_bytes(){
  local path="$1"
  du -s -B1 -- "$path" 2>/dev/null | awk '{print $1}' || printf '0\n'
}

backup_item_source(){
  local path="$1"
  case "$path" in
    "$APP_DIR"/backups/*) printf 'current\n' ;;
    "$BACKUP_KEEP_DIR"/*) printf 'archived\n' ;;
    *) printf 'unknown\n' ;;
  esac
}

emit_backup_candidate(){
  local path="$1" epoch
  [ -e "$path" ] || return 0
  [ -L "$path" ] && return 0
  epoch="$(backup_item_epoch "$path")"
  printf '%s\t%s\n' "${epoch:-0}" "$path"
}

collect_backup_candidates(){
  local item group children=()
  shopt -s nullglob

  if [ -d "$APP_DIR/backups" ]; then
    for item in "$APP_DIR/backups"/*; do
      emit_backup_candidate "$item"
    done
  fi

  if [ -d "$BACKUP_KEEP_DIR" ]; then
    for group in "$BACKUP_KEEP_DIR"/*; do
      [ -e "$group" ] || continue
      if [ -d "$group" ] && [[ "$(basename -- "$group")" == backups-* ]]; then
        children=("$group"/*)
        if [ "${#children[@]}" -gt 0 ]; then
          for item in "${children[@]}"; do
            emit_backup_candidate "$item"
          done
        else
          emit_backup_candidate "$group"
        fi
      else
        emit_backup_candidate "$group"
      fi
    done
  fi

  shopt -u nullglob
}

refresh_backup_inventory(){
  BACKUP_PATHS=()
  BACKUP_EPOCHS=()
  BACKUP_NAMES=()
  BACKUP_DATES=()
  BACKUP_SIZES=()
  BACKUP_BYTES=()
  BACKUP_SOURCES=()
  BACKUP_TOTAL_BYTES=0

  local epoch path name human_date size bytes source
  while IFS=$'\t' read -r epoch path; do
    [ -n "$path" ] || continue
    [ -e "$path" ] || continue
    name="$(basename -- "$path")"
    human_date="$(date -d "@${epoch:-0}" '+%Y-%m-%d %H:%M:%S' 2>/dev/null || printf 'Unknown')"
    size="$(backup_item_size "$path")"
    bytes="$(backup_item_bytes "$path")"
    source="$(backup_item_source "$path")"

    BACKUP_PATHS+=("$path")
    BACKUP_EPOCHS+=("$epoch")
    BACKUP_NAMES+=("$name")
    BACKUP_DATES+=("$human_date")
    BACKUP_SIZES+=("$size")
    BACKUP_BYTES+=("$bytes")
    BACKUP_SOURCES+=("$source")
    BACKUP_TOTAL_BYTES=$((BACKUP_TOTAL_BYTES + bytes))
  done < <(collect_backup_candidates | sort -n -k1,1)
}

backup_total_human(){
  numfmt --to=iec-i --suffix=B "${BACKUP_TOTAL_BYTES:-0}" 2>/dev/null || printf '%s bytes\n' "${BACKUP_TOTAL_BYTES:-0}"
}

show_backup_inventory(){
  refresh_backup_inventory
  local count="${#BACKUP_PATHS[@]}" i
  echo -e "${C_YELLOW}Saved D Bot backups${C_NC}"
  echo -e "${C_DIM}Ordered from oldest to newest. Only D Bot backup directories are scanned.${C_NC}"
  echo
  if [ "$count" -eq 0 ]; then
    echo -e "${C_GREEN}No saved D Bot backups were found.${C_NC}"
    return 1
  fi

  printf '%-4s %-38s %-19s %-9s %-9s\n' 'No.' 'Backup name' 'Date' 'Size' 'Location'
  printf '%-4s %-38s %-19s %-9s %-9s\n' '---' '--------------------------------------' '-------------------' '---------' '---------'
  for i in "${!BACKUP_PATHS[@]}"; do
    printf '%-4s %-38.38s %-19s %-9s %-9s\n' \
      "$((i + 1)))" "${BACKUP_NAMES[$i]}" "${BACKUP_DATES[$i]}" "${BACKUP_SIZES[$i]}" "${BACKUP_SOURCES[$i]}"
  done
  echo
  echo -e "Backups: ${C_WHITE}${count}${C_NC}    Disk used: ${C_WHITE}$(backup_total_human)${C_NC}"
  return 0
}

backup_path_is_safe(){
  local target="$1" resolved="" app_root="" keep_root=""
  [ -e "$target" ] || return 1
  [ -L "$target" ] && return 1
  resolved="$(realpath -- "$target" 2>/dev/null || true)"
  app_root="$(realpath -m -- "$APP_DIR/backups" 2>/dev/null || true)"
  keep_root="$(realpath -m -- "$BACKUP_KEEP_DIR" 2>/dev/null || true)"
  [ -n "$resolved" ] || return 1
  case "$resolved" in
    "$app_root"/*|"$keep_root"/*) return 0 ;;
    *) return 1 ;;
  esac
}

cleanup_empty_backup_dirs(){
  if [ -d "$APP_DIR/backups" ]; then
    find "$APP_DIR/backups" -mindepth 1 -depth -type d -empty -delete 2>/dev/null || true
  fi
  if [ -d "$BACKUP_KEEP_DIR" ]; then
    find "$BACKUP_KEEP_DIR" -mindepth 1 -depth -type d -empty -delete 2>/dev/null || true
  fi
}

delete_backup_path(){
  local target="$1"
  if ! backup_path_is_safe "$target"; then
    echo -e "${C_RED}Refusing to delete an unsafe or unknown path.${C_NC}"
    return 1
  fi
  rm -rf -- "$target"
  cleanup_empty_backup_dirs
}

delete_all_backups(){
  refresh_backup_inventory
  local count="${#BACKUP_PATHS[@]}" target deleted=0 failed=0
  if [ "$count" -eq 0 ]; then
    echo -e "${C_GREEN}There are no backups to delete.${C_NC}"
    return 0
  fi

  echo -e "${C_RED}This will permanently delete all ${count} listed D Bot backups ($(backup_total_human)).${C_NC}"
  local confirm=""
  read -r -p 'Type DELETE ALL to continue: ' confirm || true
  [ "$confirm" = 'DELETE ALL' ] || { echo 'Delete all cancelled.'; return 0; }

  for target in "${BACKUP_PATHS[@]}"; do
    if delete_backup_path "$target"; then
      deleted=$((deleted + 1))
    else
      failed=$((failed + 1))
    fi
  done
  echo -e "${C_GREEN}Deleted ${deleted} backup item(s).${C_NC}"
  [ "$failed" -eq 0 ] || echo -e "${C_RED}${failed} item(s) could not be deleted.${C_NC}"
}

backup_manager(){
  local choice="" idx target
  while true; do
    header
    echo -e "${C_BOLD}${C_YELLOW}Delete Old Backup${C_NC}"
    echo
    show_backup_inventory || true
    echo
    echo -e "${C_RED}A) Delete All Backups${C_NC}"
    echo '0) Back'
    echo
    read -r -p 'Select a backup number to delete: ' choice || true

    case "$choice" in
      0|q|Q|back) return ;;
      a|A|all|ALL)
        echo
        delete_all_backups
        pause
        ;;
      ''|*[!0-9]*)
        echo -e "${C_RED}Invalid option.${C_NC}"
        sleep 1
        ;;
      *)
        refresh_backup_inventory
        idx=$((choice - 1))
        if [ "$choice" -lt 1 ] || [ "$idx" -ge "${#BACKUP_PATHS[@]}" ]; then
          echo -e "${C_RED}Backup number not found.${C_NC}"
          sleep 1
          continue
        fi
        target="${BACKUP_PATHS[$idx]}"
        echo
        echo -e "Selected: ${C_WHITE}${BACKUP_NAMES[$idx]}${C_NC}"
        echo "Date:     ${BACKUP_DATES[$idx]}"
        echo "Size:     ${BACKUP_SIZES[$idx]}"
        echo "Location: ${BACKUP_SOURCES[$idx]}"
        echo
        if confirm_action 'Permanently delete this backup?'; then
          if delete_backup_path "$target"; then
            echo -e "${C_GREEN}Backup deleted.${C_NC}"
          else
            echo -e "${C_RED}Backup could not be deleted.${C_NC}"
          fi
        else
          echo 'Deletion cancelled.'
        fi
        pause
        ;;
    esac
  done
}

backup_app(){
  cd_app
  TS="$(date +%Y%m%d_%H%M%S)"
  BACKUP_DIR="$APP_DIR/backups/$TS"
  mkdir -p "$BACKUP_DIR"
  load_env
  DB_NAME="${POSTGRES_DB:-d_bot}"
  DB_USER="${POSTGRES_USER:-dbot}"
  echo "Creating database backup..."
  if $COMPOSE ps db >/dev/null 2>&1; then
    $COMPOSE exec -T db pg_dump -U "$DB_USER" "$DB_NAME" > "$BACKUP_DIR/database.sql" || true
  fi
  echo "Creating project files backup..."
  tar --exclude='./backups' --exclude='./postgres_data' --exclude='./.git' -czf "$BACKUP_DIR/project-files.tar.gz" .
  if [ -f .env ]; then cp .env "$BACKUP_DIR/.env.backup"; chmod 600 "$BACKUP_DIR/.env.backup"; fi
  cd "$APP_DIR/backups"
  tar -czf "dbot-backup-$TS.tar.gz" "$TS"
  rm -rf "$TS"
  echo "Backup created: $APP_DIR/backups/dbot-backup-$TS.tar.gz"
}

verify_service_types_api(){
  load_env
  local port="${API_PORT:-8000}" code="" ctype="" header_file body_file
  header_file="$(mktemp)"
  body_file="$(mktemp)"
  code="$(curl -sS --max-time 10 -D "$header_file" -o "$body_file" -w '%{http_code}' "http://127.0.0.1:${port}/api/admin/service-types" 2>/dev/null || true)"
  ctype="$(awk 'BEGIN{IGNORECASE=1} /^Content-Type:/{print tolower($0)}' "$header_file" | tail -n1)"
  rm -f "$header_file" "$body_file"
  case "$code" in
    200) grep -qi 'application/json' <<<"$ctype" ;;
    401|403) grep -qi 'application/json' <<<"$ctype" ;;
    *) return 1 ;;
  esac
}

wait_for_api_health(){
  load_env
  local port="${API_PORT:-8000}" i
  for i in $(seq 1 40); do
    if curl -fsS --max-time 4 "http://127.0.0.1:${port}/health" >/dev/null 2>&1; then
      return 0
    fi
    sleep 2
  done
  return 1
}

update_app(){
  cd_app
  ENV_BACKUP=""
  if [ -f .env ]; then ENV_BACKUP="$(mktemp)"; cp .env "$ENV_BACKUP"; fi
  if [ -d .git ]; then
    echo "Fetching latest source from GitHub..."
    git fetch origin main || echo "Git fetch failed. Rebuilding current local source..."
    if git rev-parse --verify origin/main >/dev/null 2>&1; then git reset --hard origin/main; fi
  else
    echo "Git repository not found. Rebuilding current local source..."
  fi
  if [ -n "$ENV_BACKUP" ] && [ -f "$ENV_BACKUP" ]; then cp "$ENV_BACKUP" .env; chmod 600 .env; rm -f "$ENV_BACKUP"; fi

  # Refresh the host-side CLI immediately from the source that was just pulled.
  # This is intentionally done before Docker rebuilds, so `dbot`/`dbot menu`
  # are fixed even if a later container build fails.
  if [ -f "$APP_DIR/scripts/repair-dbot-cli.sh" ]; then
    bash "$APP_DIR/scripts/repair-dbot-cli.sh"
  else
    install_manager_self
  fi

  $COMPOSE up -d --build

  echo "Waiting for the API to become healthy..."
  if ! wait_for_api_health; then
    echo -e "${C_RED}Update failed: API did not become healthy.${C_NC}"
    return 1
  fi

  echo "Verifying the Service Types JSON endpoint..."
  if ! verify_service_types_api; then
    echo -e "${C_YELLOW}The API container is still serving an old/static build. Rebuilding API without cache...${C_NC}"
    $COMPOSE build --no-cache api || return 1
    $COMPOSE up -d --force-recreate api || return 1
    wait_for_api_health || { echo -e "${C_RED}API did not recover after the forced rebuild.${C_NC}"; return 1; }
    verify_service_types_api || {
      echo -e "${C_RED}Update verification failed: /api/admin/service-types is not registered as a JSON endpoint in the running API container.${C_NC}"
      echo "Check that the latest source was pushed to the main branch, then run dbot update again."
      return 1
    }
  fi
  echo -e "${C_GREEN}Service Types API verification passed.${C_NC}"
}

uninstall_app(){
  PURGE="${1:-}"
  echo; echo -e "${C_RED}WARNING! This will remove D Bot from this VPS.${C_NC}"; echo
  echo "Project directory: $APP_DIR"
  echo "Docker containers, volumes, networks, database files, Redis data, .env and CLI commands will be removed."
  [ "$PURGE" = "--purge" ] && echo "Backups will also be deleted." || echo "Backups will be moved to: $BACKUP_KEEP_DIR"
  echo
  read -r -p "Type yes to continue: " CONFIRM
  [ "$CONFIRM" = "yes" ] || { echo "Uninstall cancelled."; exit 0; }
  if [ -d "$APP_DIR" ]; then
    cd "$APP_DIR"
    $COMPOSE down -v --remove-orphans || true
    if [ "$PURGE" != "--purge" ] && [ -d "$APP_DIR/backups" ]; then
      mkdir -p "$BACKUP_KEEP_DIR"; TS="$(date +%Y%m%d_%H%M%S)"; mv "$APP_DIR/backups" "$BACKUP_KEEP_DIR/backups-$TS" || true
    fi
    rm -rf "$APP_DIR"
  fi
  rm -f /usr/local/bin/dbot /usr/local/bin/d-bot
  echo "D Bot has been completely removed."
}

show_main_menu(){
  printf '%b\n' "${C_CYAN}╔══════════════════════════════════════════════════════════════╗${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}1${C_NC}) Status                  ${C_DIM}Show containers${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}2${C_NC}) Logs                    ${C_DIM}Live logs, Ctrl+C to exit${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}3${C_NC}) Restart                 ${C_DIM}Restart all services${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}4${C_NC}) Start                   ${C_DIM}Start services${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}5${C_NC}) Stop                    ${C_DIM}Stop services${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}6${C_NC}) Update                  ${C_DIM}Pull/rebuild and restart${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}7${C_NC}) Backup                  ${C_DIM}Create a full backup${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}8${C_NC}) Setup Info              ${C_DIM}View values from setup wizard${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}9${C_NC}) Edit Setup              ${C_DIM}Change saved .env values${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}10${C_NC}) Apply Nginx/SSL        ${C_DIM}Rebuild reverse proxy/certificate${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}11${C_NC}) Credentials Center     ${C_DIM}Live username/password and secrets${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_RED}12${C_NC}) Uninstall --purge     ${C_DIM}Remove app and backups${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}13${C_NC}) Delete Old Backup       ${C_DIM}List/delete saved backups${C_NC}"
  printf '%b\n' "${C_CYAN}║${C_NC} ${C_YELLOW}0${C_NC}) Exit"
  printf '%b\n' "${C_CYAN}╚══════════════════════════════════════════════════════════════╝${C_NC}"
}

control_menu(){
  while true; do
    header; service_card; echo; show_main_menu; echo
    read -r -p "Select an option: " choice || true
    case "$choice" in
      1|status) show_status; pause ;;
      2|logs) header; echo -e "${C_YELLOW}Showing live logs. Press Ctrl+C to exit logs.${C_NC}"; cd_app && $COMPOSE logs -f --tail=200 ;;
      3|restart) header; cd_app && $COMPOSE restart; echo -e "${C_GREEN}Done.${C_NC}"; pause ;;
      4|start) header; cd_app && $COMPOSE up -d; echo -e "${C_GREEN}Done.${C_NC}"; pause ;;
      5|stop) header; if confirm_action "Stop all D Bot services?"; then cd_app && $COMPOSE down; echo -e "${C_GREEN}Services stopped.${C_NC}"; fi; pause ;;
      6|update) header; if confirm_action "Update, rebuild and restart D Bot?"; then update_app; echo -e "${C_GREEN}Update completed.${C_NC}"; fi; pause ;;
      7|backup) header; backup_app; pause ;;
      8|info) show_setup_info; pause ;;
      9|edit) edit_setup_menu ;;
      10|nginx|ssl) header; configure_nginx_ssl; pause ;;
      11|secrets|credentials) credentials_center ;;
      12|uninstall|purge) header; uninstall_app "--purge"; exit 0 ;;
      13|backups|delete-backup|delete-backups) backup_manager ;;
      0|q|Q|exit) exit 0 ;;
      *) echo -e "${C_RED}Invalid option.${C_NC}"; sleep 1 ;;
    esac
  done
}

# Running only `dbot` or `dbot menu` opens the graphic Control Center.
# Direct commands like `dbot status` or `dbot restart` still work.
if [ "$#" -eq 0 ]; then
  control_menu
  exit 0
fi

case "${1}" in
  menu|m|control|center|--menu) control_menu ;;
  start) cd_app && $COMPOSE up -d ;;
  stop) cd_app && $COMPOSE down ;;
  restart) cd_app && $COMPOSE restart ;;
  logs) cd_app && $COMPOSE logs -f --tail=200 ;;
  update) update_app ;;
  backup) backup_app ;;
  backups|backup-manager|delete-backup|delete-backups) backup_manager ;;
  backup-list|list-backups) header; show_backup_inventory || true ;;
  status) show_status ;;
  secrets|credentials) credentials_center ;;
  remove|uninstall) uninstall_app "${2:-}" ;;
  help|-h|--help) control_menu ;;
  *) control_menu ;;
esac
